...
║                                                                              ║
║ Cambios V14 (build V21) respecto a V13 (build V20):                        ║
║                                                                              ║
║ CORRECCIONES CRÍTICAS (P0):                                                  ║
║  ✔ [V21-P1]  Estado transitorio en VRAM swap (H-01)                         ║
║  ✔ [V21-R2]  Lazy init de DB_PATH post-validación filesystem (H-02)         ║
║  ✔ [V21-A2]  Truncamiento inteligente de contexto en Agent Engine (H-04)    ║
║                                                                              ║
║ CORRECCIONES IMPORTANTES (P1):                                               ║
║  ✔ [V21-RAG3] Revalidación periódica de UUID ChromaDB (H-03)               ║
║  ✔ [V21-P2]  Error HTTP real en streaming pre-chunk (H-09)                  ║
║  ✔ [V21-E1]  Validación JSON en agent tasks endpoint (H-14)                ║
║  ✔ [V21-R3]  Validación anti-ciclos en fallback chain (H-18)               ║
║  ✔ [V21-MOD] Refactorización modular completa (H-29)                       ║
║                                                                              ║
║ CORRECCIONES MEDIAS (P2):                                                    ║
║  ✔ [V21-C3]  Cache con versionado (H-06)                                   ║
║  ✔ [V21-RL1] deque para rate limiting (H-07)                               ║
║  ✔ [V21-P4]  Thinking por modelo, no por nivel (H-12)                      ║
║  ✔ [V21-C2]  Coseno optimizado con numpy (H-20/H-37)                       ║
║  ✔ [V21-P3]  Reutilización de httpx.AsyncClient (H-22/H-38)                ║
║  ✔ [V21-M1]  Lock en lectura de métricas (H-24)                            ║
║  ✔ [V21-SEC1] API key opcional para endpoints admin (H-33)                 ║
║  ✔ [V21-RAG2] Nonce en delimitadores RAG (H-34)                            ║
║  ✔ [V21-A3]  Sanitización de respuesta del agente (H-35)                   ║
║  ✔ [V21-A5]  Tareas antiguas marcadas como fallidas en resume (H-41)       ║
║                                                                              ║
║ MEJORAS ADICIONALES:                                                         ║
║  ✔ [V21-RL2] Rate limiting con collections.deque (O(1) cleanup)            ║
║  ✔ [V21-HC1] Health check con cliente compartido                            ║
║  ✔ [V21-LOG1] Logging estructurado con RotatingFileHandler                 ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
async def _check_rate_limit() -> bool:
    ...
    async with _rate_limit_lock:
        # Limpiar entradas expiradas desde el frente (O(1) amortizado)
...
def _check_admin_auth(request: Request) -> bool:
    ...
    Si OMEN_ADMIN_KEY no está configurado, permite acceso libre (backward compatible).
    """
    if not _ADMIN_API_KEY:
        return True  # Sin key configurada = acceso libre
    auth = request.headers.get("authorization", "")
    if auth.startswith("Bearer "):
        return auth[7:] == _ADMIN_API_KEY
    return request.query_params.get("key", "") == _ADMIN_API_KEY
...
# ─────────────────────────────────────────────────────────────────────────────
# SHARED HTTP CLIENT — [V21-P3] Reutilización de conexiones
# ─────────────────────────────────────────────────────────────────────────────
...
async def lifespan(app: FastAPI):
    """Lifespan context manager para startup y graceful shutdown."""
    global _phi4_cpu_available, _shared_http_client

    log.info("═" * 66)
    log.info(" OMEN AI Router V14 (build V21) — iniciando…")
    log.info("═" * 66)
    ...
    # Reanudar tareas pendientes del agente
    ...
    log.info(f" Max active tasks: {MAX_ACTIVE_TASKS}")
    ...
    # Esperar tareas activas del agente
...
async def metrics(request: Request):
    ...
    return {
        ...
        "cache_hit_ratio": round(
            capas.get("cache", 0) / max(1, sum(capas.values())), 3
        ),
        "agent": {
...
async def chat(request: Request):
    ...
    if nivel == "PHI4_DIRECTO":
        ...
        # Redirigir a endpoint de agente
        return JSONResponse(
        ...
        async with _metricas_lock:
            # Incrementar alias counter en classifier
            pass
    ...
    # ── Ajustes del body ────────────────────────────────────────────────────
    body = inject_opciones_extra(body, nivel)
    body = inject_thinking(body, nivel, body["model"])
    check_tools(body, nivel, body["model"])
...
async def create_agent_task(request: Request):
    ...
    return JSONResponse(
        content={"task_id": task_id, "status": TaskStatus.PENDING, "message": "Tarea creada. Usa GET /v1/agent/tasks/{task_id} para consultar el progreso."},
        status_code=202,
    )
...
async def get_agent_task(task_id: str):
    ...
    with closing(_db_conn()) as conn:
        ...
        if not task:
            return JSONResponse(content={"error": {"message": "Tarea no encontrada", "type": "not_found"}}, status_code=404)

        subtasks = conn.execute(
            "SELECT id, seq_order, description, required_level, status, retry_count, error_feedback FROM subtasks WHERE task_id=? ORDER BY seq_order",
            (task_id,),
        ).fetchall()
...
async def cancel_agent_task(task_id: str):
    ...
    from omen_router_modules.agent_engine import _db_conn, _now_iso as agent_now
    with closing(_db_conn()) as conn:
        ...
        if task["status"] in TERMINAL_STATES:
            return JSONResponse(content={"message": f"Tarea ya finalizada con estado: {task['status']}"}, status_code=409)

        conn.execute("UPDATE tasks SET status=?, updated_at=? WHERE id=?", (TaskStatus.CANCELLED, _now_iso(), task_id))
        conn.commit()
...
async def get_agent_task_result(task_id: str):
    ...
    from omen_router_modules.agent_engine import _db_conn
    with closing(_db_conn()) as conn:
        task = conn.execute("SELECT status, final_result, error_message FROM tasks WHERE id=?", (task_id,)).fetchone()
            ...
            try:
                with closing(_db_conn()) as conn:
                    current = conn.execute(
                        "SELECT status, completed_subtasks, total_subtasks FROM tasks WHERE id=?", (task_id,)
                    ).fetchone()

                    new_logs = conn.execute(
                        "SELECT id, phase, message, timestamp FROM task_logs WHERE task_id=? AND id > ? ORDER BY id",
                        (task_id, last_log_id),
                    ).fetchall()
                ...
                for log_entry in new_logs:
                    ...
                    event_data = {"event": "log", "phase": log_entry["phase"], "message": log_entry["message"], "timestamp": log_entry["timestamp"]}
                    yield f"data: {json.dumps(event_data, ensure_ascii=False)}\n\n"

                progress_data = {"event": "progress", "status": current["status"], "completed": current["completed_subtasks"], "total": current["total_subtasks"]}
                yield f"data: {json.dumps(progress_data)}\n\n"

                if current["status"] in TERMINAL_STATES:
                    with closing(_db_conn()) as conn:
                        final = conn.execute("SELECT final_result, error_message FROM tasks WHERE id=?", (task_id,)).fetchone()
                    done_data = {"event": "done", "status": current["status"], "result": final["final_result"] if final else None, "error": final["error_message"] if final else None}
