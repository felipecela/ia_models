...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║ OMEN AI CLUSTER — Orchestrador Semántico V11 (build V18)                   ║
║ RTX 4070 8GB · Intel Ultra 7 · 32GB RAM · SSD exFAT                        ║
╠══════════════════════════════════════════════════════════════════════════════╣
║ V18 — Autonomous Reasoning Agent + Correcciones de auditoría:              ║
║  ✔ [V18-A1]  Autonomous Reasoning Agent: planificación, ejecución,          ║
║              validación e iteración autónoma de tareas complejas            ║
║  ✔ [V18-A2]  Task Manager con persistencia SQLite (agent_tasks.db)          ║
║  ✔ [V18-A3]  Planner Module: descomposición inteligente de tareas           ║
║  ✔ [V18-A4]  Execution Engine: bucle asíncrono con delegación interna       ║
║  ✔ [V18-A5]  Validation Loop: verificación + feedback + reintentos          ║
║  ✔ [V18-A6]  Consolidator: fusión final de resultados parciales             ║
║  ✔ [V18-A7]  Endpoints REST: /v1/agent/tasks (POST, GET, LIST)             ║
║  ✔ [V18-A8]  VRAM Affinity: agrupación de subtareas por modelo para         ║
║              minimizar conmutaciones de contenedores Docker                 ║
║  ✔ [V18-A9]  Graceful shutdown: persistencia de estado ante SIGTERM         ║
║  ✔ [V18-A10] Health check ampliado con estado del Agent Engine              ║
║  ✔ [V18-A11] Streaming de progreso via SSE en /v1/agent/tasks/{id}/stream  ║
║  ✔ [V18-A12] Rate limiting interno para evitar saturación de backends       ║
╠══════════════════════════════════════════════════════════════════════════════╣
║ Heredado de V10/V17 (funcionalidades completas):                           ║
║  ✔ Clasificador 4 capas (agente, caché, embeddings, phi4)                  ║
║  ✔ RAG ChromaDB con inyección contextual                                   ║
║  ✔ Conmutación de VRAM serializada con asyncio.Lock                        ║
║  ✔ Fallbacks automáticos por timeout                                       ║
║  ✔ TabbAPI model swap                                                       ║
║  ✔ Métricas thread-safe                                                     ║
║  ✔ RotatingFileHandler 50MB × 3                                            ║
╠══════════════════════════════════════════════════════════════════════════════╣
║ Niveles V18:                                                                ║
...
# ─────────────────────────────────────────────────────────────────────────────
_LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "router_v11.log")
...
log = logging.getLogger("router-v11")

# ─────────────────────────────────────────────────────────────────────────────
# RUTAS V18 — 7 niveles de razonamiento
# ─────────────────────────────────────────────────────────────────────────────
RUTAS: dict[str, dict] = {
    "CHAT": {
...
_EMBED_DESCRIPTIONS: dict[str, str] = {
    ...
    "PRECISO": (
        ...
        "física química biología ciencias posgrado lógica formal demostración"
    ),
    "MASIVO": (
...
# ─────────────────────────────────────────────────────────────────────────────
# CACHÉ LRU — clave incluye agent_id para evitar colisiones
# ─────────────────────────────────────────────────────────────────────────────
...
# ═══════════════════════════════════════════════════════════════════════════════
# AUTONOMOUS REASONING AGENT — [V18-A1] a [V18-A11]
# ═══════════════════════════════════════════════════════════════════════════════
...
# ─────────────────────────────────────────────────────────────────────────────
# [V18-A2] Usar AGENT_DB_DIR (ext4) si está definido, sino directorio del script
_AGENT_DB_DIR = os.environ.get("AGENT_DB_DIR", os.path.dirname(os.path.abspath(__file__)))
...
3. Asigna el nivel de razonamiento adecuado a cada subtarea:
   ...
   - PRECISO: matemáticas, STEM, lógica formal
...
async def _agent_llm_call(
    ...
    Usa la infraestructura de enrutamiento existente (conmutación VRAM, etc.)
    pero sin pasar por la capa HTTP pública.
    """
...
def _task_log(task_id: str, phase: str, message: str, subtask_id: str = None) -> None:
...
async def _plan_task(task_id: str, prompt: str) -> bool:
    ...
    # Persistir plan y subtareas en SQLite
    conn = _db_conn()
    try:
        conn.execute(
        ...
        conn.commit()
        _task_log(task_id, "PLANNING", f"Plan generado: {len(subtasks)} subtareas")
...
async def _validate_subtask(task_id: str, subtask: dict) -> bool:
    ...
    try:
        val_text = validation_raw.strip()
        if val_text.startswith("```"):
            val_text = val_text.split("\n", 1)[1] if "\n" in val_text else val_text
            if val_text.endswith("```"):
                val_text = val_text[:-3]
            val_text = val_text.strip()
        if val_text.startswith("json"):
            val_text = val_text[4:].strip()
...
async def _consolidate_task(task_id: str) -> bool:
    ...
    if len(subtasks) == 1:
        ...
        # Usar MASIVO o AGIL para consolidación (necesita contexto largo)
...
async def _conmutar_vram(nivel: str) -> str:
    ...
    Fallback inmediato ante docker.errors.NotFound.
...
async def lifespan(app: FastAPI):
    """[V18-A9] Lifespan context manager para startup y graceful shutdown."""
    global _phi4_model_activo, _rag_disponible, _chroma_collection_id, _phi4_cpu_available

    log.info("═" * 66)
    log.info(" OMEN AI Router V11 (build V18) — iniciando…")
    log.info("═" * 66)
    ...
    if not _phi4_cpu_available:
        log.warning(
            "[V18] phi4/phi4-mini NO disponible en Ollama CPU (:11435). "
    ...
    # Reanudar tareas pendientes del agente (si las hay tras un reinicio)
    ...
    # ── Shutdown ──────────────────────────────────────────────────────────
    log.info("[SHUTDOWN] Deteniendo tareas en background…")
    health_task.cancel()
    try:
        await health_task
    except asyncio.CancelledError:
        pass
    log.info("[SHUTDOWN] ✔ Router V11 detenido correctamente")


def _resume_pending_tasks() -> None:
    """Reanuda tareas que quedaron en estado EXECUTING tras un reinicio."""
    try:
        conn = _db_conn()
        pending = conn.execute(
...
# ═══════════════════════════════════════════════════════════════════════════════
# ENDPOINTS — Heredados de V10
# ═══════════════════════════════════════════════════════════════════════════════

@app.get("/")
async def raiz():
    return {
        "servicio": "OMEN AI Router V11",
        "build":    "V18",
        "version":  "11.18.0",
...
async def chat(request: Request):
    ...
    if nivel == "PHI4_DIRECTO":
        ...
        if _phi4_cpu_available:
            ...
            log.warning("[V18] phi4 no disponible en CPU — redirigiendo a GPU :11434")
...
async def create_agent_task(request: Request):
    ...
    max_iterations = min(body.get("max_iterations", 3), 10)  # Máximo 10 iteraciones
        ...
        while True:
            ...
            if task and task["status"] in (TaskStatus.COMPLETED, TaskStatus.FAILED, TaskStatus.CANCELLED):
                done_event = json.dumps({
                    "phase": "DONE",
                    "message": f"Tarea finalizada: {task['status']}",
                    "timestamp": _now_iso(),
                })
