...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V39 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V39"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V39"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V39]
...
# ─── [V39-O1] Esperar el SEGUNDO ciclo configure+doctor ─────────────────────
# Los logs confirman que el entrypoint ejecuta configure+doctor DOS veces:
#   1ª pasada: "no persisted config found" → escribe JSON base
#   2ª pasada: "merged persisted config"   → doctor borra providers
# Hay que esperar la señal de finalización de la 2ª pasada antes de inyectar.
info "Esperando el 2º ciclo configure+doctor de OpenClaw (máx ${TIMEOUT_OPENCLAW}s)…"
_OC_CONFIGURE_COUNT=0
_OC_DOCTOR_COUNT=0
OPENCLAW_READY=false
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    _LOG=$(docker logs openclaw-server 2>&1)
    _OC_CONFIGURE_COUNT=$(echo "$_LOG" | grep -c "config written")
    _OC_DOCTOR_COUNT=$(echo "$_LOG"    | grep -c "Doctor changes")
    # Condición: ≥2 "config written" (2ª pasada de configure) Y gateway activo
    if [[ "$_OC_CONFIGURE_COUNT" -ge 2 ]] && echo "$_LOG" | grep -q "Gateway is binding"; then
        OPENCLAW_READY=true
        ok "OpenClaw listo: ${_OC_CONFIGURE_COUNT}× configure, ${_OC_DOCTOR_COUNT}× doctor (${_oc_i}× 2s = $((_oc_i * 2))s)"
        break
    fi
    sleep 2
done
if [[ "$OPENCLAW_READY" == "false" ]]; then
    warn "Timeout — el 2º ciclo no completó en ${TIMEOUT_OPENCLAW}s. Inyectando de todos modos."
    warn "configure count: ${_OC_CONFIGURE_COUNT}, doctor count: ${_OC_DOCTOR_COUNT}"
fi
# Pausa de seguridad post-gateway (el proceso gateway arranca justo después del log)
sleep 4
...
GATEWAY_PID=$(docker exec openclaw-server sh -c \
    'pgrep -f "openclaw.*gateway\|openclaw.*run\|node.*openclaw\|openclaw serve" 2>/dev/null | head -1' \
    2>/dev/null || echo "")
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V39"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V39 — iniciado${NC}"
