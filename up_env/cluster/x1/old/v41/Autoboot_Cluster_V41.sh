...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V41 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V41"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V41"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V41]
# V41 corrige definitivamente el error 1006 eliminando cualquier recarga del
# contenedor/proceso gateway tras abrir la UI. La config completa se prepara
# ANTES del primer arranque y se monta como config persistida inicial.
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

info "[V41] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
# ─── [V41-O3/O5] Preseed de config persistida ANTES del arranque ────────────
info "[V41-O3] Generando config persistida inicial de OpenClaw…"
...
ok "Config persistida inicial preparada en $OPENCLAW_PRECONFIG"

# Pre-crear /data/.openclaw en el volumen para que el bind-mount :ro funcione
info "[V41] Pre-creando directorio de config en el volumen…"
docker run --rm \
    -v openclaw_data_final:/data \
    busybox sh -c "mkdir -p /data/.openclaw && chmod 700 /data/.openclaw" 2>/dev/null || true
ok "Directorio /data/.openclaw listo en el volumen"

info "[V41] Lanzando openclaw-server (imagen: $IMG_OPENCLAW)…"
docker run -d \
    ...
    -v openclaw_data_final:/data \
    -v "$OPENCLAW_PRECONFIG":/data/.openclaw/openclaw.json:ro \
    "$IMG_OPENCLAW"

# ─── [V41-O1] Esperar señal real: [gateway] ready ───────────────────────────
info "[V41-O1] Esperando señal real del gateway ([gateway] ready) (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if [[ "$_OC_READY_COUNT" -ge 1 ]]; then
        OPENCLAW_READY=true
        ok "[V41-O1] Gateway listo: configure×${_OC_CONFIGURE_COUNT}, doctor×${_OC_DOCTOR_COUNT}, ready×${_OC_READY_COUNT} (${_oc_i}× 2s = $((_oc_i * 2))s)"
        break
    fi
    sleep 2
done
if [[ "$OPENCLAW_READY" == "false" ]]; then
    warn "[V41-O1] No apareció '[gateway] ready' en ${TIMEOUT_OPENCLAW}s"
    docker logs openclaw-server 2>&1 | tail -10 | while IFS= read -r line; do
        warn "  log: $line"
    done || true
fi
...
# ─── [V41-O2/O4] Sin restart, sin SIGHUP, sin POST /api/providers ───────────
OPENCLAW_URL_TOKEN="http://localhost:${PORT_OPENCLAW}/#token=${OPENCLAW_TOKEN}"
info "[V41-O2] Sin recargas del gateway: se evita el cierre WS 1006 por diseño"
info "[V41-O4] Sin POST /api/providers: los providers ya van en la config persistida"
echo ""
echo -e "  ${GRN}${BLD}→ ${OPENCLAW_URL_TOKEN}${NC}"
echo ""
if command -v xdg-open &>/dev/null; then
    xdg-open "$OPENCLAW_URL_TOKEN" 2>/dev/null &
    ok "[V41] Navegador abierto automáticamente con token embebido"
else
    info "[V41] xdg-open no disponible — copia la URL de arriba en el navegador"
fi

# ═══════════════════════════════════════════════════════════════════════════════
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V41"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V41 — iniciado${NC}"
