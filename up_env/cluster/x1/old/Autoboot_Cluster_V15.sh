#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║         OMEN AI CLUSTER — Autoboot_Cluster_V15.sh                       ║
# ╠══════════════════════════════════════════════════════════════════════════╣
# ║  Correcciones V15 sobre V14 (auditoría técnica):                         ║
# ║  ✔ [V15-I7]  set -euo pipefail: errores críticos detienen el script    ║
# ║  ✔ [V15-I8]  ROUTER_SCRIPT corregido: orchestrator_router_V8.py        ║
# ║  ✔ [V15-I9]  GATEWAY_TOKEN dinámico vía openssl rand (no hardcoded)    ║
# ║  ✔ [V15-O2]  TOP_K unificado a 6 en mcp.json (= router RAG_TOP_K)      ║
# ║  ✔ [V15-O4]  Log gestionado por Python RotatingFileHandler (sin >>)     ║
# ║  ✔ [V15-O5]  Validación JSON (python3) tras inyección de cada config   ║
# ╠══════════════════════════════════════════════════════════════════════════╣
# ║  Heredado de V15 (funcionalidades nuevas):                               ║
# ║  ✔ [V15-A] phi4-reasoning:plus  → nivel PRECISO (Phi Mayor Precisión)   ║
# ║  ✔ [V15-A] phi4-reasoning:14b-q4_K_M → nivel PRECISO_OPT (Optimizada)  ║
# ║  ✔ [V15-B] Obsidian Docker (puerto 3000) — base de conocimiento         ║
# ║  ✔ [V15-C] ChromaDB (puerto 8001) — motor RAG para el vault Obsidian    ║
# ║  ✔ [V15-D] SearXNG (puerto 8888) — búsqueda web privada, sin API keys   ║
# ║  ✔ [V15-E] OpenClaw con @coder @analyst @reasoner @researcher            ║
# ║             + plugins SearXNG + MCP knowledge-base ChromaDB              ║
# ╚══════════════════════════════════════════════════════════════════════════╝
#
# USO:
#   ./Autoboot_Cluster_V15.sh             # Arranque completo
#   ./Autoboot_Cluster_V15.sh --stop      # Parar todo
#   ./Autoboot_Cluster_V15.sh --status    # Estado de todos los servicios
#   ./Autoboot_Cluster_V15.sh --reindex   # Re-indexar vault Obsidian en ChromaDB
#
# [V15-I7] -e añadido: cualquier comando que falle (sin || handler) para el
...
# [V15-I8] Nombre correcto del router — era V6.py en V15, corregido a V8.py
...
# [V15-O4] El log es gestionado por Python (RotatingFileHandler 50MB×3).
# Esta variable solo se usa para mostrar la ruta en el resumen final.
ROUTER_LOG="$SCRIPT_DIR/router_v8.log"
ROUTER_PID_FILE="$SCRIPT_DIR/router_V15.pid"
OLLAMA_TMPDIR="${OLLAMA_TMPDIR:-/tmp/ollama_tmp}"

# [V15-I9] Archivo que persiste el token entre reinicios (generado una sola vez)
...
# [V15-I7] wait_http: corregido para compatibilidad con set -euo pipefail.
...
# ─────────────────────────────────────────────────────────────────────────────
if [ "${1:-}" = "--stop" ]; then
    step "Deteniendo clúster OMEN V15…"
    ...
    ok "Clúster V15 detenido."; exit 0
fi
...
# ─────────────────────────────────────────────────────────────────────────────
if [ "${1:-}" = "--status" ]; then
    step "Estado del clúster OMEN V15"
    echo ""
    declare -A CHK=(
...
echo ""
echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}║    OMEN AI CLUSTER V15 — INICIALIZANDO                          ║${NC}"
...
# [V15-I8] Verificar que el script del router V8 existe
[ -f "$ROUTER_SCRIPT" ] || {
...
# Verificar sintaxis Python del router antes de continuar [V15-O5 prevención]
...
# [V15-A] Verificar modelos Phi-4-reasoning
...
# [V15-I7] Añadido || true: si el PID file no existe, [ -f ] retorna 1,
...
# [V15-I7] sudo systemctl start ollama SIN || true: si Ollama no puede
...
if [ ! -f "$SEARXNG_CONFIG_DIR/settings.yml" ]; then
    SECRET=$(openssl rand -hex 32 2>/dev/null || echo "omen-cluster-searxng-secret-key-V15")
...
# ─────────────────────────────────────────────────────────────────────────────
#  PASO 9: OPENCLAW — con config V15 completa
# ─────────────────────────────────────────────────────────────────────────────
step "PASO 9/11 — OpenClaw (puerto 8080)"

# ── [V15-I9] Token de autenticación dinámico ────────────────────────────────
...
docker volume create openclaw_data_V15 >/dev/null 2>&1 || true
...
# ── Inyectar openclaw.json V15 (agentes + PRECISO + plugins SearXNG) ────────
info "Inyectando openclaw.json V15…"
docker exec openclaw-server bash -c 'cat > /data/.openclaw/openclaw.json' << 'OPENCLAW_V15'
{
  "gateway": {
    "bind": "lan",
    "controlUi": {
...
# ── [V15-O5] Validar openclaw.json inmediatamente tras la inyección ──────────
...
docker exec openclaw-server bash -c 'cat > /data/.openclaw/mcp.json' << 'MCP_V15'
{
  "mcpServers": {
    "knowledge-base": {
      "command": "npx",
...
# ── [V15-O5] Validar mcp.json inmediatamente tras la inyección ───────────────
...
info "Reiniciando OpenClaw para aplicar configuración V15…"
...
ok "OpenClaw V15 configurado: @coder @analyst @reasoner @researcher + RAG + web search"

# ─────────────────────────────────────────────────────────────────────────────
#  PASO 10: ROUTER SEMÁNTICO V8 (build V15) — arranca ÚLTIMO
# ─────────────────────────────────────────────────────────────────────────────
step "PASO 10/11 — Router Semántico V8 build V15 (puerto 8000)"

# [V15-O4] El router ya NO se lanza con >> $ROUTER_LOG 2>&1.
...
ok "Router V8 (build V15) activo (PID: $(cat "$ROUTER_PID_FILE"))"
...
# [V15] Configurar cron para re-indexación automática cada hora.
...
echo -e "${GREEN}${BOLD}║          ✔ CLÚSTER OMEN V15 COMPLETAMENTE OPERATIVO             ║${NC}"
echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo ""
echo -e "  ${CYAN}${BOLD}[V15-A] Modelos de razonamiento Phi-4:${NC}"
...
echo ""
echo -e "  ${CYAN}${BOLD}[V15-B/C/D] Herramientas de conocimiento:${NC}"
...
echo ""
echo -e "  ${CYAN}${BOLD}[V15] Correcciones aplicadas:${NC}"
