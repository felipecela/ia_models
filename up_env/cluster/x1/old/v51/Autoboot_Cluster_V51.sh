...
# ║             Se define SCRIPT_VERSION="V51" al inicio del script.           ║
...
set -euo pipefail
SCRIPT_VERSION="V51"  # [V44-B1] Versión del script — usada en banners y logs
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V51 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V51"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V51"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V51]
#
# ROOT CAUSE confirmado V50:
#   inject_v50.py fallaba los 3 reintentos porque el heredoc dentro del
#   bucle `for _inj_retry in 1 2 3` no funciona con `docker exec … sh -c`:
#     docker exec openclaw-server sh -c "cat > /tmp/inject.py" << INJECT_EOF
#   El stdin del heredoc lo consume el shell del HOST, no el proceso dentro
#   del contenedor. docker exec no hereda el stdin de heredocs del padre.
#   Resultado: /tmp/inject_v50.py quedaba vacío → python3 lo ejecutaba
#   sin contenido → salía con código 0 pero sin escribir nada en el config.
#
# FIX V51:
#   1. Escribir inject_v51.py en el HOST con `cat > /tmp/inject_v51_host.py`
#      usando un heredoc FUERA de cualquier bucle.
#   2. Aplicar sed PORT_ROUTER_PLACEHOLDER en el HOST.
#   3. Copiar al contenedor con `docker cp` (ruta garantizada).
#   4. Ejecutar con `docker exec … python3 /tmp/inject_v51.py`.
#   5. Sin heredocs dentro de bucles — nunca más.
# ═══════════════════════════════════════════════════════════════════════════════
...
# ─── Limpieza ────────────────────────────────────────────────────────────────
info "[V51] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
# ─── Preseed mínimo ────────────────────────────────────────────────────────
info "[V51] Generando preseed (gateway.auth + controlUi)…"
...
info "[V51] Copiando preseed al volumen…"
docker run --rm \
...
# ─── Escribir inject_v51.py en el HOST (FUERA de cualquier bucle) ─────────────
# [V51] Esta es la corrección clave respecto a V50.
#       El heredoc se ejecuta aquí, en el nivel raíz del script,
#       no dentro de un bucle. El resultado se guarda en el HOST
#       y se copia al contenedor con docker cp.
_INJECT_HOST_PATH="/tmp/inject_v51_host.py"
cat > "$_INJECT_HOST_PATH" << 'INJECT_PYTHON_EOF'
import json, sys
...
try:
    ...
    print(f"[V51] ERROR leyendo modelos: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"[V51] WARN: config no legible ({e}), partiendo de dict vacío")
    cfg = {}
...
for _rm in ("ollama", "openai"):
    ...
    if removed:
        print(f"[V51] Provider externo eliminado: {_rm}")
...
print(f"[V51] OK: provider registrado: {PROVIDER_ID}")
print(f"[V51] OK: {len(models)} modelos bajo models.providers.{PROVIDER_ID}.models")
print(f"[V51] OK: agents.defaults.model.primary = {PROVIDER_ID}/ruteador-auto")
print(f"[V51] OK: plugins.entries.{PROVIDER_ID}.enabled = true")
print(f"[V51] Providers en config: {list(cfg['models']['providers'].keys())}")
INJECT_PYTHON_EOF

# Sustituir PORT_ROUTER_PLACEHOLDER con el valor real en el HOST
sed -i "s|PORT_ROUTER_PLACEHOLDER|${PORT_ROUTER}|g" "$_INJECT_HOST_PATH"
ok "[V51] inject_v51.py escrito en HOST: $_INJECT_HOST_PATH"

# ─── Lanzar openclaw-server ───────────────────────────────────────────────────
info "[V51] Lanzando openclaw-server…"
docker run -d \
...
info "[V51] Esperando gateway (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if echo "$_LOG" | grep -q "\[health-monitor\] started"; then
        OPENCLAW_READY=true; ok "[V51] Gateway arrancado tras $((_oc_i * 2))s"; break
    fi
    if echo "$_LOG" | grep -q "\[gateway\] security warning"; then
        OPENCLAW_READY=true; ok "[V51] Gateway UP (security-warning) tras $((_oc_i * 2))s"; break
    fi
    sleep 2
done
[[ "$OPENCLAW_READY" == "false" ]] && warn "[V51] Timeout gateway (${TIMEOUT_OPENCLAW}s)"

# ─── Esperar estado "running" ─────────────────────────────────────────────────
info "[V51] Verificando estado del contenedor…"
_OC_RUNNING=false
for _cr in $(seq 1 20); do
    _ST=$(docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo "missing")
    if [[ "$_ST" == "running" ]]; then
        _OC_RUNNING=true; ok "[V51] Contenedor 'running' tras $((_cr * 3))s"; break
    fi
    sleep 3
done
[[ "$_OC_RUNNING" == "false" ]] && warn "[V51] Contenedor NO 'running' — estado: $( docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo '?' )"
sleep 3

# ─── Construir lista de modelos ───────────────────────────────────────────────
info "[V51] Consultando backends activos en Router V14…"
...
info "[V51] Backends activos: ${_ACTIVE_BKS}"

_MODELS_JSON=$(echo "$_HEALTH_JSON" | python3 - << 'PYEOF'
import json, sys
...
if [[ -z "$_MODELS_JSON" || "$_MODEL_COUNT" == "0" ]]; then
    warn "[V51] Usando lista de modelos estática (fallback)"
...
info "[V51] Modelos a registrar: ${_MODEL_COUNT}"

# ─── Copiar modelos y script al contenedor y ejecutar ─────────────────────────
info "[V51] Inyectando provider OMEN Router V14 en openclaw.json…"
echo "$_MODELS_JSON" > /tmp/oc_models_v51.json

_INJECT_OK=false
for _inj_retry in 1 2 3; do
    # docker cp: copia desde HOST al contenedor — sin heredoc, sin stdin
    docker cp /tmp/oc_models_v51.json openclaw-server:/tmp/oc_models.json 2>/dev/null || true
    docker cp "$_INJECT_HOST_PATH"    openclaw-server:/tmp/inject_v51.py  2>/dev/null || true

    _OC_INJ_LOG="/tmp/oc_inject_v51_try${_inj_retry}.log"
    docker exec openclaw-server python3 /tmp/inject_v51.py > "$_OC_INJ_LOG" 2>&1
    _INJ_EXIT=$?

    if [[ -f "$_OC_INJ_LOG" ]]; then
        while IFS= read -r _il; do
            if echo "$_il" | grep -qi "error\|fail"; then warn "  $_il"; else info "  $_il"; fi
        done < "$_OC_INJ_LOG"
    fi

    if [[ $_INJ_EXIT -eq 0 ]] && grep -q "omen-router" "$_OC_INJ_LOG" 2>/dev/null; then
        _INJECT_OK=true
        ok "[V51] inject_v51.py completado en intento ${_inj_retry}"
        break
    else
        warn "[V51] inject fallido (intento ${_inj_retry}/3, exit=${_INJ_EXIT}) — esperando 5s…"
        sleep 5
    fi
done

if [[ "$_INJECT_OK" == "false" ]]; then
    warn "[V51] ATENCIÓN: inject_v51.py no pudo completar. OpenClaw usará provider 'openai'."
    warn "[V51] Ejecuta manualmente:"
    warn "[V51]   docker cp /tmp/oc_models_v51.json openclaw-server:/tmp/oc_models.json"
    warn "[V51]   docker cp $_INJECT_HOST_PATH openclaw-server:/tmp/inject_v51.py"
    warn "[V51]   docker exec openclaw-server python3 /tmp/inject_v51.py"
    warn "[V51]   docker kill --signal=HUP openclaw-server"
fi

# ─── SIGHUP para reload del config ───────────────────────────────────────────
info "[V51] Enviando SIGHUP al gateway para reload del config…"
...
if [[ -n "$_GW_PID" && "$_GW_PID" =~ ^[0-9]+$ ]]; then
    ...
    ok "[V51] SIGHUP enviado al PID ${_GW_PID}"
else
    docker kill --signal=HUP openclaw-server 2>/dev/null || true
    ok "[V51] SIGHUP enviado al contenedor"
fi
sleep 4
...
info "[V51] Verificando config final…"
_OC_VERIFY_LOG="/tmp/oc_verify_v51.log"
docker exec openclaw-server python3 /tmp/inject_v51.py --verify > "$_OC_VERIFY_LOG" 2>&1 || true

# Verificar usando python3 -c inline (sin fichero, siempre funciona)
...
try:
    ...
    print(f'[V51] Providers registrados: {list(providers.keys())}')
    print(f'[V51] Modelo primario: {primary}')
    for pid, pdata in providers.items():
        mcount = len(pdata.get('models', []))
        print(f'[V51]   {pid} -> {pdata.get(\"baseUrl\", \"?\")}  ({mcount} modelos)')
    if 'omen-router' in providers:
        print('[V51] STATUS: OK omen-router registrado correctamente')
    else:
        print('[V51] STATUS: FALLO omen-router NO encontrado en providers')
        sys.exit(2)
except Exception as e:
    print(f'[V51] ERROR verificando config: {e}')
    sys.exit(1)
" > "$_OC_VERIFY_LOG" 2>&1
_VERIFY_EXIT=$?
...
if [[ $_VERIFY_EXIT -ne 0 ]]; then
    warn "[V51] Config final sin omen-router. Comandos manuales:"
    warn "[V51]   docker cp /tmp/oc_models_v51.json openclaw-server:/tmp/oc_models.json"
    warn "[V51]   docker cp $_INJECT_HOST_PATH openclaw-server:/tmp/inject_v51.py"
    warn "[V51]   docker exec openclaw-server python3 /tmp/inject_v51.py"
    warn "[V51]   docker kill --signal=HUP openclaw-server"
...
if command -v xdg-open &>/dev/null; then
    ...
    ok "[V51] Navegador abierto con token embebido"
else
    info "[V51] Abre manualmente: ${OPENCLAW_URL_TOKEN}"
fi

# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V51"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V51 — iniciado${NC}"
