...
# ║             Se define SCRIPT_VERSION="V52" al inicio del script.           ║
...
set -euo pipefail
SCRIPT_VERSION="V52"  # [V44-B1] Versión del script — usada en banners y logs
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V52 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V52"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V52"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V52]
#
...
# ─── Limpieza ────────────────────────────────────────────────────────────────
info "[V52] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
info "[V52] Generando preseed (gateway.auth + controlUi)…"
...
info "[V52] Copiando preseed al volumen…"
docker run --rm \
...
# ─── Escribir inject_v52.py en el HOST (FUERA de cualquier bucle) ─────────────
...
# El heredoc usa comillas simples 'INJECT_PY_V52_EOF' para que bash NO expanda
# nada dentro; la sustitución de PORT_ROUTER se hace con sed después.
_INJECT_HOST_PATH="/tmp/inject_v52_host.py"
cat > "$_INJECT_HOST_PATH" << 'INJECT_PY_V52_EOF'
import json, sys
...
try:
    ...
    print("[V52] ERROR leyendo modelos: " + str(e))
    sys.exit(1)
...
try:
    ...
    print("[V52] WARN config no legible (" + str(e) + "), partiendo de vacio")
    cfg = {}
...
for _rm in ("ollama", "openai"):
    ...
    if removed:
        print("[V52] Provider externo eliminado: " + _rm)
...
print("[V52] OK: provider registrado: " + PROVIDER_ID)
print("[V52] OK: " + str(model_count) + " modelos bajo models.providers." + PROVIDER_ID + ".models")
print("[V52] OK: agents.defaults.model.primary = " + primary_now)
print("[V52] OK: plugins.entries." + PROVIDER_ID + ".enabled = true")
print("[V52] Providers en config: " + str(providers_now))
...
info "[V52] Esperando gateway (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if echo "$_LOG" | grep -q "\[health-monitor\] started"; then
        OPENCLAW_READY=true; ok "[V52] Gateway arrancado (health-monitor) tras $((_oc_i * 2))s"; break
    fi
    if echo "$_LOG" | grep -q "\[gateway\] security warning"; then
        OPENCLAW_READY=true; ok "[V52] Gateway UP (security-warning) tras $((_oc_i * 2))s"; break
    fi
    sleep 2
done
[[ "$OPENCLAW_READY" == "false" ]] && warn "[V52] Timeout gateway (${TIMEOUT_OPENCLAW}s)"

# ─── Esperar estado "running" ─────────────────────────────────────────────────
info "[V52] Verificando estado del contenedor…"
_OC_RUNNING=false
for _cr in $(seq 1 20); do
    _ST=$(docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo "missing")
    if [[ "$_ST" == "running" ]]; then
        _OC_RUNNING=true; ok "[V52] Contenedor 'running' tras $((_cr * 3))s"; break
    fi
    sleep 3
done
[[ "$_OC_RUNNING" == "false" ]] && warn "[V52] Contenedor NO 'running' — estado: $(docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo '?')"
sleep 3
...
info "[V52] Esperando que Router V14 reporte backends activos…"
_HEALTH_JSON="{}"
...
" 2>/dev/null || echo "0")
    if [[ "$_BK" -gt 0 ]]; then
        _HEALTH_JSON="$_HJ"
        ok "[V52] Router reporta ${_BK} backends activos (intento ${_rw})"
        break
    fi
    info "[V52] Router sin backends aún (intento ${_rw}/10) — esperando 2s…"
    sleep 2
done

# ─── Construir lista de modelos ───────────────────────────────────────────────
info "[V52] Consultando backends activos en Router V14…"
...
info "[V52] Backends activos: ${_ACTIVE_BKS}"
...
if [[ -z "$_MODELS_JSON" || "$_MODEL_COUNT" -lt 2 ]]; then
    warn "[V52] Router sin backends confirmados — usando lista estática completa"
    _MODELS_JSON='[
...
info "[V52] Modelos a registrar: ${_MODEL_COUNT}"

# ─── Copiar modelos y script al contenedor y ejecutar ─────────────────────────
info "[V52] Inyectando provider OMEN Router V14 en openclaw.json…"
echo "$_MODELS_JSON" > /tmp/oc_models_v52.json

_INJECT_OK=false
for _inj_retry in 1 2 3; do
    # docker cp: copia desde HOST al contenedor — sin heredoc, sin stdin problems
    docker cp /tmp/oc_models_v52.json openclaw-server:/tmp/oc_models.json  2>/dev/null || true
    docker cp "$_INJECT_HOST_PATH"    openclaw-server:/tmp/inject_v52.py   2>/dev/null || true

    _OC_INJ_LOG="/tmp/oc_inject_v52_try${_inj_retry}.log"
    docker exec openclaw-server python3 /tmp/inject_v52.py > "$_OC_INJ_LOG" 2>&1
    _INJ_EXIT=$?
    ...
    if [[ $_INJ_EXIT -eq 0 ]] && grep -q "omen-router" "$_OC_INJ_LOG" 2>/dev/null; then
        _INJECT_OK=true
        ok "[V52] inject_v52.py completado en intento ${_inj_retry}"
        break
    else
        warn "[V52] inject fallido (intento ${_inj_retry}/3, exit=${_INJ_EXIT}) — esperando 5s…"
        sleep 5
    fi
done

if [[ "$_INJECT_OK" == "false" ]]; then
    warn "[V52] ATENCIÓN: inject no pudo completar. OpenClaw usará config de configure/doctor."
    warn "[V52] Comandos manuales para corregir:"
    warn "[V52]   docker cp /tmp/oc_models_v52.json openclaw-server:/tmp/oc_models.json"
    warn "[V52]   docker cp $_INJECT_HOST_PATH openclaw-server:/tmp/inject_v52.py"
    warn "[V52]   docker exec openclaw-server python3 /tmp/inject_v52.py"
    warn "[V52]   docker kill --signal=HUP openclaw-server"
fi

# ─── SIGHUP para reload del config ───────────────────────────────────────────
info "[V52] Enviando SIGHUP al gateway para reload del config…"
...
if [[ -n "$_GW_PID" && "$_GW_PID" =~ ^[0-9]+$ ]]; then
    ...
    ok "[V52] SIGHUP enviado al PID ${_GW_PID}"
else
    docker kill --signal=HUP openclaw-server 2>/dev/null || true
    ok "[V52] SIGHUP enviado al contenedor"
fi
sleep 4
...
info "[V52] Verificando config final…"
_OC_VERIFY_LOG="/tmp/oc_verify_v52.log"
...
try:
    ...
    print('[V52] Providers registrados: ' + str(provider_keys))
    print('[V52] Modelo primario: ' + primary_val)
    for pid in provider_keys:
        pdata  = providers[pid]
        mcount = len(pdata.get('models', []))
        base   = pdata.get('baseUrl', '?')
        print('[V52]   ' + pid + ' -> ' + base + '  (' + str(mcount) + ' modelos)')
    if 'omen-router' in providers:
        mc = len(providers['omen-router'].get('models', []))
        print('[V52] STATUS: OK omen-router registrado correctamente (' + str(mc) + ' modelos)')
    else:
        print('[V52] STATUS: FALLO omen-router NO encontrado en providers')
        sys.exit(2)
except Exception as e:
    print('[V52] ERROR verificando config: ' + str(e))
    sys.exit(1)
" > "$_OC_VERIFY_LOG" 2>&1
_VERIFY_EXIT=$?
...
if [[ $_VERIFY_EXIT -ne 0 ]]; then
    warn "[V52] Config sin omen-router. Corrección manual:"
    warn "[V52]   docker cp /tmp/oc_models_v52.json openclaw-server:/tmp/oc_models.json"
    warn "[V52]   docker cp $_INJECT_HOST_PATH openclaw-server:/tmp/inject_v52.py"
    warn "[V52]   docker exec openclaw-server python3 /tmp/inject_v52.py"
    warn "[V52]   docker kill --signal=HUP openclaw-server"
...
if command -v xdg-open &>/dev/null; then
    ...
    ok "[V52] Navegador abierto con token embebido"
else
    info "[V52] Abre manualmente: ${OPENCLAW_URL_TOKEN}"
fi

# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V52"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V52 — iniciado${NC}"
