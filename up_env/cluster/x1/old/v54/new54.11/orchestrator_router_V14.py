...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # [V28-PHI4] Normalizar a lowercase para detectar phi4-mini:latest y variantes
            modelos_cpu_set = {m.lower() for m in modelos_cpu}
            _phi4_cpu_available = any(
                m in modelos_cpu_set
                for m in ["phi4-mini", "phi4-mini:latest", "phi4", "phi4:latest"]
            )
    except Exception:
...
async def chat(request: Request):
    ...
    # ── RAG injection ───────────────────────────────────────────────────────
    # [V28-RAG-SKIP] Evitar inyectar contexto RAG en prompts triviales/saludos.
    # Un "hola" con RAG puede generar ~3000 tokens extra antes incluso de las tools.
    _prompt_lower = prompt.strip().lower()
    _skip_rag = (
        len(_prompt_lower) < 40
        or _prompt_lower in {"hola", "hi", "hello", "buenas", "gracias", "ok", "test", "ping"}
    )
    if not _skip_rag:
        body = await rag_inject(body, prompt, nivel, _shared_http_client)
    else:
        log.debug("[RAG] omitido por prompt trivial/corto (len=%d)", len(_prompt_lower))
