...
def validate_and_truncate_messages(body: dict, nivel: str, model: str) -> dict:
    ...
    # [V28-SAFE-TOOLS] Si el exceso está en los schemas de tools, eliminarlos primero.
    # Cubre el caso: prompt trivial ("hola") + tools pesados → exceso que el
    # truncado de mensajes no puede resolver porque el mensaje ya es mínimo.
    if "tools" in body:
        body.pop("tools", None)
        body.pop("tool_choice", None)
        new_est_notools = estimate_body_tokens(body)
        log.info(
            "[V28-SAFE-TOOLS] tools eliminadas: %d -> %d tokens",
            estimated, new_est_notools,
        )
        if new_est_notools <= max_input_tokens:
            return body
        estimated = new_est_notools
