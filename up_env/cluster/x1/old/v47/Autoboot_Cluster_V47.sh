...
# ║             Se define SCRIPT_VERSION="V47" al inicio del script.           ║
...
set -euo pipefail
SCRIPT_VERSION="V47"  # [V44-B1] Versión del script — usada en banners y logs
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V47 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V47"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V47"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V47]
#
# SCHEMA REAL confirmado por 'cat /data/.openclaw/openclaw.json':
#
#   models.providers  → OBJETO (dict), clave = ID del provider
#     "omen-router": {
...
#     }
#   agents.defaults.model.primary  → "omen-router/ruteador-auto"
#   plugins.entries."omen-router"  → {enabled: true}
#
# MÉTODO: python3 dentro del contenedor modifica openclaw.json directamente
# con la estructura exacta. La CLI 'openclaw config set' se usa solo para las
# claves escalares simples; los modelos (array de objetos) se escriben con python3.
# Después se envía SIGHUP al proceso node para reload en caliente.
#
# FIX SIGPIPE (error 141): todos los docker exec redirigen a fichero log,
# sin pipes en primer plano bajo set -euo pipefail.
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

# ─── Constante ID del provider (debe ser un identificador simple sin espacios) ─
_OC_PROVIDER_ID="omen-router"

# ─── Limpieza contenedor anterior ────────────────────────────────────────────
info "[V47] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
# configure.js leerá este archivo y añadirá el provider Ollama automáticamente.
# Eso es aceptable — lo eliminaremos luego con python3 tras que doctor termine.
# Lo importante es que gateway.auth quede configurado desde el inicio.
info "[V47] Generando preseed mínimo (gateway.auth + controlUi)…"
...
ok "Preseed mínimo listo"

# ─── Copiar preseed al volumen antes del arranque ────────────────────────────
info "[V47] Copiando preseed al volumen…"
docker run -d --name openclaw-init \
    -v openclaw_data_final:/data \
    busybox sh -c "mkdir -p /data/.openclaw && sleep 15" >/dev/null 2>&1 || true
_INIT_READY=false
...
if [[ "$_INIT_READY" == "true" ]]; then
    ...
    docker run --rm \
        ...
        busybox sh -c "mkdir -p /data/.openclaw && cp /tmp/preconfig.json /data/.openclaw/openclaw.json" 2>/dev/null || true
    ok "Preseed copiado (fallback busybox)"
fi
docker rm -f openclaw-init 2>/dev/null || true

# ─── Lanzar OpenClaw SIN OLLAMA_BASE_URL ─────────────────────────────────────
# Sin OLLAMA_BASE_URL configure autodetecta Ollama en host.docker.internal:11434
# (porque busca en la red interna). Aunque lo encuentre y añada el provider Ollama,
# lo eliminaremos después con python3 junto con la inyección del nuestro.
info "[V47] Lanzando openclaw-server…"
docker run -d \
...
# ─── Esperar que doctor termine (health-monitor started) ─────────────────────
info "[V47] Esperando que doctor + gateway terminen (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if echo "$_LOG" | grep -q "\[health-monitor\] started"; then
        OPENCLAW_READY=true
        ok "[V47] Gateway arrancado tras $((_oc_i * 2))s"
        break
    fi
    sleep 2
done
if [[ "$OPENCLAW_READY" == "false" ]]; then
    warn "[V47] Timeout esperando health-monitor"
    docker logs openclaw-server 2>&1 | tail -6 | while IFS= read -r l; do warn "  $l"; done || true
fi
# Pausa: asegurar que openclaw doctor haya escrito openclaw.json final
sleep 4

# ─── Consultar backends activos en Router V14 ────────────────────────────────
info "[V47] Consultando backends activos en Router V14…"
...
info "[V47] Backends activos: ${_ACTIVE_BKS}"

# ─── Construir lista de modelos según backends activos ───────────────────────
cat > /tmp/build_models_v47.py << 'PYEOF'
import json, sys
...
_MODELS_JSON=$(echo "$_HEALTH_JSON" | python3 /tmp/build_models_v47.py 2>/dev/null || echo "[]")
_MODEL_COUNT=$(echo "$_MODELS_JSON" | python3 -c "import json,sys; print(len(json.load(sys.stdin)))" 2>/dev/null || echo "0")

# Fallback estático si la lista quedó vacía
if [[ "$_MODELS_JSON" == "[]" || -z "$_MODELS_JSON" || "$_MODEL_COUNT" == "0" ]]; then
    warn "[V47] Usando lista de modelos estática (fallback)"
...
info "[V47] Modelos a registrar: ${_MODEL_COUNT}"

# ─── Copiar lista de modelos al contenedor como fichero ──────────────────────
_MODELS_FILE="/tmp/oc_models_v47.json"
echo "$_MODELS_JSON" > "$_MODELS_FILE"
docker cp "$_MODELS_FILE" openclaw-server:/tmp/oc_models.json 2>/dev/null || true

# ─── Escribir script de inyección y ejecutarlo en el contenedor ──────────────
# [V47] Usa la estructura REAL de openclaw.json:
#   models.providers = OBJETO, clave = ID del provider
#   agents.defaults.model.primary = "provider_id/model_id"
#   plugins.entries."provider_id".enabled = true
# Elimina el provider ollama (añadido por configure) y añade omen-router.
info "[V47] Inyectando provider OMEN Router V14 en openclaw.json…"
docker exec openclaw-server sh -c 'cat > /tmp/inject_v47.py' << 'PYEOF'
import json, sys
...
try:
    ...
    print(f"[V47] ERROR leyendo modelos: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"[V47] ERROR leyendo config: {e}")
    cfg = {}

# Asegurar que models.providers existe como objeto (dict)
...
# Eliminar el provider ollama (añadido automáticamente por configure)
cfg["models"]["providers"].pop("ollama", None)

# Añadir nuestro provider con la estructura correcta
...
# Habilitar el provider como plugin
...
if "entries" not in cfg["plugins"] or not isinstance(cfg["plugins"]["entries"], dict):
    cfg["plugins"]["entries"] = {}
cfg["plugins"]["entries"].pop("ollama", None)
...
print(f"[V47] OK: provider '{PROVIDER_ID}' registrado")
print(f"[V47] OK: {len(models)} modelos bajo models.providers.{PROVIDER_ID}.models")
print(f"[V47] OK: agents.defaults.model.primary = '{PROVIDER_ID}/ruteador-auto'")
print(f"[V47] OK: plugins.entries.{PROVIDER_ID}.enabled = true")
print(f"[V47] OK: provider 'ollama' eliminado")
PYEOF

# Reemplazar el placeholder del puerto con el valor real del script
docker exec openclaw-server sh -c \
    "sed -i 's|PORT_ROUTER_PLACEHOLDER|${PORT_ROUTER}|g' /tmp/inject_v47.py" 2>/dev/null || true

# Ejecutar el script — sin pipe en primer plano (fix SIGPIPE/error 141)
_OC_INJ_LOG="/tmp/oc_inject_v47.log"
docker exec openclaw-server python3 /tmp/inject_v47.py > "$_OC_INJ_LOG" 2>&1 || true
if [[ -f "$_OC_INJ_LOG" ]]; then
    while IFS= read -r _il; do
        if echo "$_il" | grep -qi "error\|fail"; then warn "  $_il"; else ok "  $_il"; fi
    done < "$_OC_INJ_LOG"
fi

# ─── SIGHUP al proceso node del gateway para reload del config ───────────────
info "[V47] Enviando SIGHUP al gateway (reload config en caliente)…"
...
if [[ -n "$_GW_PID" && "$_GW_PID" =~ ^[0-9]+$ ]]; then
    ...
    ok "[V47] SIGHUP enviado al PID ${_GW_PID}"
else
    docker kill --signal=HUP openclaw-server 2>/dev/null || true
    ok "[V47] SIGHUP enviado al contenedor"
fi
sleep 4

# ─── Verificar HTTP y config final ───────────────────────────────────────────
...
# ─── Verificar config final dentro del contenedor ───────────────────────────
info "[V47] Verificando config final…"
_OC_VERIFY_LOG="/tmp/oc_verify_v47.log"
...
try:
    ...
    print(f'[V47] Providers registrados: {list(providers.keys())}')
    print(f'[V47] Modelo primario: {primary}')
    for pid, pdata in providers.items():
        mcount = len(pdata.get('models', []))
        print(f'[V47]   {pid} → {pdata.get(\"baseUrl\",\"?\")}  ({mcount} modelos)')
except Exception as e:
    print(f'[V47] ERROR verificando config: {e}')
" > "$_OC_VERIFY_LOG" 2>&1 || true
if [[ -f "$_OC_VERIFY_LOG" ]]; then
    while IFS= read -r _vl; do ok "  $_vl"; done < "$_OC_VERIFY_LOG"
fi
...
if command -v xdg-open &>/dev/null; then
    ...
    ok "[V47] Navegador abierto con token embebido"
else
    info "[V47] Abre manualmente: ${OPENCLAW_URL_TOKEN}"
fi
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V47"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V47 — iniciado${NC}"
