...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V28"
...
if [[ "$_FLAG_SGLANG" == "true" ]]; then
    ...
    if is_container_running "exllamav2-api" && [[ "$TABBY_HEALTH" == "200" ]]; then
        ...
        # [V28-B1] Entrypoint correcto: 'python3 -m tabbyapi' antes de los args.
        # Sin él, Python intercepta --model-dir como su propio flag CLI.
        docker run -d \
            --name exllamav2-api \
            --network "$DOCKER_NET" \
            --gpus all \
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V28"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V28 — iniciado${NC}"
