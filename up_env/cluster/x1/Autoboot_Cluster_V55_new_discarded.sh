...
# ═══════════════════════════════════════════════════════════════════════════════
# 7.5/9 — NVIDIA AI ENGINE (NIM / Triton / vLLM :8002) [V55]
# ═══════════════════════════════════════════════════════════════════════════════
section "7.5/9 — NVIDIA AI Engine (:8002)"

PORT_NVIDIA_AI=8002
info "[V55] Verificando/Lanzando contenedor NVIDIA AI Engine..."

docker stop nvidia-ai-engine 2>/dev/null || true
docker rm nvidia-ai-engine 2>/dev/null || true

if command -v nvidia-smi &>/dev/null; then
    info "[V55] GPU detectada, preparando NVIDIA AI Engine..."
    # IMPORTANTE: Reemplaza nvcr.io/nvidia/tritonserver... por la imagen exacta 
    # de NIM o Triton que usabas, y ajusta los volúmenes de tus modelos descargados.
    docker run -d --name nvidia-ai-engine --gpus all \
        --network "$DOCKER_NET" \
        -p "${PORT_NVIDIA_AI}:8000" \
        -v "$AI_HOME/ia_models:/models" \
        --restart unless-stopped \
        nvcr.io/nvidia/tritonserver:23.10-vllm-python-py3 >/dev/null 2>&1 || warn "[V55] No se pudo lanzar la imagen de NVIDIA AI"
    
    # Check simple al puerto (adaptar el path de health según motor)
    if wait_port "NVIDIA AI Engine" localhost "$PORT_NVIDIA_AI" 15; then
        ok "NVIDIA AI Engine ✔ → http://localhost:${PORT_NVIDIA_AI}"
    else
        warn "NVIDIA AI Engine en puerto $PORT_NVIDIA_AI (puede tardar en cargar el modelo)"
    fi
else
    warn "NVIDIA AI Engine omitido (No se detectó GPU Nvidia)"
fi
...
_MODELS_JSON='[
  {"id":"deepseek-r1-14b", "name":"DeepSeek R1 14B (Ollama GPU)", "contextWindow":16384},
  {"id":"qwen2.5-32b", "name":"Qwen 2.5 32B (Ollama GPU)", "contextWindow":32768},
  {"id":"phi4-reasoning-plus", "name":"Phi-4 Reasoning Plus (Ollama GPU)", "contextWindow":32768},
  {"id":"phi4-mini", "name":"Phi-4 Mini (Ollama GPU)", "contextWindow":131072},
  {"id":"phi4-reasoning-14b", "name":"Phi-4 Reasoning 14B (Ollama GPU)", "contextWindow":32768},
  {"id":"phi4", "name":"Phi-4 (Ollama GPU)", "contextWindow":16384},
  {"id":"deepseek-coder-v2", "name":"DeepSeek Coder V2 (Ollama GPU)", "contextWindow":32768},
  {"id":"nomic-embed-text", "name":"Nomic Embed Text (Ollama CPU)", "contextWindow":8192},
  {"id":"llama-3.1-8b-awq", "name":"Llama 3.1 8B (SGLang)", "contextWindow":8192},
  {"id":"llama-3.1-8b-exl2", "name":"Llama 3.1 8B (TabbAPI)", "contextWindow":8192},
  {"id":"qwen2.5-coder-7b-exl2", "name":"Qwen 2.5 Coder 7B (TabbAPI)", "contextWindow":32768},
  {"id":"nvidia-nim-model", "name":"NVIDIA AI Engine Model", "contextWindow":8192}
]'
_MODEL_COUNT=12
...
_health_url() {
    ...
    case "$port" in
        ...
        8002)              echo "http://localhost:8002/v2/health/ready" ;;
...
check_service "NVIDIA AI Engine"         localhost    8002
