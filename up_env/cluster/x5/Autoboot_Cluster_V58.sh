#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║ OMEN AI Cluster — Autoboot V56 (consolidación V55 + análisis externo)      ║
# ║ RTX 4070 8GB · Intel Ultra 7 · 32GB RAM · SSD exFAT /mnt/ai_core          ║
# ╠══════════════════════════════════════════════════════════════════════════════╣
# ║ V56 — Consolidación (base V55 + mejoras del análisis externo):             ║
# ║  ✔ [V56-R1] Router V14 pasa a LEGACY: ya no es dependencia obligatoria.    ║
# ║             V55 lo verificaba y abortaba si faltaba, pero NUNCA lo         ║
# ║             arrancaba (LiteLLM lo sustituyó). Ahora solo warn informativo. ║
# ║  ✔ [V56-P1] BUG: primary inyectado era "litellm-omen/profundo-r1", un id   ║
# ║             que NO existe en la lista de modelos. Ahora el primary se      ║
# ║             calcula según el modo VRAM activo (--exl2/--sglang/default)    ║
# ║             y siempre apunta a un alias/modelo real. Se añaden fallbacks.  ║
# ║  ✔ [V56-M1] Lista de modelos de OpenClaw DINÁMICA desde LiteLLM            ║
# ║             /v1/models (solo lo realmente servible), con lista estática    ║
# ║             de respaldo basada en los 3 modelos nativos instalados.        ║
# ║  ✔ [V56-L1] PORT_LITELLM=4000 centralizado + incluido en check de puertos ║
# ║             (sustituye a PORT_ROUTER=8000 del router legacy).              ║
# ║  ✔ [V56-L2] Health check HTTP real para LiteLLM (/health/liveliness,       ║
# ║             90s) en lugar de wait_port TCP 30s (TCP abierto != API lista). ║
# ║  ✔ [V56-L3] LITELLM_MASTER_KEY definida y pasada al contenedor; coincide   ║
# ║             con el apiKey inyectado en OpenClaw (antes incoherente).       ║
# ║  ✔ [V56-N1] Flag --no-ollama: omite Ollama GPU/CPU (inventario actual sin  ║
# ║             contenedores Ollama). Activa --exl2 automático si no se        ║
# ║             indica motor. Evita re-pulls de ~40GB no deseados.             ║
# ║  ✔ [V56-B2] Fix set -e: _INJ_EXIT/_VERIFY_EXIT se capturaban con $? tras   ║
# ║             docker exec — bajo set -euo pipefail un exit!=0 abortaba el    ║
# ║             script antes de llegar al retry. Ahora patrón `|| var=$?`.     ║
# ║  ✔ [V56-S1] --status: entrada Router V14 (:8000, muerto) sustituida por    ║
# ║             LiteLLM (:4000). Banners V54/V55 unificados a $SCRIPT_VERSION. ║
# ╠══════════════════════════════════════════════════════════════════════════════╣
...
# ─── CHANGELOG V58 ────────────────────────────────────────────────────────────
#  ✔ [V58-A1] CAPA AGÉNTICA (Flow Engineering & Agentic Loops): nueva sección
#             9b/9. Aider (bucles agénticos de programación) + CrewAI (debate
#             lector↔crítico sobre el vault Obsidian). Ambos consumen los
#             modelos EXISTENTES vía LiteLLM (:4000) — sin motores nuevos y
#             sin tocar ningún servicio previo del clúster.
#  ✔ [V58-A2] Aider instalado en el host (aider-install → uv, Python 3.12,
#             entorno aislado — sin contenedor). Config en $AI_HOME/agents/
#             aider/ ajustada a modelos 7-8B: edit-format whole, map-tokens 0,
#             weak model orquestador/phi4-mini (CPU, sin swaps de GPU para
#             commits), timeout 1800s. Lanzador: $AI_HOME/bin/aider_omen.
#  ✔ [V58-A3] CrewAI en contenedor BAJO DEMANDA (imagen local crewai-debate,
#             no es un servicio residente). Proceso secuencial y CERO tools:
#             el knowledge source inyecta el contexto del vault en el prompt
#             — evita los fallos de tool-calling JSON de los modelos 7-8B.
#             Lector = instantaneo (GPU) · Crítico = orquestador/phi4-mini
#             (CPU): sin swaps de motor GPU en cada ronda del debate.
#             Roles y tareas editables en YAML sin rebuild (montados :ro).
#  ✔ [V58-A4] El resultado del debate se guarda como nota Markdown en
#             $VAULT_DIR/debates_ia/ → el indexador la incorpora a ChromaDB
#             (encadenado de conocimiento entre conversaciones e hilos).
#  ✔ [V58-A5] Flags nuevos: --no-agents (omite la capa agéntica) y --agents
#             (fuerza reinstalar Aider + rebuild de la imagen CrewAI).
#             --stop incluye crewai-debate; --status muestra la capa agéntica.
# ─────────────────────────────────────────────────────────────────────────

# ─── CHANGELOG V57 ────────────────────────────────────────────────────────────
#  ✔ [V57-W1] Nuevo modo --swap (RECOMENDADO): llama-swap (:9090) arranca y
#             detiene TabbyAPI/SGLang bajo demanda según el modelo pedido.
#             Cada modelo dispone de los 8GB de VRAM completos. Criterio:
#             penalizar TIEMPO de swap (1-3 min), nunca el RAZONAMIENTO.
#             Binario en $AI_HOME/bin/llama-swap (descarga automática) +
#             config $AI_HOME/llama-swap_config.yaml (obligatoria).
#  ✔ [V57-E1] Guard Ollama separado: GPU se omite con --no-ollama O --swap;
#             CPU solo con --no-ollama (embeddings nomic + phi4-mini para el
#             vault Obsidian/ChromaDB — sin consumo de VRAM).
#  ✔ [V57-C1] TabbyAPI --max-seq-len dinámico: 4096 conviviendo con Ollama,
#             8192 con VRAM completa (--no-ollama/--swap). Antes: 2048.
#  ✔ [V57-C2] TIMEOUT_LITELLM 90→180s; llama-swap healthCheckTimeout 900s;
#             LiteLLM request_timeout 1800s (en litellm_config.yaml).
#  ✔ [V57-X1] Placeholder NVIDIA NIM (:8002) descartado definitivamente
#             (sin espacio en disco para modelos NIM — decisión del usuario).
#  ✔ [V57-S1] --stop detiene llama-swap + contenedores swap-*; --status y
#             resumen incluyen llama-swap (:9090).
# ─────────────────────────────────────────────────────────────────────────

set -euo pipefail
SCRIPT_VERSION="V58"  # [V44-B1][V56-S1][V57][V58] Versión del script — usada en banners y logs
...
_FLAG_NO_OLLAMA=false # [V56-N1]
_FLAG_SWAP=false      # [V57-W1]
_FLAG_AGENTS=false    # [V58-A5] fuerza reinstalación Aider + rebuild imagen CrewAI
_FLAG_NO_AGENTS=false # [V58-A5] omite la capa agéntica por completo
...
║  Uso: ai_cluster [--last] [--swap|--exl2|--sglang] [--no-ollama] [--help]                    ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  FLAGS DISPONIBLES                                                         ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
...
║                                                                            ║
║  --no-ollama    Omite Ollama GPU/CPU por completo. Activa --exl2 de forma  ║
║                 automática si no se indica motor. Recomendado cuando solo  ║
║                 se usan los modelos nativos EXL2/AWQ instalados. [V56]     ║
║                                                                            ║
║  --swap         RECOMENDADO [V57]: llama-swap (:9090) arranca y detiene    ║
║                 TabbyAPI/SGLang bajo demanda según el modelo solicitado.   ║
║                 Cada modelo usa los 8GB de VRAM completos y contexto 8192. ║
║                 Penaliza tiempo de swap (1-3 min), nunca el razonamiento.  ║
║                 Omite Ollama GPU pero MANTIENE Ollama CPU (embeddings      ║
║                 nomic-embed-text + phi4-mini para el vault Obsidian).      ║
║                 Incompatible con --exl2/--sglang.                          ║
║                                                                            ║
║  --agents       [V58] Fuerza reinstalar Aider y reconstruir la imagen      ║
║                 CrewAI de la capa agéntica (útil para actualizar).         ║
║                                                                            ║
║  --no-agents    [V58] Omite la capa agéntica (Aider + CrewAI). Los         ║
║                 lanzadores ya generados siguen funcionando.                ║
║                                                                            ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  CAPA AGÉNTICA [V58]                                                       ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
║  aider_omen              Bucles agénticos de programación (Aider) contra   ║
║                          LiteLLM :4000. Ejecutar dentro de un repo git.    ║
║  debate_omen "tema"      Debate lector↔crítico sobre el vault Obsidian     ║
║                          (CrewAI). Guarda la nota final en debates_ia/.    ║
║                          Rondas: MAX_RONDAS=5 debate_omen "tema"           ║
║                                                                            ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  EJEMPLOS                                                                  ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
for _arg in "$@"; do
    case "$_arg" in
        ...
        --exl2)         # [V31-E1]
            ...
            ;;
        --no-ollama)    # [V56-N1]
            _FLAG_NO_OLLAMA=true
            ;;
        --swap)         # [V57-W1] llama-swap: motores bajo demanda, VRAM completa por modelo
            _FLAG_SWAP=true
            ;;
        --agents)       # [V58-A5] fuerza reinstalación/rebuild de la capa agéntica
            _FLAG_AGENTS=true
            ;;
        --no-agents)    # [V58-A5] omite la capa agéntica
            _FLAG_NO_AGENTS=true
            ;;
        *)
...
# [V57-W1] --swap es incompatible con --exl2/--sglang: llama-swap gestiona ambos motores
if [[ "$_FLAG_SWAP" == "true" ]] && { [[ "$_FLAG_EXL2" == "true" ]] || [[ "$_FLAG_SGLANG" == "true" ]]; }; then
    echo "[ERROR] --swap gestiona TabbyAPI y SGLang bajo demanda: no lo combines con --exl2/--sglang."
    exit 1
fi

# [V56-N1] Con --no-ollama y sin motor elegido, activar TabbAPI automáticamente
# para que el cluster arranque con al menos un motor de inferencia nativo.
if [[ "$_FLAG_NO_OLLAMA" == "true" ]] && [[ "$_FLAG_SGLANG" != "true" ]] && [[ "$_FLAG_EXL2" != "true" ]] && [[ "$_FLAG_SWAP" != "true" ]]; then
    _FLAG_EXL2=true
    echo "[INFO] --no-ollama sin motor indicado: activando --exl2 (TabbAPI EXL2) automáticamente."
fi
...
PORT_LITELLM=4000           # [V56-L1] LiteLLM Gateway (router activo del cluster)
PORT_LLAMASWAP=9090         # [V57-W1] llama-swap — orquestador de motores bajo demanda (:8080 lo usa OpenClaw)
...
LITELLM_MASTER_KEY="sk-litellm-local"               # [V56-L3] Debe coincidir con el apiKey inyectado en OpenClaw
...
TIMEOUT_LITELLM=180          # [V56-L2][V57-C2] Ampliado 90→180s (criterio: penalizar tiempo, no capacidad)
TIMEOUT_LLAMASWAP=60         # [V57-W1] Solo el proxy — los modelos cargan bajo demanda (1-3 min por swap)

# ─── [V57-W1] llama-swap ──────────────────────────────────────────────────────
LLAMASWAP_BIN="$AI_HOME/bin/llama-swap"
LLAMASWAP_CONFIG="$AI_HOME/llama-swap_config.yaml"
LLAMASWAP_PID_FILE="$AI_HOME/llama_swap.pid"

# ─── [V58-A1] Capa agéntica: Aider + CrewAI ──────────────────────────────────
AGENTS_DIR="$AI_HOME/agents"                 # raíz de la capa agéntica (ext4)
AIDER_DIR="$AGENTS_DIR/aider"                # configs de Aider (host, sin Docker)
CREWAI_DIR="$AGENTS_DIR/crewai_debate"       # proyecto CrewAI (contenedor bajo demanda)
CREWAI_IMAGE="crewai-debate:v58"             # imagen local — se construye una vez
CREWAI_CONTAINER="crewai-debate"             # contenedor efímero (--rm) por debate
DEBATE_OUT_SUBDIR="debates_ia"               # subcarpeta del vault donde se guardan los debates
BIN_DIR="$AI_HOME/bin"                       # lanzadores aider_omen / debate_omen

# ─── [V57-C1] Contexto TabbyAPI según VRAM disponible ───────────────────────
# Con Ollama GPU conviviendo (modo --exl2 clásico): 4096 (evita OOM en warmup).
# Con VRAM completa (--no-ollama o --swap): 8192 — KV cache FP16 ≈ 1GB en Llama-8B.
# Para 16384+ hace falta cache cuantizada (cache_mode Q4) en TabbyAPI reciente.
TABBY_MAX_SEQ_LEN=4096
if [[ "$_FLAG_NO_OLLAMA" == "true" || "$_FLAG_SWAP" == "true" ]]; then
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    # [V57-W1] llama-swap + contenedores bajo demanda
    if [[ -f "$LLAMASWAP_PID_FILE" ]] && kill -0 "$(cat "$LLAMASWAP_PID_FILE" 2>/dev/null)" 2>/dev/null; then
        kill "$(cat "$LLAMASWAP_PID_FILE")" 2>/dev/null && ok "llama-swap detenido" || true
    fi
    rm -f "$LLAMASWAP_PID_FILE"
    # 3. Contenedores Docker (incluye los gestionados por llama-swap: swap-*)
    CONTAINERS=("ollama-gpu-main" "ollama-cpu-router" "exllamav2-api" "sglang-server" "swap-tabby-llama" "swap-tabby-qwen" "swap-sglang-awq" "chromadb" "obsidian-kb" "searxng" "openclaw-server" "crewai-debate")  # [V58-A5] incluye el contenedor de debate
    ...
    ok "Cluster ${SCRIPT_VERSION} detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster ${SCRIPT_VERSION}"
    echo ""
    ...
    declare -A SVC_URLS=(
        ...
        ["LiteLLM Gateway:${PORT_LITELLM}"]="http://localhost:${PORT_LITELLM}/health/liveliness"  # [V56-S1] LiteLLM sustituye al Router V14
        ["llama-swap     :${PORT_LLAMASWAP}"]="http://localhost:${PORT_LLAMASWAP}/v1/models"       # [V57-W1]
    )
    ...
    SVC_ORDER=(
        ...
        "LiteLLM Gateway:${PORT_LITELLM}"
        "llama-swap     :${PORT_LLAMASWAP}"
    )
    printf "  %-30s %-10s %s
...
" "$svc" "" "$http_code"
    ...
    # [V56-S1] LiteLLM (contenedor) sustituye al Router V14 (proceso PID)
    if docker ps --format '{{.Names}}' | grep -qx "litellm-router"; then
        ok "LiteLLM Gateway contenedor activo (litellm-router)"
    else
        warn "LiteLLM Gateway (litellm-router) no está en ejecución"
    fi
    echo ""
    # [V58-A5] Capa agéntica (componentes bajo demanda — no son servicios residentes)
    if command -v aider &>/dev/null; then
        ok "Aider instalado ($(command -v aider)) — lanzador: $BIN_DIR/aider_omen"
    elif [[ -x "$HOME/.local/bin/aider" ]]; then
        ok "Aider instalado ($HOME/.local/bin/aider) — lanzador: $BIN_DIR/aider_omen"
    else
        warn "Aider no instalado — arranca el cluster (sin --no-agents) para instalarlo"
    fi
    if docker image inspect "$CREWAI_IMAGE" &>/dev/null; then
        ok "Imagen CrewAI presente ($CREWAI_IMAGE) — debate: $BIN_DIR/debate_omen \"tema\""
    else
        warn "Imagen CrewAI ($CREWAI_IMAGE) no construida — arranca el cluster para construirla"
    fi
    echo ""
    info "Modelos:     curl -s http://localhost:${PORT_LITELLM}/v1/models -H \"Authorization: Bearer ${LITELLM_MASTER_KEY}\" | python3 -m json.tool"
    info "Health:      curl -s http://localhost:${PORT_LITELLM}/health/liveliness"
    info "Log:         docker logs -f litellm-router"
    exit 0
fi
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot ${SCRIPT_VERSION}"
...
# [V56-R1] Router V14 es LEGACY: LiteLLM (:4000) es el router activo desde V55.
# V55 verificaba estos ficheros y ABORTABA si faltaban, pero nunca arrancaba el
# router — dependencia fantasma. Ahora la verificación es informativa, no bloqueante.
_ROUTER_PRESENT=false
if [[ -f "$ROUTER_SCRIPT" ]]; then
    if python3 -m py_compile "$ROUTER_SCRIPT" 2>/dev/null; then
        _ROUTER_PRESENT=true
        info "Router V14 (legacy) presente — NO se arranca: LiteLLM (:${PORT_LITELLM}) es el router activo"
    else
        warn "Router V14 (legacy) presente pero con errores de sintaxis — ignorado"
    fi
else
    info "Router V14 no encontrado — correcto: LiteLLM (:${PORT_LITELLM}) lo sustituye desde V55"
fi
...
PORTS_OK=true
# [V56-L1] PORT_ROUTER (legacy, :8000) fuera del check; PORT_LITELLM (:4000) dentro
for port_var in PORT_OLLAMA_GPU PORT_OLLAMA_CPU PORT_TABBYAPI PORT_SGLANG PORT_CHROMADB PORT_OBSIDIAN PORT_SEARXNG PORT_LITELLM PORT_OPENCLAW PORT_OPENCLAW_ADMIN; do
    port_val="${!port_var}"
...
# [V56-N1][V57-E1] Ollama GPU se omite con --no-ollama O con --swap (los motores
# nativos EXL2/AWQ cubren la inferencia GPU; sus modelos ya no están instalados).
if [[ "$_FLAG_NO_OLLAMA" == "true" || "$_FLAG_SWAP" == "true" ]]; then
    if is_container_running "ollama-gpu-main"; then
        info "Deteniendo contenedor ollama-gpu-main…"
        docker stop ollama-gpu-main >/dev/null 2>&1 || true
    fi
    if [[ "$_FLAG_SWAP" == "true" ]]; then
        warn "Ollama GPU omitido (--swap: VRAM reservada a los motores nativos bajo demanda)."
    else
        warn "Ollama GPU omitido (--no-ollama)."
    fi
else
...
fi  # [V57-E1] fin guard Ollama GPU (sección 1/8)
...
# [V57-E1] Ollama CPU solo se omite con --no-ollama. En modo --swap se MANTIENE:
# aporta embeddings (nomic-embed-text) para el vault Obsidian/ChromaDB y
# phi4-mini como orquestador ligero — todo en CPU/RAM, sin tocar VRAM.
if [[ "$_FLAG_NO_OLLAMA" == "true" ]]; then
    if is_container_running "ollama-cpu-router"; then
        info "[--no-ollama] Deteniendo contenedor ollama-cpu-router…"
        docker stop ollama-cpu-router >/dev/null 2>&1 || true
    fi
    warn "Ollama CPU omitido (--no-ollama)."
    warn "  → Sin Ollama CPU no hay embeddings (nomic-embed-text): el indexado del vault se omitirá."
else
...
fi  # [V57-E1] fin guard Ollama CPU (sección 2/8)
...
# [V57-W1] En modo --swap, llama-swap arranca/detiene TabbyAPI y SGLang bajo demanda.
if [[ "$_FLAG_SWAP" == "true" ]]; then
    for _sw_res in exllamav2-api sglang-server; do
        if is_container_running "$_sw_res"; then
            info "[--swap] Deteniendo $_sw_res residual (llama-swap lo gestionará bajo demanda)…"
            docker stop "$_sw_res" >/dev/null 2>&1 || true
            docker rm   "$_sw_res" >/dev/null 2>&1 || true
        elif docker ps -a --format '{{.Names}}' | grep -qx "$_sw_res"; then
            docker rm "$_sw_res" >/dev/null 2>&1 || true
        fi
    done
    info "TabbAPI sin arranque directo (modo --swap: gestión bajo demanda vía llama-swap)"
elif [[ "$_FLAG_SGLANG" == "true" ]]; then
...
# [V57-W1] En modo --swap lo gestiona llama-swap (residuales ya limpiados en 3/8).
if [[ "$_FLAG_SWAP" == "true" ]]; then
    info "SGLang sin arranque directo (modo --swap: gestión bajo demanda vía llama-swap)"
elif [[ "$_FLAG_SGLANG" != "true" ]]; then
...
# ═══════════════════════════════════════════════════════════════════════════════
# 4b. LLAMA-SWAP (:9090) — [V57-W1] Orquestador de motores bajo demanda
#     Criterio del usuario: penalizar TIEMPO (swap 1-3 min), nunca RAZONAMIENTO.
#     Cada modelo dispone de los 8GB de VRAM completos y contexto ampliado.
# ═══════════════════════════════════════════════════════════════════════════════
if [[ "$_FLAG_SWAP" == "true" ]]; then
section "4b/8 — llama-swap (:$PORT_LLAMASWAP)"

# ─── Binario (descarga automática si falta) ──────────────────────────────
if [[ ! -x "$LLAMASWAP_BIN" ]]; then
    info "llama-swap no encontrado — descargando última release de GitHub…"
    mkdir -p "$AI_HOME/bin"
    _LS_URL=$(curl --noproxy "*" -s -m 30 https://api.github.com/repos/mostlygeek/llama-swap/releases/latest \
        | grep -o 'https://[^"]*linux_amd64\.tar\.gz' | head -1 || true)
    if [[ -n "${_LS_URL:-}" ]]; then
        curl --noproxy "*" -L -s -m 120 -o /tmp/llama-swap.tar.gz "$_LS_URL" \
            && tar -xzf /tmp/llama-swap.tar.gz -C "$AI_HOME/bin" llama-swap 2>/dev/null \
            && chmod +x "$LLAMASWAP_BIN" || true
    fi
fi
if [[ ! -x "$LLAMASWAP_BIN" ]]; then
    err "No se pudo instalar llama-swap automáticamente."
    err "  Descárgalo de https://github.com/mostlygeek/llama-swap/releases (linux_amd64)"
    err "  y colócalo como: $LLAMASWAP_BIN"
    exit 1
fi
ok "llama-swap binario ✔ ($("$LLAMASWAP_BIN" --version 2>/dev/null | head -1 || echo 'versión desconocida'))"

# ─── Config ─────────────────────────────────────────────────────────────
if [[ ! -f "$LLAMASWAP_CONFIG" ]]; then
    err "Falta la configuración: $LLAMASWAP_CONFIG"
    err "  Copia llama-swap_config.yaml a $AI_HOME/"
    exit 1
fi

# ─── Detener instancia previa y arrancar ──────────────────────────────────
if [[ -f "$LLAMASWAP_PID_FILE" ]] && kill -0 "$(cat "$LLAMASWAP_PID_FILE" 2>/dev/null)" 2>/dev/null; then
    info "Deteniendo llama-swap previo (PID $(cat "$LLAMASWAP_PID_FILE"))…"
    kill "$(cat "$LLAMASWAP_PID_FILE")" 2>/dev/null || true
    sleep 2
fi
nohup "$LLAMASWAP_BIN" --config "$LLAMASWAP_CONFIG" --listen "0.0.0.0:${PORT_LLAMASWAP}" \
    >> "$LOG_DIR/llama_swap.log" 2>&1 &
echo $! > "$LLAMASWAP_PID_FILE"
info "llama-swap arrancado (PID $(cat "$LLAMASWAP_PID_FILE")) — log: $LOG_DIR/llama_swap.log"

# ─── Health check del proxy (no carga modelos todavía) ────────────────────────
_LS_READY=false
_LS_HTTP="000"
for _r in $(seq 1 "$TIMEOUT_LLAMASWAP"); do
    _LS_HTTP=$(curl --noproxy "*" --ipv4 -s -m 2 -o /dev/null -w "%{http_code}" \
        "http://localhost:${PORT_LLAMASWAP}/v1/models" 2>/dev/null || echo "000")
    if [[ "$_LS_HTTP" == "200" ]]; then
        _LS_READY=true
        break
    fi
    sleep 1
done
if [[ "$_LS_READY" == "true" ]]; then
    ok "llama-swap ✔ — proxy listo (:${PORT_LLAMASWAP}); motores bajo demanda"
    info "  Primera petición a cada modelo: 1-3 min (docker + carga de pesos en VRAM completa)"
else
    err "llama-swap no respondió tras ${TIMEOUT_LLAMASWAP}s (HTTP ${_LS_HTTP})"
    err "  Revisa: tail -50 $LOG_DIR/llama_swap.log"
fi
fi  # [V57-W1] fin sección llama-swap
...
info "[V56] Lanzando LiteLLM como pasarela universal de modelos…"
docker run -d \
    --name litellm-router \
    --network "$DOCKER_NET" \
    -p "${PORT_LITELLM}:4000" \
...
# [V56-L2] Health check HTTP real: wait_port TCP no garantiza que la API esté lista
# (la imagen de LiteLLM tarda 20-60s en importar dependencias y validar el YAML).
LITELLM_READY=false
_LL_HTTP="000"
info "Esperando LiteLLM /health/liveliness en localhost:${PORT_LITELLM}… (máx ${TIMEOUT_LITELLM}s)"
for _retry in $(seq 1 "$TIMEOUT_LITELLM"); do
    if ! docker ps --format '{{.Names}}' | grep -qx "litellm-router"; then
        err "Contenedor 'litellm-router' ya no está en ejecución. Revisa: docker logs litellm-router"
        break
    fi
    _LL_HTTP=$(curl --noproxy "*" --ipv4 -s -m 2 -o /dev/null -w "%{http_code}" \
        "http://localhost:${PORT_LITELLM}/health/liveliness" 2>/dev/null || echo "000")
    if [[ "$_LL_HTTP" == "200" ]]; then
        LITELLM_READY=true
        break
    fi
    sleep 1
...
if [[ "$LITELLM_READY" == "true" ]]; then
    ok "LiteLLM ✔ — API lista → http://localhost:${PORT_LITELLM} (key: \$LITELLM_MASTER_KEY)"
else
    warn "LiteLLM no respondió tras ${TIMEOUT_LITELLM}s (último HTTP: ${_LL_HTTP}). Revisa: docker logs litellm-router"
fi
...
# [V56-P1] Primary dinámico según modo VRAM (el host escribe /tmp/oc_primary.txt).
# BUG V55: se fijaba "profundo-r1", un id que no existía en ningún model_list.
try:
    with open("/tmp/oc_primary.txt") as f:
        _primary = f.read().strip()
except Exception:
    _primary = PROVIDER_ID + "/instantaneo"
if not _primary:
    _primary = PROVIDER_ID + "/instantaneo"
cfg["agents"]["defaults"]["model"]["primary"] = _primary

# [V56-P1] Fallbacks: si el primary no responde, degradar a un modelo real distinto
_model_ids = {m.get("id") for m in models if isinstance(m, dict)}
_fb_candidates = [PROVIDER_ID + "/" + mid for mid in ("instantaneo", "agil", "codigo") if mid in _model_ids]
cfg["agents"]["defaults"]["model"]["fallbacks"] = [x for x in _fb_candidates if x != _primary][:2]
...
# [V56-M1] Lista DINÁMICA desde LiteLLM /v1/models: registra en OpenClaw exactamente
# lo que LiteLLM sirve (físicos + alias), en vez de una lista estática desactualizada.
_MODELS_JSON=""
_LL_MODELS_RAW=$(curl --noproxy "*" --ipv4 -s -m 5 \
    -H "Authorization: Bearer ${LITELLM_MASTER_KEY}" \
    "http://localhost:${PORT_LITELLM}/v1/models" 2>/dev/null || echo "")
if [[ -n "$_LL_MODELS_RAW" ]]; then
    _MODELS_JSON=$(echo "$_LL_MODELS_RAW" | python3 -c "
import json, sys
try:
    data = json.load(sys.stdin).get('data', [])
    out = [{'id': m['id'], 'name': m['id'], 'contextWindow': 32768} for m in data if m.get('id')]
...
if [[ -z "$_MODELS_JSON" || "$_MODELS_JSON" == "[]" ]]; then
    warn "[V56-M1] /v1/models de LiteLLM no disponible — usando lista estática de respaldo (modelos nativos instalados)"
    _MODELS_JSON='[
  {"id":"instantaneo", "name":"INSTANTANEO — Llama 3.1 8B (TabbAPI EXL2)", "contextWindow":2048},
  {"id":"codigo", "name":"CODIGO — Qwen 2.5 Coder 7B (TabbAPI EXL2)", "contextWindow":2048},
  {"id":"agil", "name":"AGIL — Llama 3.1 8B AWQ (SGLang)", "contextWindow":4096},
  {"id":"llama-3.1-8b-exl2", "name":"Llama 3.1 8B (TabbAPI)", "contextWindow":2048},
  {"id":"qwen2.5-coder-7b-exl2", "name":"Qwen 2.5 Coder 7B (TabbAPI)", "contextWindow":2048},
  {"id":"llama-3.1-8b-awq", "name":"Llama 3.1 8B (SGLang)", "contextWindow":4096}
]'
fi
_MODEL_COUNT=$(echo "$_MODELS_JSON" | python3 -c "import json,sys; print(len(json.load(sys.stdin)))" 2>/dev/null || echo "?")

info "[V55] Modelos a registrar: ${_MODEL_COUNT}"

# ─── Copiar modelos y script al contenedor y ejecutar ─────────────────────────
info "[V56] Inyectando provider LiteLLM en openclaw.json…"
echo "$_MODELS_JSON" > /tmp/oc_models_v55.json

# [V56-P1] Primary según modo VRAM activo — siempre un id que existe en la lista
if [[ "$_FLAG_SGLANG" == "true" ]]; then
    _OC_PRIMARY="litellm-omen/agil"            # SGLang AWQ activo
elif [[ "$_FLAG_EXL2" == "true" ]]; then
    _OC_PRIMARY="litellm-omen/instantaneo"     # TabbAPI EXL2 activo
else
    _OC_PRIMARY="litellm-omen/profundo"        # default: Ollama GPU con toda la VRAM
fi
echo "$_OC_PRIMARY" > /tmp/oc_primary_v56.txt
info "[V56] Modelo primario de OpenClaw: ${_OC_PRIMARY}"

_INJECT_OK=false
for _inj_retry in 1 2 3; do
    docker cp /tmp/oc_models_v55.json openclaw-server:/tmp/oc_models.json  2>/dev/null || true
    docker cp /tmp/oc_primary_v56.txt openclaw-server:/tmp/oc_primary.txt  2>/dev/null || true
    ...
    # [V56-B2] Bajo set -e, capturar el exit code sin abortar el script
    _INJ_EXIT=0
    docker exec openclaw-server python3 /tmp/inject_v55.py > "$_OC_INJ_LOG" 2>&1 || _INJ_EXIT=$?
...
_VERIFY_EXIT=0  # [V56-B2] pre-init: evita abortar bajo set -e
...
try:
    ...
    sys.exit(1)
" > "$_OC_VERIFY_LOG" 2>&1 || _VERIFY_EXIT=$?  # [V56-B2] captura el exit code sin abortar (set -e)
...
# ═══════════════════════════════════════════════════════════════════════════════
# 9b/9 — CAPA AGÉNTICA: Aider + CrewAI (Flow Engineering & Agentic Loops) [V58]
#
# Dos servicios NUEVOS que consumen los modelos existentes vía LiteLLM (:4000):
#
#  · Aider  — bucles agénticos de programación. Instalado en el HOST con
#             aider-install (uv + Python 3.12, entorno aislado — sin Docker).
#             Config en $AIDER_DIR ajustada a modelos 7-8B (edit-format whole,
#             map-tokens 0, weak model phi4-mini CPU, timeout 1800s).
#             Lanzador: $BIN_DIR/aider_omen (ejecutar dentro de un repo git).
#
#  · CrewAI — debate lector↔crítico sobre el vault Obsidian. Contenedor
#             BAJO DEMANDA (no residente): imagen local $CREWAI_IMAGE,
#             proceso secuencial, CERO tools (el knowledge source inyecta el
#             contexto — sin tool-calling JSON, punto débil de los 7-8B).
#             Lector = instantaneo (GPU) · Crítico = orquestador/phi4-mini
#             (CPU) → el motor GPU permanece cargado entre rondas (sin swaps).
#             La nota final se guarda en $VAULT_DIR/$DEBATE_OUT_SUBDIR/ y el
#             indexador la incorpora a ChromaDB (encadenado de conocimiento).
#             Lanzador: $BIN_DIR/debate_omen "tema".
#
# Idempotente: los ficheros de configuración solo se generan si NO existen
# (las ediciones del usuario se conservan; borrar un fichero lo regenera).
# La imagen CrewAI solo se construye si falta (o con --agents).
# ═══════════════════════════════════════════════════════════════════════════════
if [[ "$_FLAG_NO_AGENTS" == "true" ]]; then
    section "9b/9 — Capa agéntica (omitida: --no-agents)"
    info "Los lanzadores ya generados ($BIN_DIR/aider_omen, $BIN_DIR/debate_omen) siguen operativos."
else
    section "9b/9 — Capa agéntica: Aider + CrewAI (Flow Engineering)"

    mkdir -p "$AIDER_DIR" "$CREWAI_DIR/config" "$CREWAI_DIR/src" "$CREWAI_DIR/storage" "$BIN_DIR"
    mkdir -p "$VAULT_DIR/$DEBATE_OUT_SUBDIR" 2>/dev/null || warn "No se pudo crear $VAULT_DIR/$DEBATE_OUT_SUBDIR (¿vault accesible?)"

    # ─── [V58-A2] Aider — configuración (solo si falta; borrar para regenerar) ───
    if [[ ! -f "$AIDER_DIR/aider.conf.yml" ]]; then
        cat > "$AIDER_DIR/aider.conf.yml" << AIDER_CONF_EOF
# ============================================================================
# Configuración de Aider — Clúster OMEN V58 (generada por Autoboot)
# Ajustada a modelos locales 7-8B servidos vía LiteLLM (:${PORT_LITELLM}).
# Bórrala para regenerarla en el próximo arranque del clúster.
# ============================================================================

# Endpoint del proxy LiteLLM del clúster
openai-api-base: http://localhost:${PORT_LITELLM}
openai-api-key: ${LITELLM_MASTER_KEY}

# Modelo principal: alias 'codigo' (Qwen2.5-Coder 7B EXL2 vía llama-swap)
model: openai/codigo

# Modelo débil (mensajes de commit, resúmenes): phi4-mini en CPU —
# no provoca swaps del motor GPU
weak-model: openai/orquestador

# CRÍTICO con 7-8B: formato de archivo completo (los diff salen malformados)
edit-format: whole

# El repo map confunde a los modelos pequeños (editan el mapa, no el código)
map-tokens: 0

# Verificar la salida del modelo local antes de commitear
auto-commits: false

# Los alias del proxy no están en la BD de precios de LiteLLM (warning inocuo)
show-model-warnings: false

# Primer prompt tras un swap = arranque en frío del motor (1-3 min)
timeout: 1800

check-update: false
model-settings-file: ${AIDER_DIR}/aider.model.settings.yml
model-metadata-file: ${AIDER_DIR}/aider.model.metadata.json
AIDER_CONF_EOF
        ok "Generado: $AIDER_DIR/aider.conf.yml"
    else
        ok "aider.conf.yml existente — se conservan tus ajustes"
    fi

    if [[ ! -f "$AIDER_DIR/aider.model.settings.yml" ]]; then
        cat > "$AIDER_DIR/aider.model.settings.yml" << 'AIDER_SET_EOF'
# Comportamiento por modelo — Clúster OMEN V58 (alias del proxy LiteLLM).
# edit_format whole + use_repo_map false + examples_as_sys_msg true:
# combinación más fiable con modelos 7-8B cuantizados.
- name: openai/codigo
  edit_format: whole
  weak_model_name: openai/orquestador
  use_repo_map: false
...
- name: openai/orquestador
  ...
  reminder: user
AIDER_SET_EOF
        ok "Generado: $AIDER_DIR/aider.model.settings.yml"
    fi

    if [[ ! -f "$AIDER_DIR/aider.model.metadata.json" ]]; then
        cat > "$AIDER_DIR/aider.model.metadata.json" << 'AIDER_META_EOF'
{
  "openai/codigo": {
    "max_tokens": 4096,
...
AIDER_META_EOF
        ok "Generado: $AIDER_DIR/aider.model.metadata.json"
    fi

    # NOTA: max_input_tokens=8192 refleja el contexto REAL servido por llama-swap
    # (TabbyAPI --max-seq-len 8192 / SGLang --max-total-tokens 8192), no el
    # contexto nativo del modelo (32k/128k). Si amplías el contexto en
    # llama-swap_config.yaml, actualiza también aider.model.metadata.json.

    # ─── Lanzador aider_omen (se regenera en cada arranque) ───
    cat > "$BIN_DIR/aider_omen" << AIDER_BIN_EOF
#!/bin/bash
# ============================================================================
# aider_omen — Aider contra el clúster OMEN (LiteLLM :${PORT_LITELLM})
# Generado por Autoboot_Cluster ${SCRIPT_VERSION}. Ejecutar DENTRO de un repo git.
#   aider_omen fichero1.py fichero2.py       # modelo por defecto: openai/codigo
#   aider_omen --model openai/instantaneo    # razonamiento general (Llama AWQ)
# ============================================================================
export OPENAI_API_BASE="http://localhost:${PORT_LITELLM}"
export OPENAI_API_KEY="${LITELLM_MASTER_KEY}"

AIDER_BIN="\$(command -v aider || true)"
[[ -z "\$AIDER_BIN" && -x "\$HOME/.local/bin/aider" ]] && AIDER_BIN="\$HOME/.local/bin/aider"
if [[ -z "\$AIDER_BIN" ]]; then
    echo "[ERROR] Aider no está instalado. Ejecuta: pip install aider-install && aider-install"
    exit 1
fi

exec "\$AIDER_BIN" --config "${AIDER_DIR}/aider.conf.yml" "\$@"
AIDER_BIN_EOF
    chmod +x "$BIN_DIR/aider_omen"
    ok "Lanzador listo: $BIN_DIR/aider_omen"

    # ─── Instalación de Aider en el host (aislada vía uv — no toca el sistema) ───
    _AIDER_FOUND=""
    if command -v aider &>/dev/null; then
        _AIDER_FOUND="$(command -v aider)"
    elif [[ -x "$HOME/.local/bin/aider" ]]; then
        _AIDER_FOUND="$HOME/.local/bin/aider"
    fi
    if [[ -z "$_AIDER_FOUND" || "$_FLAG_AGENTS" == "true" ]]; then
        info "Instalando Aider (aider-install → uv + Python 3.12, entorno aislado)…"
        _PIP_A_ARGS=""
        if python3 -c "import sys; sys.exit(0 if sys.version_info >= (3,11) else 1)" 2>/dev/null; then
            _STDLIB=$(python3 -c 'import sysconfig; print(sysconfig.get_path("stdlib"))' 2>/dev/null || echo "")
            if [[ -f "/usr/lib/python3/EXTERNALLY-MANAGED" ]] || { [[ -n "$_STDLIB" ]] && [[ -f "${_STDLIB}/EXTERNALLY-MANAGED" ]]; }; then
                _PIP_A_ARGS="--break-system-packages"
            fi
        fi
        _AIDER_OK=false
        if pip3 install --quiet $_PIP_A_ARGS aider-install >> "$LOG_DIR/aider_install.log" 2>&1; then
            _AIDER_INSTALLER="$(command -v aider-install || echo "$HOME/.local/bin/aider-install")"
            if [[ -x "$_AIDER_INSTALLER" ]] && "$_AIDER_INSTALLER" >> "$LOG_DIR/aider_install.log" 2>&1; then
                _AIDER_OK=true
            fi
        fi
        if [[ "$_AIDER_OK" == "true" ]]; then
            ok "Aider instalado — usa: $BIN_DIR/aider_omen (dentro de un repo git)"
        else
            warn "No se pudo instalar Aider automáticamente (log: $LOG_DIR/aider_install.log)"
            warn "Instalación manual: pip install aider-install && aider-install"
        fi
    else
        ok "Aider ya instalado ($_AIDER_FOUND)"
    fi

    # ─── [V58-A3] CrewAI — proyecto del debate (ficheros solo si faltan) ───
    if [[ ! -f "$CREWAI_DIR/requirements.txt" ]]; then
        cat > "$CREWAI_DIR/requirements.txt" << 'CREW_REQ_EOF'
# Dependencias del debate multiagente — Clúster OMEN V58.
# SIN litellm (paquete cuarentenado en PyPI, 2026): CrewAI habla con el
# proxy LiteLLM mediante el driver openai nativo (prefijo openai/ + base_url).
crewai[openai]==1.14.6
crewai-tools==0.17.0
python-dotenv==1.0.1
pyyaml>=6.0
CREW_REQ_EOF
        ok "Generado: $CREWAI_DIR/requirements.txt"
    fi

    if [[ ! -f "$CREWAI_DIR/Dockerfile" ]]; then
        cat > "$CREWAI_DIR/Dockerfile" << 'CREW_DOCKER_EOF'
# Imagen del debate multiagente — Clúster OMEN V58 (generada por Autoboot)
...
# src y config se montan :ro en tiempo de ejecución (editables sin rebuild);
# src se copia también en build para que la imagen funcione sin montajes.
COPY src/ ./src/

RUN mkdir -p /app/storage /app/knowledge

ENTRYPOINT ["python", "src/main.py"]
CREW_DOCKER_EOF
        ok "Generado: $CREWAI_DIR/Dockerfile"
    fi

    if [[ ! -f "$CREWAI_DIR/config/agents.yaml" ]]; then
        cat > "$CREWAI_DIR/config/agents.yaml" << 'CREW_AG_EOF'
# ============================================================================
# Roles del debate — Clúster OMEN V58 (editable SIN rebuild: montado :ro)
# 'llm' es un ALIAS de LiteLLM (:4000): instantaneo / codigo / agil / orquestador
# Diseño: Lector en GPU (instantaneo) · Crítico en CPU (orquestador/phi4-mini)
# para que el motor GPU permanezca cargado entre rondas (sin swaps).
# ============================================================================

lector_obsidian:
  role: Lector y Sintetizador del Vault Obsidian
  goal: >
    Analizar en profundidad el conocimiento del vault sobre {tema} y producir
    un análisis estructurado y fiel a las notas. Si existe una crítica previa,
    responderla punto por punto y refinar el análisis. No inventar información.
  backstory: >
    Eres un analista experto en síntesis de bases de conocimiento personales.
    Solo afirmas lo que está respaldado por las notas del vault y citas la
    nota de origen de cada punto. Reconoces las lagunas con honestidad.
  llm: instantaneo
  temperature: 0.2
  max_tokens: 1200
  max_iter: 3
...
critico_debatidor:
  role: Crítico y Debatidor Constructivo
  goal: >
    Someter a debate el análisis sobre {tema}: detectar afirmaciones sin
    evidencia, lagunas, contradicciones y perspectivas ausentes, y proponer
    mejoras concretas y priorizadas para la siguiente ronda.
  backstory: >
    Eres un crítico riguroso y constructivo. Tu misión es elevar la calidad
    del razonamiento: desafías todo punto débil, exiges evidencia y propones
    ángulos alternativos. Nunca repites la crítica anterior sin aportar algo
    nuevo.
  llm: orquestador
  temperature: 0.4
  max_tokens: 1000
  max_iter: 3
CREW_AG_EOF
        ok "Generado: $CREWAI_DIR/config/agents.yaml"
    fi

    if [[ ! -f "$CREWAI_DIR/config/tasks.yaml" ]]; then
        cat > "$CREWAI_DIR/config/tasks.yaml" << 'CREW_TASK_EOF'
# ============================================================================
# Tareas del debate — Clúster OMEN V58 (editable SIN rebuild: montado :ro)
# {tema} y {critica_previa} los inyecta src/main.py en cada ronda.
# El contexto del vault lo aporta el knowledge source (SIN tool-calling).
# ============================================================================
...
tarea_lectura:
  ...
  description: >
    ...
    Crítica de la ronda anterior (en la primera ronda no hay):
    {critica_previa}

    Analiza el tema usando exclusivamente el conocimiento disponible del
    vault Obsidian. Si hay crítica previa, respóndela punto por punto y
    refina el análisis. Estructura (máximo 500 palabras): 1) Puntos Clave,
    2) Fuentes (notas del vault), 3) Lagunas identificadas.
  expected_output: >
    Análisis en markdown con las secciones: Puntos Clave, Fuentes, Lagunas.
...
tarea_critica:
  ...
  description: >
    ...
    Revisa el análisis producido en la tarea anterior. Identifica:
    (1) afirmaciones sin evidencia suficiente, (2) perspectivas o información
    faltante, (3) contradicciones internas. Después propón mejoras concretas
    y priorizadas para la siguiente ronda de refinamiento.
  expected_output: >
    Crítica constructiva en markdown con las secciones: Debilidades,
    Perspectivas Faltantes, Mejoras Propuestas.
  context:
    - tarea_lectura
CREW_TASK_EOF
        ok "Generado: $CREWAI_DIR/config/tasks.yaml"
    fi

    if [[ ! -f "$CREWAI_DIR/src/main.py" ]]; then
        cat > "$CREWAI_DIR/src/main.py" << 'CREW_MAIN_EOF'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Debate lector<->critico sobre el vault Obsidian — Clúster OMEN V58.

Bucle agéntico (Flow Engineering):
  1. El Lector analiza el tema con el knowledge source (vault Obsidian,
     embeddings nomic-embed-text vía Ollama CPU). El contexto se inyecta
     directamente en el prompt — SIN tool-calling (fiable con 7-8B).
  2. El Crítico (phi4-mini, CPU) señala debilidades y lagunas.
  3. La crítica se reinyecta al Lector en la siguiente ronda (refinamiento).
  4. Tras MAX_RONDAS, la nota final se guarda en el vault (debates_ia/)
     para que el indexador la incorpore a ChromaDB.

Roles y tareas: /app/config/agents.yaml + tasks.yaml (editables sin rebuild).
"""
import datetime
import os
import re
...
EMBED_HOST = os.getenv("OLLAMA_EMBED_HOST", "http://host.docker.internal:11435")
KNOWLEDGE_DIR = Path(os.getenv("KNOWLEDGE_DIR", "/app/knowledge"))
OUT_DIR = Path(os.getenv("DEBATE_OUT", "/vault_out"))
CONFIG_DIR = Path(os.getenv("CONFIG_DIR", "/app/config"))
MAX_RONDAS = int(os.getenv("MAX_RONDAS", "3"))
MAX_NOTAS = int(os.getenv("MAX_NOTAS", "300"))

# CrewAI exige OPENAI_API_KEY aunque se use base_url propio
os.environ.setdefault("OPENAI_API_KEY", API_KEY)

EMBEDDER = {
    "provider": "ollama",
    "config": {"model": "nomic-embed-text", "host": EMBED_HOST},
}


def cargar_yaml(nombre):
    with open(CONFIG_DIR / nombre, encoding="utf-8") as fh:
        return yaml.safe_load(fh)


def fuentes_conocimiento():
    """Notas .md del vault, rutas relativas a /app/knowledge (convención CrewAI)."""
    notas = sorted(p for p in KNOWLEDGE_DIR.rglob("*.md") if p.is_file())
    if not notas:
        print(f"[WARN] Sin notas .md en {KNOWLEDGE_DIR} — debate sin contexto del vault")
        return []
    if len(notas) > MAX_NOTAS:
        print(f"[INFO] Vault con {len(notas)} notas; se indexan las {MAX_NOTAS} más recientes (MAX_NOTAS)")
        notas = sorted(notas, key=lambda p: p.stat().st_mtime, reverse=True)[:MAX_NOTAS]
    relativas = [str(p.relative_to(KNOWLEDGE_DIR)) for p in notas]
    return [TextFileKnowledgeSource(file_paths=relativas, metadata={"source": "obsidian_vault"})]


def construir_crew(knowledge):
    ag_cfg = cargar_yaml("agents.yaml")
    t_cfg = cargar_yaml("tasks.yaml")

    agentes = {}
    for nombre, c in ag_cfg.items():
        llm = LLM(
            model=f"openai/{c.get('llm', 'instantaneo')}",  # alias LiteLLM
            base_url=BASE_URL,
            api_key=API_KEY,
            temperature=float(c.get("temperature", 0.2)),
            max_tokens=int(c.get("max_tokens", 1200)),
            timeout=1800,  # criterio del clúster: penalizar tiempo, no razonamiento
        )
        agentes[nombre] = Agent(
            role=c["role"],
            goal=c["goal"],
            backstory=c["backstory"],
            llm=llm,
            tools=[],                # CERO tools: sin JSON tool-calls (fiable en 7-8B)
            allow_delegation=False,
            max_iter=int(c.get("max_iter", 3)),
            verbose=True,
        )

    tareas, lista = {}, []
    for nombre, c in t_cfg.items():
        t = Task(
            description=c["description"],
            expected_output=c["expected_output"],
            agent=agentes[c["agent"]],
            context=[tareas[x] for x in c.get("context", [])],
        )
        tareas[nombre] = t
        lista.append(t)
    ...
    return Crew(
        ...
        tasks=lista,
        process=Process.sequential,   # determinista — hierarchical falla con 7-8B
...
def guardar_nota(tema, analisis, critica, rondas):
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    slug = re.sub(r"[^a-z0-9]+", "-", tema.lower()).strip("-")[:60] or "debate"
    destino = OUT_DIR / f"{stamp}_{slug}.md"
    ...
    destino.write_text(
        ...
        f"fecha: {fecha}\n"
        f"rondas: {rondas}\n"
        "origen: debate_crewai_v58\n"
        "tags: [debate-ia, consenso]\n"
        "---\n\n"
        f"# Debate: {tema}\n\n"
        "## Análisis final consensuado\n\n"
        f"{analisis}\n\n"
        "## Última crítica (puntos abiertos)\n\n"
        f"{critica}\n",
        encoding="utf-8",
    )
    return destino
...
def main():
    tema = " ".join(sys.argv[1:]).strip() or os.getenv("TEMA", "").strip()
    if not tema:
        print('Uso: debate_omen "tema a debatir"')
        sys.exit(1)

    print(f"[INFO] Debate sobre: {tema} — {MAX_RONDAS} rondas — proxy: {BASE_URL}")
    knowledge = fuentes_conocimiento()
    crew = construir_crew(knowledge)

    analisis, critica = "", "(primera ronda: aún no hay crítica previa)"
    for ronda in range(1, MAX_RONDAS + 1):
        print(f"\n======== RONDA {ronda}/{MAX_RONDAS} ========\n")
        resultado = crew.kickoff(inputs={"tema": tema, "critica_previa": critica})
        salidas = resultado.tasks_output
        analisis = salidas[0].raw
        critica = salidas[-1].raw

    destino = guardar_nota(tema, analisis, critica, MAX_RONDAS)
    print("\n======== RESULTADO FINAL ========\n")
    print(analisis)
    print(f"\n[OK] Nota guardada en el vault: {destino}")
    print("[OK] Ejecuta 'ai_cluster --reindex' (o espera al indexador) para incorporarla a ChromaDB")


if __name__ == "__main__":
    main()
CREW_MAIN_EOF
        ok "Generado: $CREWAI_DIR/src/main.py"
    fi

    # ─── Build de la imagen (solo si falta o con --agents) ───
    if ! docker image inspect "$CREWAI_IMAGE" &>/dev/null || [[ "$_FLAG_AGENTS" == "true" ]]; then
        info "Construyendo imagen CrewAI ($CREWAI_IMAGE)… (primera vez: varios minutos)"
        if docker build -t "$CREWAI_IMAGE" "$CREWAI_DIR" >> "$LOG_DIR/crewai_build.log" 2>&1; then
            ok "Imagen CrewAI construida: $CREWAI_IMAGE"
        else
            warn "Falló el build de la imagen CrewAI (log: $LOG_DIR/crewai_build.log)"
            warn "Build manual: docker build -t $CREWAI_IMAGE $CREWAI_DIR"
        fi
    else
        ok "Imagen CrewAI presente ($CREWAI_IMAGE) — usa --agents para reconstruirla"
    fi

    # ─── Lanzador debate_omen (se regenera en cada arranque) ───
    cat > "$BIN_DIR/debate_omen" << DEBATE_BIN_EOF
#!/bin/bash
# ============================================================================
# debate_omen — Debate multiagente (CrewAI) sobre el vault Obsidian
# Generado por Autoboot_Cluster ${SCRIPT_VERSION}. Uso:
#   debate_omen "tema a debatir"          (3 rondas por defecto)
#   MAX_RONDAS=5 debate_omen "tema"       (más rondas de refinamiento)
# Requiere el clúster activo (LiteLLM :${PORT_LITELLM} + Ollama CPU :${PORT_OLLAMA_CPU}).
# ============================================================================
set -euo pipefail

TEMA="\$*"
if [[ -z "\$TEMA" ]]; then
    echo 'Uso: debate_omen "tema a debatir"'
    echo '     MAX_RONDAS=5 debate_omen "tema"   (por defecto: 3 rondas)'
    exit 1
fi

mkdir -p "${VAULT_DIR}/${DEBATE_OUT_SUBDIR}" 2>/dev/null || true
docker rm -f ${CREWAI_CONTAINER} 2>/dev/null || true

exec docker run --rm -it \\
    --name ${CREWAI_CONTAINER} \\
    --network ${DOCKER_NET} \\
    --add-host host.docker.internal:host-gateway \\
    --user "\$(id -u):\$(id -g)" \\
    -e HOME=/tmp \\
    -v "${VAULT_DIR}:/app/knowledge:ro" \\
    -v "${VAULT_DIR}/${DEBATE_OUT_SUBDIR}:/vault_out" \\
    -v "${CREWAI_DIR}/storage:/app/storage" \\
    -v "${CREWAI_DIR}/config:/app/config:ro" \\
    -v "${CREWAI_DIR}/src:/app/src:ro" \\
    -e LITELLM_BASE_URL="http://host.docker.internal:${PORT_LITELLM}" \\
    -e LITELLM_API_KEY="${LITELLM_MASTER_KEY}" \\
    -e OLLAMA_EMBED_HOST="http://host.docker.internal:${PORT_OLLAMA_CPU}" \\
    -e CREWAI_STORAGE_DIR="/app/storage" \\
    -e MAX_RONDAS="\${MAX_RONDAS:-3}" \\
    ${CREWAI_IMAGE} "\$TEMA"
DEBATE_BIN_EOF
    chmod +x "$BIN_DIR/debate_omen"
    ok "Lanzador listo: $BIN_DIR/debate_omen \"tema a debatir\""

    info "Roles/tareas editables sin rebuild: $CREWAI_DIR/config/{agents,tasks}.yaml"
    info "Las notas del debate se guardan en: $VAULT_DIR/$DEBATE_OUT_SUBDIR/"
fi

# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster ${SCRIPT_VERSION}"

echo ""
# [V27-S3][V31-E1] Modo VRAM activo
if [[ "$_FLAG_SWAP" == "true" ]]; then
    echo -e "  Modo VRAM:  ${BLD}--swap${NC}   (llama-swap: cada modelo usa los 8GB completos, carga bajo demanda)"
elif [[ "$_FLAG_SGLANG" == "true" ]]; then
...
_health_url() {
    ...
    case "$port" in
        ...
        "$PORT_LITELLM")   echo "http://localhost:${port}/health/liveliness" ;;  # [V56-S1]
        "$PORT_LLAMASWAP") echo "http://localhost:${port}/v1/models" ;;           # [V57-W1]
...
if [[ "$_FLAG_NO_OLLAMA" == "true" ]]; then
    # [V56-N1] Contenedores Ollama omitidos: no evaluar como fallo
    printf "%-30s %-12s %b\n" "Ollama GPU (main)"       ":$PORT_OLLAMA_GPU" "${YEL}— omitido (--no-ollama)${NC}"
    printf "%-30s %-12s %b\n" "Ollama CPU (router/emb)" ":$PORT_OLLAMA_CPU" "${YEL}— omitido (--no-ollama)${NC}"
else
...
if [[ "$_FLAG_SWAP" == "true" ]]; then
    printf "%-30s %-12s %b\n" "TabbAPI ExLlamaV2" ":${PORT_TABBYAPI}" "${YEL}— bajo demanda (--swap)${NC}"
elif [[ "$_FLAG_SGLANG" == "true" ]]; then
...
if [[ "$_FLAG_SWAP" == "true" ]]; then
    printf "%-30s %-12s %b\n" "SGLang" ":${PORT_SGLANG}" "${YEL}— bajo demanda (--swap)${NC}"
elif [[ "$_FLAG_SGLANG" != "true" ]]; then
...
if [[ "$_FLAG_SWAP" == "true" ]]; then
    check_service "llama-swap"           localhost    "$PORT_LLAMASWAP"
fi
check_service "LiteLLM Gateway"          localhost    "$PORT_LITELLM"
...
echo ""
echo -e "${BLD}Capa agéntica [V58]:${NC}"
echo "  Programar (bucle):   $BIN_DIR/aider_omen          (ejecutar dentro de un repo git)"
echo "  Debate Obsidian:     $BIN_DIR/debate_omen \"tema\"  (lector + crítico sobre el vault)"
echo "  Más rondas:          MAX_RONDAS=5 $BIN_DIR/debate_omen \"tema\""
echo "  Roles del debate:    $CREWAI_DIR/config/agents.yaml"
echo "  Notas de salida:     $VAULT_DIR/${DEBATE_OUT_SUBDIR}/  (se indexan en ChromaDB al reindexar)"
echo ""
...
echo "  ai_cluster --no-ollama  Omite contenedores Ollama (solo motores nativos)"
echo "  ai_cluster --swap     RECOMENDADO: llama-swap gestiona los motores bajo demanda"
echo "                        (VRAM completa por modelo; mantiene Ollama CPU p/ embeddings)"
echo "  ai_cluster --agents   Fuerza reinstalación/rebuild de la capa agéntica (Aider+CrewAI)"
echo "  ai_cluster --no-agents  Omite la capa agéntica en este arranque"
echo "  ai_cluster --help     Muestra la ayuda completa"
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster ${SCRIPT_VERSION} — iniciado${NC}"
