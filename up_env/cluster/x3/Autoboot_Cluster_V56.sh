...
set -euo pipefail
SCRIPT_VERSION="V56"  # [V44-B1][V56-S1] Versión del script — usada en banners y logs
...
if [[ "$_FLAG_NO_OLLAMA" == "true" ]] && [[ "$_FLAG_SGLANG" != "true" ]] && [[ "$_FLAG_EXL2" != "true" ]]; then
    _FLAG_EXL2=true
...
TIMEOUT_LITELLM=90           # [V56-L2] La imagen de LiteLLM tarda en importar y validar el YAML
...
# [V56-N1] Modo --no-ollama: los motores nativos (EXL2/AWQ) cubren la inferencia.
# Se omiten Ollama GPU y CPU por completo (secciones 1/8 y 2/8).
if [[ "$_FLAG_NO_OLLAMA" == "true" ]]; then
    for _oc_name in ollama-gpu-main ollama-cpu-router; do
        if is_container_running "$_oc_name"; then
            info "[--no-ollama] Deteniendo contenedor $_oc_name…"
            docker stop "$_oc_name" >/dev/null 2>&1 || true
        fi
    done
    warn "Ollama GPU/CPU omitidos (--no-ollama)."
...
fi  # [V56-N1] fin guard --no-ollama (secciones 1/8 y 2/8)
