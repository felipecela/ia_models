#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║         OMEN AI CLUSTER — Autoboot_Cluster_V12.sh                       ║
# ╠══════════════════════════════════════════════════════════════════════════╣
...
#
# USO:
#   ./Autoboot_Cluster_V12.sh            # Arranque completo
#   ./Autoboot_Cluster_V12.sh --stop     # Parar todo
#   ./Autoboot_Cluster_V12.sh --status   # Estado de servicios
#   ./Autoboot_Cluster_V12.sh --warmup   # Solo warmup del backend por defecto
#
set -uo pipefail

# ─────────────────────────────────────────────────────────────────────────────
#  CONFIGURACIÓN DE RUTAS — ajusta AI_CORE si tu SSD exFAT tiene otro punto
# ─────────────────────────────────────────────────────────────────────────────
MODELS_ROOT="/home/fcela-ga/sgoinfre/ai_core/models"
# AI_CORE="${AI_CORE:-/home/fcela-ga/sgoinfre/ai_core}"
AI_CORE="$MODELS_ROOT"
OLLAMA_MODELS_DIR="$AI_CORE"
EXLLAMA_MODELS_DIR="$AI_CORE"
SGLANG_MODELS_DIR="$AI_CORE"
OLLAMA_CPU_MODELS_DIR="$AI_CORE/ollama-cpu"

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROUTER_SCRIPT="$SCRIPT_DIR/orchestrator_router_V6.py"
TABBYAPI_CONFIG="$SCRIPT_DIR/config_tabbyapi_v12.yml"
ROUTER_LOG="$SCRIPT_DIR/router_v12.log"
ROUTER_PID_FILE="$SCRIPT_DIR/router_v12.pid"
...
# ─────────────────────────────────────────────────────────────────────────────
if [ "${1:-}" = "--stop" ]; then
    step "Deteniendo clúster OMEN V12…"
...
# ─────────────────────────────────────────────────────────────────────────────
if [ "${1:-}" = "--status" ]; then
    step "Estado del clúster OMEN V12"
    echo ""
    printf "  %-28s %s\n" "SERVICIO" "ESTADO"
    printf "  %-28s %s\n" "────────────────────────────" "────────"
    declare -A CHECKS=(
...
echo ""
echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}║      OMEN AI CLUSTER V12 — INICIALIZANDO                    ║${NC}"
...
[ -d "$OLLAMA_MODELS_DIR" ]  || { error "Directorio ollama_storage no encontrado: $OLLAMA_MODELS_DIR"; exit 1; }
[ -d "$EXLLAMA_MODELS_DIR" ] || { error "Directorio exllamav2_storage no encontrado"; exit 1; }
[ -d "$SGLANG_MODELS_DIR" ]  || { error "Directorio sglang_storage no encontrado"; exit 1; }
...
# ───────────────────────────────────────────────────────────────
#  TabbyAPI Configuration V12 — OMEN Cluster
...
TABBY_CONF
ok "config_tabbyapi_v12.yml generado"
...
docker volume create openclaw_data_v12 >/dev/null 2>&1 || true
...
docker run -d \
  ...
  -v openclaw_data_v12:/data \
  coollabsio/openclaw:latest

info "Esperando inicialización de OpenClaw (25s)…"
sleep 25

# Inyectar openclaw.json V12 con agentes especializados
info "Inyectando configuración V12 de OpenClaw (agentes especializados)…"
...
info "Reiniciando OpenClaw para asimilar configuración V12…"
docker restart openclaw-server >/dev/null
sleep 8

wait_http "http://localhost:8080" 35 "OpenClaw UI" || warn "OpenClaw no responde en :8080"
ok "OpenClaw V12 configurado con agentes @coder, @analyst, @reasoner"
...
echo -e "${GREEN}${BOLD}║         ✔ CLÚSTER OMEN V12 OPERATIVO                        ║${NC}"
echo -e "${GREEN}${BOLD}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "  ${CYAN}${BOLD}📊 OpenClaw UI:${NC}       http://localhost:8080"
