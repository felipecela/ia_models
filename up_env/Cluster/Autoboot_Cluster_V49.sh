...
# ║             Se define SCRIPT_VERSION="V49" al inicio del script.           ║
...
set -euo pipefail
SCRIPT_VERSION="V49"  # [V44-B1] Versión del script — usada en banners y logs
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V49 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V49"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V49"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V49]
#
# CAUSA RAÍZ V47/V48:
#   El entrypoint de OpenClaw 2026.6.8 exige al menos una variable de entorno
#   de API key ANTES de ejecutar configure o doctor. Sin ella, el proceso termina
#   inmediatamente y Docker lo reinicia en bucle.
#
# SOLUCIÓN V49 — doble variable para garantía:
#   1. OPENAI_API_KEY="sk-ollama-local"
#      Satisface SIEMPRE la validación del entrypoint. Es un valor de marcador
#      (cualquier string no vacío es aceptado). OpenClaw no llega a contactar
#      OpenAI porque inject_v49.py sobrescribe el config antes del primer uso.
#
#   2. OLLAMA_BASE_URL="http://host.docker.internal:${PORT_OLLAMA_CPU}"
#      Sin /v1 — formato que el entrypoint espera para Ollama local.
#      configure.js añade el provider ollama en openclaw.json.
#      inject_v49.py lo elimina y registra omen-router con la estructura
#      exacta confirmada en V47 (models.providers = OBJETO dict).
#
# ESTRUCTURA REAL openclaw.json (confirmada V47):
...
# ─── Limpieza ────────────────────────────────────────────────────────────────
info "[V49] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
# ─── Preseed: solo gateway.auth + controlUi ──────────────────────────────────
info "[V49] Generando preseed (gateway.auth + controlUi)…"
...
info "[V49] Copiando preseed al volumen…"
...
# ─── Lanzar openclaw-server con las dos variables de entorno ─────────────────
# [V49] OPENAI_API_KEY satisface la validación obligatoria del entrypoint.
# [V49] OLLAMA_BASE_URL (sin /v1) permite que configure detecte Ollama local.
# inject_v49.py elimina ambos providers y registra omen-router.
info "[V49] Lanzando openclaw-server…"
docker run -d \
...
# ─── Esperar health-monitor ───────────────────────────────────────────────────
info "[V49] Esperando que doctor + gateway terminen (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if echo "$_LOG" | grep -q "\[health-monitor\] started"; then
        OPENCLAW_READY=true
        ok "[V49] Gateway arrancado tras $((_oc_i * 2))s"
        break
    fi
    if echo "$_LOG" | grep -q "At least one AI provider API key env var is required"; then
        warn "[V49] ERROR CRÍTICO: entrypoint rechaza las keys — revisar variables de entorno"
...
info "[V49] Backends activos: ${_ACTIVE_BKS}"

# ─── Construir lista de modelos según backends activos ───────────────────────
cat > /tmp/build_models_v49.py << 'PYEOF'
import json, sys
...
_MODELS_JSON=$(echo "$_HEALTH_JSON" | python3 /tmp/build_models_v49.py 2>/dev/null || echo "[]")
...
if [[ "$_MODELS_JSON" == "[]" || -z "$_MODELS_JSON" || "$_MODEL_COUNT" == "0" ]]; then
    warn "[V49] Usando lista de modelos estática (fallback)"
...
info "[V49] Modelos a registrar: ${_MODEL_COUNT}"

# ─── Copiar modelos al contenedor y ejecutar inject_v49.py ───────────────────
echo "$_MODELS_JSON" > /tmp/oc_models_v49.json
docker cp /tmp/oc_models_v49.json openclaw-server:/tmp/oc_models.json 2>/dev/null || true

info "[V49] Inyectando provider OMEN Router V14 en openclaw.json…"
docker exec openclaw-server sh -c 'cat > /tmp/inject_v49.py' << 'PYEOF'
import json, sys
...
try:
    ...
    print(f"[V49] ERROR leyendo modelos: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"[V49] WARN: config no legible ({e}), partiendo de dict vacío")
    cfg = {}
...
# Eliminar providers inyectados por configure (ollama y openai)
for _rm in ("ollama", "openai"):
    removed = cfg["models"]["providers"].pop(_rm, None)
    if removed:
        print(f"[V49] Provider '{_rm}' eliminado")

# Añadir omen-router con estructura correcta (confirmada V47)
...
print(f"[V49] OK: provider '{PROVIDER_ID}' registrado")
print(f"[V49] OK: {len(models)} modelos bajo models.providers.{PROVIDER_ID}.models")
print(f"[V49] OK: agents.defaults.model.primary = '{PROVIDER_ID}/ruteador-auto'")
print(f"[V49] OK: plugins.entries.{PROVIDER_ID}.enabled = true")
PYEOF

docker exec openclaw-server sh -c \
    "sed -i 's|PORT_ROUTER_PLACEHOLDER|${PORT_ROUTER}|g' /tmp/inject_v49.py" 2>/dev/null || true

_OC_INJ_LOG="/tmp/oc_inject_v49.log"
docker exec openclaw-server python3 /tmp/inject_v49.py > "$_OC_INJ_LOG" 2>&1 || true
...
info "[V49] Enviando SIGHUP al gateway para reload del config…"
...
if [[ -n "$_GW_PID" && "$_GW_PID" =~ ^[0-9]+$ ]]; then
    ...
    ok "[V49] SIGHUP enviado al PID ${_GW_PID}"
else
    docker kill --signal=HUP openclaw-server 2>/dev/null || true
    ok "[V49] SIGHUP enviado al contenedor"
fi
sleep 4
...
info "[V49] Verificando config final…"
_OC_VERIFY_LOG="/tmp/oc_verify_v49.log"
...
try:
    ...
    print(f'[V49] Providers registrados: {list(providers.keys())}')
    print(f'[V49] Modelo primario: {primary}')
    for pid, pdata in providers.items():
        mcount = len(pdata.get('models', []))
        print(f'[V49]   {pid} → {pdata.get(\"baseUrl\",\"?\")}  ({mcount} modelos)')
except Exception as e:
    print(f'[V49] ERROR verificando config: {e}')
...
if command -v xdg-open &>/dev/null; then
    ...
    ok "[V49] Navegador abierto con token embebido"
else
    info "[V49] Abre manualmente: ${OPENCLAW_URL_TOKEN}"
fi
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V49"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V49 — iniciado${NC}"
