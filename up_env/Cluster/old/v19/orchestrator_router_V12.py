...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║ OMEN AI CLUSTER — Orchestrador Semántico V12 (build V19)                   ║
║ RTX 4070 8GB · Intel Ultra 7 · 32GB RAM · SSD exFAT                        ║
╠══════════════════════════════════════════════════════════════════════════════╣
║ V19 — Correcciones de auditoría integral sobre V18:                        ║
║  ✔ [V19-C1]  Sincronización _EMBED_DESCRIPTIONS con _SYSTEM_PHI4           ║
║              (restaurado "resultado exacto numérico STEM" en PRECISO)       ║
║  ✔ [V19-C2]  Fallback seguro de AGENT_DB_DIR: valida fs antes de usar      ║
║              (fuerza ext4/xfs/btrfs, rechaza exFAT/ntfs para SQLite)       ║
║  ✔ [V19-C3]  Eliminación de deadlock potencial en _agent_llm_call:         ║
║              semáforo del agente NO anida con _vram_lock                    ║
║  ✔ [V19-C4]  Connection pool SQLite: reutilización de conexión en          ║
║              _run_task para reducir overhead y busy_timeout                 ║
║  ✔ [V19-C5]  Sanitización RAG: delimitadores estrictos anti prompt-inject  ║
║  ✔ [V19-C6]  Validación de entrada en /v1/agent/tasks (max_prompt_len)     ║
║  ✔ [V19-C7]  Graceful shutdown mejorado: espera subtareas activas          ║
║  ✔ [V19-C8]  Timeout de cliente propagado: cancela fallback si el          ║
║              cliente HTTP original ya desconectó                            ║
║  ✔ [V19-C9]  Protección contra tareas canceladas en _run_task              ║
║  ✔ [V19-C10] Métricas de agente: tracking de duración por tarea            ║
║  ✔ [V19-C11] PRECISO_OPT incluido en _EMBED_DESCRIPTIONS                  ║
║  ✔ [V19-C12] Logging estructurado con task_id para trazabilidad            ║
╠══════════════════════════════════════════════════════════════════════════════╣
║ Heredado de V11/V18 (funcionalidades completas):                           ║
║  ✔ Autonomous Reasoning Agent (planificación, ejecución, validación)       ║
...
║  ✔ Rate limiting interno para agente                                        ║
...
# ─────────────────────────────────────────────────────────────────────────────
_LOG_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "router_v12.log")
...
log = logging.getLogger("router-v12")

# ─────────────────────────────────────────────────────────────────────────────
# RUTAS V19 — 7 niveles de razonamiento
# ─────────────────────────────────────────────────────────────────────────────
RUTAS: dict[str, dict] = {
    "CHAT": {
...
# ═══════════════════════════════════════════════════════════════════════════════
# AUTONOMOUS REASONING AGENT — [V18-A1] a [V18-A11] + [V19-C3..C12]
# ═══════════════════════════════════════════════════════════════════════════════
...
async def _agent_llm_call(
    ...
    try:
        ...
        try:
            ...
            pass  # Ya liberado en el caso de fallback
    return None
...
def _is_task_cancelled(task_id: str) -> bool:
    ...
    try:
        ...
        conn.close()
        return row and row["status"] == TaskStatus.CANCELLED
    except Exception:
        return False
...
def _extract_json(raw: str) -> str:
    ...
    if text.startswith("json"):
        text = text[4:].strip()
    # Buscar el primer { y último } como fallback adicional
...
async def _conmutar_vram(nivel: str) -> str:
    ...
    async with _vram_lock:
        ...
        for cont_name in incompatibles:
            ...
            except (docker.errors.NotFound if docker else Exception) as _nf:
                if docker is None:
                    raise
        ...
        try:
            ...
            if fb:
                ...
                if await request.is_disconnected():
                    log.debug(f"[PROXY] Cliente desconectado — cancelando fallback")
                    return
...
async def lifespan(app: FastAPI):
    ...
    log.info("═" * 66)
    log.info(" OMEN AI Router V12 (build V19) — iniciando…")
    log.info("═" * 66)
    ...
    try:
        ...
        pass
    log.info("[SHUTDOWN] ✔ Router V12 detenido correctamente")
...
# ═══════════════════════════════════════════════════════════════════════════════
# ENDPOINTS — Heredados de V10/V11
# ═══════════════════════════════════════════════════════════════════════════════

@app.get("/")
async def raiz():
    return {
        "servicio": "OMEN AI Router V12",
        "build":    "V19",
        "version":  "12.19.0",
