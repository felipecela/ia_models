#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║ OMEN AI Cluster — Autoboot V19                                             ║
# ║ RTX 4070 8GB · Intel Ultra 7 · 32GB RAM · SSD exFAT /mnt/ai_core          ║
# ╠══════════════════════════════════════════════════════════════════════════════╣
# ║ V19 — Correcciones de auditoría integral sobre V18:                        ║
# ║  ✔ [V19-C1]  Router actualizado a V12 (orchestrator_router_V12.py)         ║
# ║  ✔ [V19-C2]  Validación de filesystem: verifica que AGENT_DATA_DIR está    ║
# ║              en ext4/xfs/btrfs antes de arrancar (previene SQLite en exFAT)║
# ║  ✔ [V19-C3]  Graceful shutdown mejorado: espera 30s con feedback visual    ║
# ║  ✔ [V19-C4]  Indexador actualizado a V4 (indexar_vault_v4.py)             ║
# ║  ✔ [V19-C5]  Verificación de integridad de agent_tasks.db antes de backup ║
# ║  ✔ [V19-C6]  Detección de procesos zombi en puerto 8000 antes de arrancar ║
# ║  ✔ [V19-C7]  Health check post-arranque con reintentos y diagnóstico      ║
# ║  ✔ [V19-C8]  Limpieza de logs antiguos (mantener últimos 10)              ║
# ║  ✔ [V19-C9]  Verificación de versión mínima de Docker (20.10+)            ║
# ║  ✔ [V19-C10] PID_FILE actualizado para V12                                ║
# ║  ✔ [V19-C11] Exportar AGENT_DB_DIR con validación de fs                   ║
# ║  ✔ [V19-C12] Verificar espacio en SSD exFAT para modelos                  ║
# ╠══════════════════════════════════════════════════════════════════════════════╣
# ║ Heredado de V18 (todas las correcciones):                                  ║
# ║  ✔ [V18-A1]  Router con Autonomous Reasoning Agent                        ║
...
# ║  ✔ [V18-A12] Log rotado: autoboot_v19_YYYYMMDD_HHMMSS.log                ║
# ╠══════════════════════════════════════════════════════════════════════════════╣
...
# ║  ✔ [V17-A4]  indexar_vault: solo si ChromaDB responde 200                 ║
...
# ║  Router: orchestrator_router_V12.py :8000 (FastAPI + Agent Engine)        ║
# ╚══════════════════════════════════════════════════════════════════════════════╝
...
# ─────────────────────────────────────────────────────────────────────────────
# RUTAS ABSOLUTAS
# [V19-C1] Router y indexador actualizados a V12/V4
...
ROUTER_SCRIPT="$AI_HOME/orchestrator_router_V12.py"
VAULT_INDEXER="$AI_HOME/indexar_vault_v4.py"
AGENT_DATA_DIR="$AI_HOME/agent_data"            # [V18-A2] SQLite del agente
LOG_DIR="$AI_HOME/logs"
LOG_FILE="$LOG_DIR/autoboot_v19_$(date +%Y%m%d_%H%M%S).log"
PID_FILE="$AI_HOME/router_v12.pid"             # [V19-C10] Actualizado para V12
...
cleanup() {
    ...
    if [[ -f "$PID_FILE" ]]; then
        ...
        if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null; then
            info "Limpieza: enviando SIGTERM al router V12 (PID $pid)…"
            ...
            if kill -0 "$pid" 2>/dev/null; then
                ...
                ok "Router V12 detenido correctamente (${wait_count}s)"
            fi
        fi
        rm -f "$PID_FILE"
    fi
...
# INICIO
# ─────────────────────────────────────────────────────────────────────────────
section "OMEN AI Cluster — Autoboot V19"
...
# [V19-C9] Verificar versión mínima de Docker
...
if [[ ! -f "$ROUTER_SCRIPT" ]]; then
    err "Router V12 no encontrado: $ROUTER_SCRIPT"
    err "Copia orchestrator_router_V12.py a $AI_HOME/"
    exit 1
fi
...
ok "Router V12: sintaxis correcta"
...
# [V19-C8] Limpieza de logs antiguos (mantener últimos 10)
LOG_COUNT=$(ls -1 "$LOG_DIR"/autoboot_v*.log 2>/dev/null | wc -l)
if (( LOG_COUNT > 10 )); then
    ls -t "$LOG_DIR"/autoboot_v*.log 2>/dev/null | tail -n +11 | xargs rm -f 2>/dev/null || true
...
# [V19-C4] Actualizado a indexar_vault_v4.py
# ─────────────────────────────────────────────────────────────────────────────
...
if [[ -f "$VAULT_INDEXER" ]]; then
    ...
    if [[ "$CHROMA_HTTP_CODE" == "200" ]] && [[ "$OLLAMA_CPU_HTTP_CODE" == "200" ]]; then
        info "Lanzando indexación incremental del vault (V4)…"
    ...
    warn "indexar_vault_v4.py no encontrado en $AI_HOME — vault no indexado"
fi
...
# ─────────────────────────────────────────────────────────────────────────────
# ROUTER V12 — FastAPI + Autonomous Reasoning Agent (:8000)
# [V19-C1] Router actualizado a V12
# ─────────────────────────────────────────────────────────────────────────────
section "Router V12 (FastAPI + Agent :8000)"
...
# [V19-C6] Asegurar que el puerto 8000 esté libre (detectar procesos zombi)
if ss -tlnp 2>/dev/null | grep -q ':8000 '; then
    ...
    if [[ -n "$BLOCKING_PID" ]]; then
        ...
        warn "  PID=$BLOCKING_PID ($BLOCKING_CMD) ocupa el puerto 8000"
    fi
...
PYTHONUNBUFFERED=1 AGENT_DB_DIR="$AGENT_DATA_DIR" python3 "$ROUTER_SCRIPT" \
    >> "$LOG_DIR/router_v12.log" 2>&1 &
ROUTER_PID=$!
echo "$ROUTER_PID" > "$PID_FILE"
info "Router V12 lanzado — PID=$ROUTER_PID"
...
for i in {1..30}; do
    ...
    if ! kill -0 "$ROUTER_PID" 2>/dev/null; then
        err "Router V12 terminó inesperadamente. Últimas líneas del log:"
        tail -20 "$LOG_DIR/router_v12.log" 2>/dev/null || true
        break
    fi
    sleep 2
done
...
if $ROUTER_READY; then
    ok "Router V12 ✔ → http://localhost:8000 (respondió en intento $HEALTH_ATTEMPTS)"
    ...
    err "Router V12 no respondió en 60s"
    if kill -0 "$ROUTER_PID" 2>/dev/null; then
...
# RESUMEN FINAL
# ─────────────────────────────────────────────────────────────────────────────
section "Resumen del Cluster V19"
...
check_service "Router V12 (Agent)"      localhost 8000
...
echo "  Ver logs:         tail -f $LOG_DIR/router_v12.log"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V19 — iniciado${NC}"
