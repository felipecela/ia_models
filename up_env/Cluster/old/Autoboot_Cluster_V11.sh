#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║         OMEN AI CLUSTER — Autoboot_Cluster_V11.sh                       ║
...
# ║  ✔ SGLang incluye --quantization awq y --served-model-name               ║
# ║  ✔ OpenClaw config en formato correcto openclaw.json                     ║
# ║  ✔ Health checks con polling activo (no sleep ciego)                     ║
# ║  ✔ Router arranca ÚLTIMO (cuando los backends están listos)              ║
# ║  ✔ Logging con timestamps y colores                                      ║
# ║  ✔ ExLlamaV2 y SGLang en STANDBY al arrancar (el router los activa)      ║
# ╚══════════════════════════════════════════════════════════════════════════╝
#
# USO:
#   ./Autoboot_Cluster_V11.sh           # Arranque completo
#   ./Autoboot_Cluster_V11.sh --stop    # Parar todo el clúster
#   ./Autoboot_Cluster_V11.sh --status  # Estado de todos los servicios
#
set -uo pipefail

# ─────────────────────────────────────────────────────────────────────────────
#  CONFIGURACIÓN DE RUTAS
#  Ajusta AI_CORE si tu punto de montaje del SSD exFAT es diferente
# ─────────────────────────────────────────────────────────────────────────────
...
echo ""
echo -e "${CYAN}${BOLD}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}${BOLD}║         OMEN AI CLUSTER V11 — INICIALIZANDO                 ║${NC}"
...
[ -d "$OLLAMA_MODELS_DIR" ] || { error "No se encuentra $OLLAMA_MODELS_DIR"; exit 1; }
[ -d "$EXLLAMA_MODELS_DIR" ] || { error "No se encuentra $EXLLAMA_MODELS_DIR"; exit 1; }
[ -d "$SGLANG_MODELS_DIR" ] || { error "No se encuentra $SGLANG_MODELS_DIR"; exit 1; }
...
# Asegurarse de que el override de systemd usa el directorio correcto
...
#
#  NOTA EXFAT: Montamos ollama_storage como lectura-escritura.
#  Los modelos ya están descargados (phi4 fue instalado vía ollama pull).
#  Esta instancia Docker usa CUDA_VISIBLE_DEVICES="" para forzar CPU puro.
# ─────────────────────────────────────────────────────────────────────────────
...
#
#  NOTA: Se crea el contenedor pero se deja PARADO.
#  El router (orchestrator_router_V5.py) lo arrancará automáticamente
#  cuando se solicite el nivel INSTANTANEO.
#  Esto evita la contención de VRAM al arrancar.
#
#  NOTA EXFAT: Los modelos EXL2 están en exFAT. TabbAPI los lee correctamente
#  (solo necesita acceso de lectura a los pesos, no crea symlinks críticos).
# ─────────────────────────────────────────────────────────────────────────────
...
model:
  # Modelo cargado por defecto al arrancar TabbAPI
  # Debe existir como subdirectorio en /models (montado desde exllamav2_storage)
  model_name: qwen2.5-coder-7b-exl2

  # Longitud máxima de contexto (reducir si hay errores de VRAM)
  max_seq_len: 4096

  # Caché en 4 bits: reduce uso de VRAM ~30% con mínima pérdida de calidad
  cache_mode: Q4

  # Cargar en VRAM completo (8GB — modelo 6.95GB + caché)
  # Si hay errores de OOM, activar gpu_split para partial offload
  # gpu_split: [6, 2]   # 6GB VRAM + 2GB RAM
...
# Crear el contenedor en modo PARADO (--rm eliminado, usamos create + start manual)
# IMAGEN CORRECTA: ghcr.io/theroyallab/tabbyapi:latest (no berot3/tabbyapi)
docker create \
  --gpus all \
  --name exllamav2-api \
  -p 5000:5000 \
...
#
#  NOTA: Igual que TabbAPI, se crea pero se deja PARADO.
#  Activado por el router cuando se solicita nivel AGIL.
#
#  FIX: Añadidos --quantization awq y --served-model-name (faltaban en V10)
# ─────────────────────────────────────────────────────────────────────────────
...
#  PASO 6: OPENCLAW
#
#  FIX: La inyección de configuración usa el formato correcto openclaw.json
#  con la estructura models.providers (antes se inyectaba initial_providers.json
#  que OpenClaw no lee). El archivo va en /data/.openclaw/openclaw.json
# ─────────────────────────────────────────────────────────────────────────────
...
#  El router arranca ÚLTIMO para que todos los backends ya estén disponibles
# ─────────────────────────────────────────────────────────────────────────────
