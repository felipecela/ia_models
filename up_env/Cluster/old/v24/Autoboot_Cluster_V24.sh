#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║ OMEN AI Cluster — Autoboot V24                                             ║
# ║ RTX 4070 8GB · Intel Ultra 7 · 32GB RAM · SSD exFAT /mnt/ai_core          ║
# ╠══════════════════════════════════════════════════════════════════════════════╣
# ║ V24 — Nuevas funcionalidades sobre V23:                                    ║
# ║  ✔ [V24-E1] Flag --last: descarga docker pull de la imagen :latest para   ║
# ║             todos los servicios antes de arrancarlos. ChromaDB siempre     ║
# ║             usa la versión pinada (0.6.3) y nunca se actualiza con --last  ║
# ║  ✔ [V24-E2] Flag --help: muestra todos los flags disponibles del script   ║
# ║  ✔ [V24-E3] Bloque de parseo de argumentos (getopt) antes de set -euo    ║
# ║             Preparado para integrar --status y --stop en V25               ║
# ╠══════════════════════════════════════════════════════════════════════════════╣
...
_FLAG_LAST=false
_FLAG_HELP=false
# Reservados para V25 (subcomandos --status / --stop):
# _FLAG_STATUS=false
# _FLAG_STOP=false
...
║                                                                            ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  PRÓXIMAS VERSIONES (V25)                                                  ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
║  --status       (pendiente) Muestra el estado actual de todos los          ║
║                 servicios del cluster sin arrancar ni detener nada.        ║
║                                                                            ║
║  --stop         (pendiente) Detiene todos los contenedores y el router     ║
║                 de forma ordenada.                                         ║
║                                                                            ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  EJEMPLOS                                                                  ║
...
for _arg in "$@"; do
    case "$_arg" in
        ...
        --help|-h)
            ...
            ;;
        # Reservados para V25:
        # --status) _FLAG_STATUS=true ;;
        # --stop)   _FLAG_STOP=true   ;;
        *)
...
LOG_FILE="$LOG_DIR/autoboot_v24_$(date +%Y%m%d_%H%M%S).log"  # [V22-C9] renombrado, [V23-D1] bridge fix
...
# Se establece aquí para que la variable de configuración inicial sea el default
# y el flag de CLI la sobreescriba si fue pasado.
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V24"
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V24"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V24 — iniciado${NC}"
