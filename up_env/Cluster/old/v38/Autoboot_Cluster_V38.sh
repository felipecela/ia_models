...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V38 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V38"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V38"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V37-O1 / V38]
# Integrado desde V10 (etapas 1/5, 3/5, 4/5, 5/5).
# Etapa 2/5 omitida: V38 usa orchestrator_router_V14.py.
# Levantado DESPUÉS del Router V14: endpoint :8000/v1 ya activo al inyectar.
# V38: config JSON completa con token + allowInsecureAuth para conexión automática.
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

# ─── Limpieza idempotente ────────────────────────────────────────────────────
info "[V38] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
# ─── Pull condicional — solo con --last  [V37-O2] ────────────────────────────
...
# ─── [V38-O5] Esperar a que el entrypoint finalice configure+doctor ──────────
# El entrypoint ejecuta 'configure' y sobreescribe /data/.openclaw/openclaw.json.
# Hay que esperar a que termine ANTES de inyectar nuestra config.
# Señal de finalización: la línea "config written" aparece en los logs.
info "Esperando a que OpenClaw complete configure+doctor (máx ${TIMEOUT_OPENCLAW}s)…"
OPENCLAW_CONFIGURE_DONE=false
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    if docker logs openclaw-server 2>&1 | grep -q "config written\|running openclaw doctor"; then
        OPENCLAW_CONFIGURE_DONE=true
        ok "OpenClaw: configure completado (${_oc_i}× 2s = $((_oc_i * 2))s)"
        break
    fi
    sleep 2
done
if [[ "$OPENCLAW_CONFIGURE_DONE" == "false" ]]; then
    warn "Timeout esperando configure — inyectando config de todos modos"
fi
# Pausa adicional para que doctor --fix termine de reescribir el JSON
sleep 5

# ─── [V38-O1] Inyección completa de openclaw.json ────────────────────────────
# Se inyecta DESPUÉS de que el entrypoint termina configure+doctor, para que
# nuestra versión no sea sobreescrita.
# Clave nueva: gateway.auth.token fija el shared-secret del WebSocket.
# gateway.controlUi.allowInsecureAuth + dangerouslyDisableDeviceAuth permiten
# conexión automática desde el navegador en HTTP local sin device-pairing.
info "[V38-O1] Inyectando openclaw.json completo (token + auth + CORS + providers)…"
...
docker exec openclaw-server sh -c "cat > /data/.openclaw/openclaw.json << 'OCLEOF'
{
  ...
  \"providers\": [
    {
      \"id\": \"openai_proxy\",
      \"name\": \"Ruteador Semantico Local V14\",
...
ok "openclaw.json inyectado con token + auth automática + providers"

# ─── [V38-O2] Recargar config sin docker restart (evita WebSocket 1006) ──────
# SIGHUP recarga la config del proceso gateway sin reiniciar el contenedor,
# evitando el cierre abrupto del WebSocket que causaba el error 1006.
# Fallback: docker restart si el PID del gateway no está disponible.
info "[V38-O2] Recargando config del gateway via SIGHUP (sin reiniciar contenedor)…"
GATEWAY_PID=$(docker exec openclaw-server sh -c 'pgrep -f "openclaw.*gateway\|openclaw.*run\|node.*openclaw" | head -1' 2>/dev/null || echo "")
if [[ -n "$GATEWAY_PID" ]]; then
    docker exec openclaw-server sh -c "kill -HUP ${GATEWAY_PID}" 2>/dev/null \
        ...
        || { warn "[V38-O2] SIGHUP falló — usando docker restart como fallback"; docker restart openclaw-server >/dev/null; }
else
    warn "[V38-O2] PID del gateway no detectado — usando docker restart como fallback"
    docker restart openclaw-server >/dev/null
fi

# ─── Verificación HTTP post-recarga ──────────────────────────────────────────
info "Esperando que OpenClaw esté disponible (máx 20s)…"
OPENCLAW_HTTP="000"
for _oc_w in $(seq 1 10); do
    ...
    if [[ "$OPENCLAW_HTTP" =~ ^(200|301|302)$ ]]; then
        ok "OpenClaw Server ✔ → HTTP ${OPENCLAW_HTTP} (${_oc_w}× 2s)"
        break
    fi
    sleep 2
done
if [[ ! "$OPENCLAW_HTTP" =~ ^(200|301|302)$ ]]; then
    warn "OpenClaw Server respondió HTTP ${OPENCLAW_HTTP} tras la recarga"
    warn "Puede necesitar unos segundos más — accede manualmente si es necesario"
fi
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V38"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V38 — iniciado${NC}"
