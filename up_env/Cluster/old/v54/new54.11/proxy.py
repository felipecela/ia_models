...
_MIN_CTX_GPU = 16384  # Valor base heredado (fallback para niveles no listados)

# [V28-CTX] Contexto mínimo por nivel — qwen2.5:32b en 8GB VRAM no puede asumir 16384
_CTX_POR_NIVEL = {
    "CHAT":        4096,
    "INSTANTANEO": 4096,
    "AGIL":        8192,
    ...
    "PRECISO":     8192,
    "PRECISO_OPT": 8192,
    "MASIVO":      4096,   # qwen2.5:32b en 8GB VRAM — no forzar ctx grande
}
...
def validate_and_truncate_messages(body: dict, nivel: str, model: str) -> dict:
    ...
    # [V28-SAFE-TOOLS] Si el exceso viene de tools/schemas (ej. OpenClaw), retirarlas primero.
    # OpenClaw manda schemas enormes aunque el prompt sea trivial.
    # Antes: proxy truncaba solo el mensaje usuario (pesa casi nada) → 35928→35915 sin reducción.
    # Ahora: si hay tools, se eliminan primero y se re-evalúa si ya cabe.
    if "tools" in body:
        tools_count = len(body.get("tools") or [])
        body.pop("tools", None)
        body.pop("tool_choice", None)
        estimated_no_tools = estimate_body_tokens(body)
        log.warning(
            "[V28-SAFE-TOOLS] Payload excede contexto; eliminadas %d tools: %d → %d tokens",
            tools_count, estimated, estimated_no_tools,
        )
        estimated = estimated_no_tools
        if estimated <= max_input_tokens:
            log.info("[V28-SAFE-TOOLS] Payload OK tras eliminar tools (%d tokens)", estimated)
            return body
...
def inject_opciones_extra(body: dict, nivel: str, modelo: str = "") -> dict:
    ...
    if nivel in _GPU_NIVELES:
        ...
        if not isinstance(opts, dict):
            opts = {}
        # [V28-CTX] Usar tabla por nivel; evita forzar num_ctx=16384 a qwen2.5:32b en MASIVO
        target_ctx = _CTX_POR_NIVEL.get(nivel, _MIN_CTX_GPU)
        if opts.get("num_ctx", 0) < target_ctx:
