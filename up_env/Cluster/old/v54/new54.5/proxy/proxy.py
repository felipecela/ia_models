#!/usr/bin/env python3
"""
proxy.py - OMEN AI Router V14 (build V24)
...
async def _proxy_streaming(
    ...
    El status HTTP llega con las cabeceras, sin coste de generacion.
    Elimina la doble llamada de V21/V22/V23.
    """
    try:
        ...
        return await _handle_timeout_fallback(
            body, request, nivel, modelo_usado, http_client
        )
    except Exception as exc:
        log.error("[PROXY] Error conectando a %s: %s", nivel, exc)
        return JSONResponse(
            ...
            try:
                async with http_client.stream(
                    ...
                    if resp.status_code >= 400:
                        err = json.dumps({
                            "error": {
                                "message": "Fallback error %d" % resp.status_code
                            }
                        })
                        yield ("data: " + err + "\n\ndata: [DONE]\n\n").encode()
                        return
