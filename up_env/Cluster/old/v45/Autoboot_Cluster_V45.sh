...
# ║             Se define SCRIPT_VERSION="V45" al inicio del script.           ║
...
set -euo pipefail
SCRIPT_VERSION="V45"  # [V44-B1] Versión del script — usada en banners y logs
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V45 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V45"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V45"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V45]
#
# DIAGNÓSTICO V43 — tres bugs identificados en los logs:
#
# [V44-B1] SCRIPT_VERSION sin definir → --stop abortaba en línea 642 con
#          "variable sin asignar" y set -euo pipefail activado.
#          Fix: SCRIPT_VERSION="V45" añadida tras set -euo pipefail.
#
# [V44-B2] Señal "[gateway] ready" nunca aparece en los logs reales.
#          La secuencia real confirmada por los logs V43 es:
#            loading configuration → resolving authentication → starting...
#            → starting HTTP server → Gateway is binding → [health-monitor] started
#          Fix: el bucle busca "[health-monitor] started" como señal definitiva.
#
# [V44-B3] OC_PROVIDER_JSON llegaba vacío al docker exec porque bash expande
#          la variable en el host antes de pasarla al contenedor, y el JSON
#          con comillas dobles/tildes se corrompe en el proceso. 
#          Fix: el JSON se escribe en /tmp/oc_provider.json en el host, se 
#          copia al contenedor con docker cp y python3 lo lee del fichero.
#
# [V44-B4] OpenClaw usaba openai/gpt-5.5 contra api.openai.com y fallaba 401
#          porque sk-router-local no es una API key de OpenAI real.
#          Fix: OPENAI_API_KEY suprimida del docker run para que configure no
#          registre el provider OpenAI externo. Solo queda nuestro provider local.
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

info "[V44] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
# ─── Preseed: SOLO gateway config, SIN providers, SIN OpenAI ─────────────
info "[V44] Generando preseed de gateway (sin providers, sin OpenAI)…"
...
ok "Preseed gateway listo"

# ─── Inyectar preseed en volumen vía docker cp ───────────────────────────
info "[V44] Copiando preseed al volumen (docker cp)…"
...
# ─── Lanzar openclaw-server [V44-B4: sin OPENAI_API_KEY] ─────────────────
info "[V44] Lanzando openclaw-server (sin OPENAI_API_KEY para evitar 401)…"
docker run -d \
...
# ─── [V44-B2] Esperar señal real: [health-monitor] started ──────────────
# Confirmada en los logs V43 como última línea del arranque normal.
info "[V44-B2] Esperando '[health-monitor] started' (máx ${TIMEOUT_OPENCLAW}s)…"
OPENCLAW_READY=false
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    _LOG=$(docker logs openclaw-server 2>&1 || true)
    _OC_HM=$(echo "$_LOG" | grep -c "\[health-monitor\] started" || true)
    if [[ "$_OC_HM" -ge 1 ]]; then
        OPENCLAW_READY=true
        ok "[V44-B2] Gateway arrancado tras $((_oc_i * 2))s — health-monitor activo"
        break
    fi
    # Detección temprana de EROFS
    _EROFS=$(echo "$_LOG" | grep -c "EROFS" || true)
    if [[ "$_EROFS" -ge 1 ]]; then
        warn "[V44] EROFS detectado — JSON puede ser de solo lectura"
...
if [[ "$OPENCLAW_READY" == "false" ]]; then
    warn "[V44-B2] health-monitor no detectado en ${TIMEOUT_OPENCLAW}s"
    docker logs openclaw-server 2>&1 | tail -8 | while IFS= read -r l; do warn "  $l"; done || true
fi
# Pausa adicional: el gateway necesita ~2s extra para que la API HTTP esté lista
sleep 3

# ─── [V44-B3] Consultar backends activos y construir JSON providers ───────
info "[V44-B3] Consultando backends activos en Router V14…"
_HEALTH_JSON=$(curl --noproxy "*" -s -m 5 "http://localhost:${PORT_ROUTER}/health" 2>/dev/null || echo "{}")

# Construir JSON del provider con python3 en el HOST y escribirlo a un fichero
# [V44-B3]: evita pasar JSON con comillas/tildes como variable de entorno al contenedor
cat > /tmp/build_provider_v44.py << 'PYEOF'
import json, sys
...
models = [{"id": "ruteador-auto", "name": "Auto — clasificador 4 capas"}]
active = []
...
phi4 = health.get("clasificador", {}).get("phi4_cpu_avail", False)
if phi4:
    models.append({"id": "phi4", "name": "Phi-4 CPU — clasificador directo"})

# baseUrl se reemplaza desde bash con sed para evitar problemas con ${}
provider = {
    "id": "omen_router_v14",
    "name": "OMEN Router V14 — Cluster Local",
    "baseUrl": "ROUTER_BASE_URL_PLACEHOLDER",
    "apiKey": "sk-router-local",
    "models": models,
    "enabled": True
}

result = {"provider": provider, "active": active, "count": len(models)}
print(json.dumps(result, ensure_ascii=False))
PYEOF

_BUILD_OUT=$(echo "$_HEALTH_JSON" | python3 /tmp/build_provider_v44.py 2>/dev/null || echo '{}')

# Extraer provider JSON y sustituir el placeholder por la URL real con sed
_PROVIDER_JSON=$(echo "$_BUILD_OUT" | python3 -c "
import json,sys
d=json.load(sys.stdin)
print(json.dumps(d.get('provider',{}), ensure_ascii=False))
" 2>/dev/null | sed "s|ROUTER_BASE_URL_PLACEHOLDER|http://host.docker.internal:${PORT_ROUTER}/v1|g" || echo "")

_ACTIVE_BKS=$(echo "$_BUILD_OUT" | python3 -c "import json,sys; d=json.load(sys.stdin); print(', '.join(d.get('active',[])))" 2>/dev/null || echo "desconocido")
_MODEL_COUNT=$(echo "$_BUILD_OUT" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d.get('count',0))" 2>/dev/null || echo "0")

info "[V44-B3] Backends activos: ${_ACTIVE_BKS:-ninguno detectado}"
info "[V44-B3] Modelos a registrar: ${_MODEL_COUNT}"

# Si python3 falló, usar JSON estático de fallback
if [[ -z "$_PROVIDER_JSON" || "$_PROVIDER_JSON" == "{}" ]]; then
    warn "[V44-B3] Fallback a provider estático (todos los modelos principales)"
    _PROVIDER_JSON=$(printf '%s' \
        '{"id":"omen_router_v14","name":"OMEN Router V14 — Cluster Local",' \
        '"baseUrl":"http://host.docker.internal:'"${PORT_ROUTER}"'/v1",' \
        '"apiKey":"sk-router-local",' \
        '"models":[' \
        '{"id":"ruteador-auto","name":"Auto — clasificador 4 capas"},' \
        '{"id":"profundo","name":"Profundo — DeepSeek R1 14B"},' \
        '{"id":"phi-mayor-precision","name":"Phi Mayor Precisión — phi4-reasoning:plus"},' \
        '{"id":"phi-optimizada","name":"Phi Optimizada — phi4-reasoning:14b-q4_K_M"},' \
        '{"id":"masivo","name":"Masivo — Qwen2.5 32B"},' \
        '{"id":"agent-autonomo","name":"Agente Autónomo — planifica+ejecuta+valida"}' \
        '],"enabled":true}')
fi

# [V44-B3] Escribir el JSON a fichero temporal y copiarlo al contenedor
# Esto evita COMPLETAMENTE el problema de expansión de variables/comillas en env vars
_OC_PROVIDER_FILE="/tmp/oc_provider_v44.json"
echo "$_PROVIDER_JSON" > "$_OC_PROVIDER_FILE"
docker cp "$_OC_PROVIDER_FILE" openclaw-server:/tmp/oc_provider.json 2>/dev/null || true

# Inyectar provider leyendo desde el fichero dentro del contenedor
info "[V44-B3] Inyectando provider desde fichero en el contenedor…"
docker exec openclaw-server python3 /tmp/inject_provider.py 2>&1 | \
# Subir el script de inyección al contenedor primero
docker exec openclaw-server sh -c 'cat > /tmp/inject_provider.py' << 'INJECT_EOF'
import json, sys
cfg_path = '/data/.openclaw/openclaw.json'
prov_path = '/tmp/oc_provider.json'
try:
    with open(prov_path, 'r') as f:
        provider = json.load(f)
    if not provider or not provider.get('id'):
        print('[V44-B3] ERROR: fichero de provider vacío o inválido')
        sys.exit(1)
    ...
    print(f'[V44-B3] OK: {len(updated)} provider(s), {len(provider.get("models",[]))} modelos escritos')
except Exception as e:
    print(f'[V44-B3] ERROR: {e}')
    sys.exit(1)
INJECT_EOF

# Ejecutar el script de inyección
docker exec openclaw-server python3 /tmp/inject_provider.py 2>&1 | \
    while IFS= read -r line; do
        if echo "$line" | grep -qi "error\|fail"; then warn "  $line"; else ok "  $line"; fi
    done || warn "[V44-B3] Error al ejecutar inject_provider.py"

# SIGHUP al gateway para reload en caliente del config actualizado
info "[V44] Enviando SIGHUP al gateway para reload del config…"
_GW_PID=$(docker exec openclaw-server sh -c \
    "ps aux 2>/dev/null | grep -v grep | grep -E 'node' | awk '{print \$1}' | head -1" \
    2>/dev/null || echo "")
if [[ -n "$_GW_PID" && "$_GW_PID" =~ ^[0-9]+$ ]]; then
    docker exec openclaw-server kill -HUP "$_GW_PID" 2>/dev/null || true
    ok "[V44] SIGHUP enviado al PID ${_GW_PID}"
else
    docker kill --signal=HUP openclaw-server 2>/dev/null || true
    ok "[V44] SIGHUP enviado al contenedor"
fi
sleep 3

# ─── Verificar HTTP y URL ────────────────────────────────────────────────
...
if command -v xdg-open &>/dev/null; then
    ...
    ok "[V44] Navegador abierto con token embebido"
else
    info "[V44] Abre manualmente: ${OPENCLAW_URL_TOKEN}"
fi
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V45"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V45 — iniciado${NC}"
