...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V42 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V42"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V42"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V42]
#
# DIAGNÓSTICO V41 → CAUSA RAÍZ:
#   El bind-mount ":ro" sobre /data/.openclaw/openclaw.json hace que el archivo
#   sea de solo lectura DENTRO del contenedor. configure.js lo lee correctamente
#   (log: "merged persisted config") pero en la última línea hace writeFileSync()
#   para guardar el estado final → EROFS crash → entrypoint se reinicia en bucle.
#
# SOLUCIÓN V42 — docker cp en lugar de bind-mount:
#   1. Se levanta un contenedor init efímero (busybox) con SOLO el volumen montado.
#   2. Se copia el JSON preseed al volumen vía "docker cp" → archivo escribible.
#   3. El contenedor init se elimina.
#   4. Se lanza openclaw-server con SOLO el volumen (sin bind-mount).
#   configure.js lee el JSON, lo modifica libremente y el gateway arranca.
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

info "[V42] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
docker stop openclaw-server  2>/dev/null || true
docker rm   openclaw-server  2>/dev/null || true
docker rm   openclaw-init    2>/dev/null || true   # por si quedó de ejecución anterior
...
if ! docker volume ls --format '{{.Name}}' | grep -qx "openclaw_data_final"; then
    ...
    # Limpiar config anterior para que reciba el preseed fresco
...
# ─── [V42-O1] Generar preseed JSON en el HOST ────────────────────────────
# El archivo se genera en /tmp del host (escribible) y se copia al volumen
# con docker cp — sin :ro, por lo que configure.js puede modificarlo.
info "[V42-O1] Generando JSON preseed en ${OPENCLAW_PRECONFIG}…"
...
ok "JSON preseed listo: ${OPENCLAW_PRECONFIG}"

# ─── [V42-O2] Inyectar JSON en el volumen vía docker cp ─────────────────
# Se usa un contenedor init efímero solo para pre-crear el directorio y
# mantenerlo activo los 2s que necesita docker cp para copiar el archivo.
info "[V42-O2] Inyectando JSON preseed en el volumen (docker cp, sin :ro)…"
docker run -d --name openclaw-init \
    -v openclaw_data_final:/data \
    busybox sh -c "mkdir -p /data/.openclaw && sleep 10" >/dev/null 2>&1

# Esperar a que el contenedor init esté Running
_INIT_READY=false
...
if [[ "$_INIT_READY" == "true" ]]; then
    ...
    ok "JSON inyectado en el volumen correctamente"
else
    warn "Contenedor init no arrancó — intentando cp directo al volumen…"
    # Fallback: montar volumen y copiar directamente con otro contenedor
    docker run --rm \
        -v openclaw_data_final:/data \
    ...
    ok "JSON inyectado vía fallback busybox"
fi

docker rm -f openclaw-init 2>/dev/null || true

# Verificar que el archivo quedó en el volumen
_JSON_OK=$(docker run --rm -v openclaw_data_final:/data busybox \
    sh -c 'test -f /data/.openclaw/openclaw.json && echo ok || echo missing' 2>/dev/null || echo "error")
if [[ "$_JSON_OK" == "ok" ]]; then
    ok "Verificado: /data/.openclaw/openclaw.json presente en el volumen"
else
    warn "No se pudo verificar el JSON en el volumen (continuando de todos modos)"
fi

# ─── Lanzar openclaw-server SIN bind-mount ─────────────────────────────
info "[V42] Lanzando openclaw-server (imagen: $IMG_OPENCLAW)…"
docker run -d \
...
# ─── [V42-O3] Esperar señal "[gateway] ready" ────────────────────────────
info "[V42-O3] Esperando '[gateway] ready' (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if [[ "$_OC_READY_COUNT" -ge 1 ]]; then
        OPENCLAW_READY=true
        ok "[V42-O3] Gateway listo: configure×${_OC_CONFIGURE_COUNT} doctor×${_OC_DOCTOR_COUNT} ready×${_OC_READY_COUNT} ($((_oc_i * 2))s)"
        break
    fi
    # Detectar crash EROFS temprano para abortar el bucle sin esperar
    _EROFS=$(echo "$_LOG" | grep -c "EROFS" || true)
    if [[ "$_EROFS" -ge 1 ]]; then
        warn "[V42-O3] EROFS detectado en logs — el JSON del volumen puede ser :ro"
        warn "  Verifica que el volumen openclaw_data_final no tenga el archivo montado en :ro"
...
if [[ "$OPENCLAW_READY" == "false" ]]; then
    warn "[V42-O3] Gateway no detectado en ${TIMEOUT_OPENCLAW}s"
    docker logs openclaw-server 2>&1 | tail -8 | while IFS= read -r line; do
        warn "  log: $line"
    done || true
fi

# ─── Verificar HTTP ──────────────────────────────────────────────────────
info "Verificando disponibilidad HTTP de OpenClaw (máx 20s)…"
OPENCLAW_HTTP="000"
...
if [[ ! "$OPENCLAW_HTTP" =~ ^(200|301|302)$ ]]; then
    warn "OpenClaw HTTP respondió ${OPENCLAW_HTTP} — puede necesitar unos segundos más"
fi

# ─── [V42-O4] Sin restart ni SIGHUP — URL con token embebido ────────────
OPENCLAW_URL_TOKEN="http://localhost:${PORT_OPENCLAW}/#token=${OPENCLAW_TOKEN}"
info "[V42-O4] Sin recargas del gateway (evita cierre WS 1006)"
echo ""
echo -e "  ${GRN}${BLD}→ ${OPENCLAW_URL_TOKEN}${NC}"
echo ""
if command -v xdg-open &>/dev/null; then
    xdg-open "$OPENCLAW_URL_TOKEN" 2>/dev/null &
    ok "[V42] Navegador abierto automáticamente con token embebido"
else
    info "[V42] xdg-open no disponible — copia la URL en el navegador"
fi
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V42"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V42 — iniciado${NC}"
