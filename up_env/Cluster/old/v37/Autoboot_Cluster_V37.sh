...
TIMEOUT_OPENCLAW=30         # [V37-O1] Espera post-reinicio para asimilar config
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V37"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V37-O1]
# Integrado desde V10 (etapas 1/5, 3/5, 4/5, 5/5).
# Etapa 2/5 de V10 omitida: V37 ya dispone de orchestrator_router_V14.py.
# Se levanta DESPUÉS del Router V14 para garantizar que el endpoint
# http://host.docker.internal:$PORT_ROUTER/v1 ya está operativo al
# inyectar la configuración de proveedores en caliente.
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

# ─── Limpieza idempotente (etapa 1/5 de V10 adaptada) ────────────────────────
info "[V37-O1] Limpiando contenedor previo 'openclaw-server' si existe…"
docker stop openclaw-server 2>/dev/null && docker rm openclaw-server 2>/dev/null || true

# ─── Volumen idempotente (etapa 3/5 de V10 — V37-O3) ─────────────────────────
...
# ─── Lanzar contenedor (etapa 4/5 de V10 adaptada) ───────────────────────────
...
# Espera inicial al motor interno
info "Esperando ${TIMEOUT_OPENCLAW}s a que OpenClaw inicialice su motor…"
sleep "$TIMEOUT_OPENCLAW"

# ─── Inyección de configuración en caliente (etapa 5/5 de V10 adaptada) ──────
info "[V37-O1] Inyectando parches de red en caliente…"

docker exec openclaw-server sh -c 'mkdir -p /data/.openclaw'

# Parche CORS: bind LAN + orígenes permitidos
...
# Proveedores: apunta al Router V14 (Puerto $PORT_ROUTER) con los modelos de V37
docker exec openclaw-server sh -c 'cat > /data/initial_providers.json << '"'"'PROVEOF'"'"'
{
  "providers": [
    {
      "id": "openai_proxy",
...
# Reinicio para asimilar la configuración inyectada
info "Reiniciando openclaw-server para aplicar configuración…"
docker restart openclaw-server >/dev/null

# Verificación HTTP post-reinicio
OPENCLAW_HTTP=$(curl --noproxy "*" --ipv4 -s -m 10 -o /dev/null -w "%{http_code}" \
    "http://localhost:${PORT_OPENCLAW}" 2>/dev/null || echo "000")
if [[ "$OPENCLAW_HTTP" =~ ^(200|301|302)$ ]]; then
    ok "OpenClaw Server ✔ → http://localhost:${PORT_OPENCLAW} (HTTP $OPENCLAW_HTTP)"
else
    warn "OpenClaw Server respondió HTTP ${OPENCLAW_HTTP} — puede necesitar unos segundos más"
    warn "Acceso manual: http://localhost:${PORT_OPENCLAW}"
    warn "Token:         7c9b84a2f1e63d5c8a4b29f7e0d1c4a5b6e7f8d9c0a1b2c3d4e5f6a7b8c9d0e1"
fi

# ═══════════════════════════════════════════════════════════════════════════════
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V37"
...
echo "  WebUI:      http://localhost:${PORT_OPENCLAW}"
echo "  Token:      7c9b84a2f1e63d5c8a4b29f7e0d1c4a5b6e7f8d9c0a1b2c3d4e5f6a7b8c9d0e1"
echo ""
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V37 — iniciado${NC}"
