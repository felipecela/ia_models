...
# ╠══════════════════════════════════════════════════════════════════════════════╣
# ║ V25 — Nuevas funcionalidades sobre V24:                                    ║
...
LOG_FILE="$LOG_DIR/autoboot_v25_$(date +%Y%m%d_%H%M%S).log"  # [V22-C9] renombrado, [V23-D1] bridge fix
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    section "Deteniendo OMEN AI Cluster V25…"
    # 1. Router Python
    if [[ -f "$PID_FILE" ]]; then
    ...
    ok "Cluster V25 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V25"
    echo ""
    ...
    declare -A SVC_URLS=(
        ...
        ["SearXNG        :${PORT_SEARXNG}"]="http://localhost:${PORT_SEARXNG}/search?q=test&format=json"
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V25"
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V25"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V25 — iniciado${NC}"
