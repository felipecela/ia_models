...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V40 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V40"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V40"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V40]
# Integrado desde V10 (etapas 1/5, 3/5, 4/5, 5/5). Etapa 2/5 omitida.
# Levantado DESPUÉS del Router V14: endpoint :8000/v1 ya activo.
#
# V39 — Dos cambios críticos sobre V38:
#   [V39-O1] El entrypoint ejecuta configure+doctor DOS veces. La segunda
#            vuelta escribe "merged persisted config" y doctor borra los
#            providers que inyectamos en la primera. Ahora se espera la
#            señal del SEGUNDO ciclo completo.
#   [V39-O2] Los providers se registran vía API REST del gateway
#            (POST /api/providers) después de que el gateway está activo.
#            Así doctor --fix no puede borrarlos.
#   [V39-O3] Los 11 modelos lógicos del Router V14 (ids exactos de /v1/models)
#            se registran con nombres legibles en el selector de OpenClaw.
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

# ─── Limpieza idempotente ────────────────────────────────────────────────────
info "[V39] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
docker stop openclaw-server 2>/dev/null && docker rm openclaw-server 2>/dev/null || true
...
if ! docker volume ls --format '{{.Name}}' | grep -qx "openclaw_data_final"; then
    ...
    ok "Volumen 'openclaw_data_final' ya existe"
fi

# ─── Pull condicional — solo con --last ──────────────────────────────────────
pull_if_last "$IMG_OPENCLAW" "OpenClaw Server"

# ─── Lanzar contenedor ───────────────────────────────────────────────────────
info "Lanzando openclaw-server (imagen: $IMG_OPENCLAW)…"
docker run -d \
...
# ─── [V40-O1] Esperar SOLO "Gateway is binding" (señal real de disponibilidad) ──
# Diagnóstico confirmado por los logs de V39:
#   - El entrypoint ejecuta configure UNA sola vez (no dos).
#   - doctor --fix sin TTY NO se bloquea: termina e imprime "Doctor changes".
#   - La señal definitiva de que el gateway está activo y listo para recibir
#     conexiones es la línea "Gateway is binding to a non-loopback address".
#   - V39 esperaba "config written" ×2 que NUNCA llega → timeout → set -e → abort.
#
# La condición correcta es una sola: "Gateway is binding" en los logs.
# Se usa || true en todos los greps para blindar set -euo pipefail (grep
# devuelve código 1 cuando no encuentra matches, lo que dispararía el trap).
info "[V40-O1] Esperando que el gateway OpenClaw esté activo (máx ${TIMEOUT_OPENCLAW}s)…"
OPENCLAW_READY=false
_OC_CONFIGURE_COUNT=0
_OC_DOCTOR_COUNT=0
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    _LOG=$(docker logs openclaw-server 2>&1 || true)
    # [V40-O2/O3] || true en cada grep para blindar set -euo pipefail
    _OC_CONFIGURE_COUNT=$(echo "$_LOG" | grep -c "config written"  || true)
    _OC_DOCTOR_COUNT=$(echo    "$_LOG" | grep -c "Doctor changes"  || true)
    # Condición única y real: el gateway está enlazado y escuchando
    # [V40-O2] grep -q devuelve 0 si hay match, 1 si no → blindado con subshell
    _GW_BINDING=$(echo "$_LOG" | grep -c "Gateway is binding" || true)
    if [[ "$_GW_BINDING" -ge 1 ]]; then
        OPENCLAW_READY=true
        ok "[V40-O1] Gateway activo: configure×${_OC_CONFIGURE_COUNT}, doctor×${_OC_DOCTOR_COUNT} (${_oc_i}× 2s = $((_oc_i * 2))s)"
        break
    fi
    sleep 2
done
if [[ "$OPENCLAW_READY" == "false" ]]; then
    warn "[V40-O1] Gateway no detectado en ${TIMEOUT_OPENCLAW}s — inyectando config de todos modos"
    warn "  configure count: ${_OC_CONFIGURE_COUNT}, doctor count: ${_OC_DOCTOR_COUNT}"
    # Mostrar las últimas líneas del log para diagnóstico
    docker logs openclaw-server 2>&1 | tail -5 | while IFS= read -r line; do
        warn "  log: $line"
    done || true
fi
# Pausa de seguridad: el gateway necesita ~2s adicionales para inicializar la API REST
sleep 4

# ─── [V39-O1] Inyectar openclaw.json con auth — SIN providers ───────────────
# Los providers los inyectamos después vía API REST para que doctor no los borre.
info "[V39-O1] Inyectando openclaw.json (token + auth + CORS — sin providers)…"

docker exec openclaw-server sh -c 'mkdir -p /data/.openclaw'

docker exec openclaw-server sh -c "cat > /data/.openclaw/openclaw.json << 'OCLEOF'
{
  \"gateway\": {
    \"bind\": \"lan\",
    \"auth\": {
...
ok "openclaw.json inyectado (auth + CORS)"

# ─── [V38-O2] Recargar config via SIGHUP (evita WebSocket 1006) ─────────────
info "[V38-O2] Recargando config del gateway via SIGHUP…"
GATEWAY_PID=$(docker exec openclaw-server sh -c \
    'pgrep -f "openclaw.*gateway\|openclaw.*run\|node.*openclaw\|openclaw serve" 2>/dev/null | head -1 || true' \
    2>/dev/null || echo "")
if [[ -n "$GATEWAY_PID" ]]; then
    docker exec openclaw-server sh -c "kill -HUP ${GATEWAY_PID}" 2>/dev/null \
        && ok "[V38-O2] SIGHUP enviado al gateway (PID interno ${GATEWAY_PID})" \
        || { warn "[V38-O2] SIGHUP falló — usando docker restart como fallback"; docker restart openclaw-server >/dev/null; sleep 6; }
else
    warn "[V38-O2] PID del gateway no detectado — usando docker restart como fallback"
    docker restart openclaw-server >/dev/null
    sleep 6
fi

# ─── Esperar que el gateway HTTP esté activo ─────────────────────────────────
info "Esperando disponibilidad HTTP del gateway (máx 20s)…"
OPENCLAW_HTTP="000"
...
# ─── [V39-O2/O3] Registrar los 11 modelos del Router V14 vía API REST ───────
# Se usa POST /api/providers del gateway (puerto admin 18789) para añadir
# el provider con los modelos lógicos del Router V14. Al ir por API REST,
# los datos se persisten en la BD interna del gateway y doctor no los borra.
# Los ids son los exactos de GET /v1/models del Router V14.
info "[V39-O2] Registrando provider con modelos Router V14 vía API REST del gateway…"
sleep 2  # breve espera para que el gateway termine de inicializar la API

PROVIDER_RESPONSE=$(curl --noproxy "*" --ipv4 -s -m 15 \
    -X POST "http://localhost:${PORT_OPENCLAW_ADMIN}/api/providers" \
  ...
  \"models\": [
    ...
    {\"id\": \"masivo\",               \"name\": \"Masivo — Qwen2.5 32B · análisis extenso\"},
    {\"id\": \"agil\",                 \"name\": \"Ágil — SGLang · agentes, documentos\"},
    {\"id\": \"chat\",                 \"name\": \"Chat — Llama 3.1 8B EXL2\"},
    {\"id\": \"instantaneo\",          \"name\": \"Instantáneo — Qwen2.5 Coder 7B\"},
    {\"id\": \"codigo\",               \"name\": \"Código — Qwen Coder 7B (instruct)\"},
...
if echo "$PROVIDER_RESPONSE" | grep -q '"id"'; then
    ok "[V39-O2] Provider 'omen_router_v14' registrado via API — 11 modelos disponibles en OpenClaw"
else
    # Fallback: inyectar providers directamente en el JSON si la API no responde
    warn "[V39-O2] API REST no disponible (resp: ${PROVIDER_RESPONSE:0:80})"
    warn "[V39-O2] Fallback: inyectando providers en openclaw.json directamente…"
    docker exec openclaw-server sh -c "cat > /data/.openclaw/openclaw.json << 'OCLEOF'
{
  \"gateway\": {
    \"bind\": \"lan\",
...
OCLEOF"
    docker exec openclaw-server sh -c \
        'pgrep -f "openclaw.*gateway\|openclaw serve" | xargs -r kill -HUP' 2>/dev/null \
        || docker restart openclaw-server >/dev/null
    ok "[V39-O2] Fallback completado — reintentando con JSON directo"
fi

# ─── [V39-O3] Imprimir URL auto-login con fragment token ────────────────────
OPENCLAW_URL_TOKEN="http://localhost:${PORT_OPENCLAW}/#token=${OPENCLAW_TOKEN}"
info "[V39-O3] URL de acceso directo (token embebido en fragment — no se transmite al servidor):"
echo ""
echo -e "  ${GRN}${BLD}→ ${OPENCLAW_URL_TOKEN}${NC}"
echo ""
# Intentar abrir el navegador automáticamente si xdg-open está disponible
if command -v xdg-open &>/dev/null; then
    xdg-open "$OPENCLAW_URL_TOKEN" 2>/dev/null &
    ok "[V39-O3] Navegador abierto automáticamente con token embebido"
else
    info "[V39-O3] xdg-open no disponible — copia la URL de arriba en el navegador"
fi
...
# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V40"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V40 — iniciado${NC}"
