...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V54]
#
...
# ─── Limpieza ────────────────────────────────────────────────────────────────
info "[V54] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
info "[V54] Generando preseed (gateway.auth + controlUi)…"
...
info "[V54] Copiando preseed al volumen…"
docker run --rm \
...
# ─── Escribir inject_v54.py en el HOST (FUERA de cualquier bucle) ─────────────
...
# El heredoc usa comillas simples 'INJECT_PY_V54_EOF' para que bash NO expanda
# nada dentro; la sustitución de PORT_ROUTER se hace con sed después.
_INJECT_HOST_PATH="/tmp/inject_v54_host.py"
cat > "$_INJECT_HOST_PATH" << 'INJECT_PY_V54_EOF'
import json, sys
...
try:
    ...
    print("[V54] ERROR leyendo modelos: " + str(e))
    sys.exit(1)
...
try:
    ...
    print("[V54] WARN config no legible (" + str(e) + "), partiendo de vacio")
    cfg = {}
...
for _rm in ("ollama", "openai"):
    ...
    if removed:
        print("[V54] Provider externo eliminado: " + _rm)
...
print("[V54] OK: provider registrado: " + PROVIDER_ID)
print("[V54] OK: " + str(model_count) + " modelos bajo models.providers." + PROVIDER_ID + ".models")
print("[V54] OK: agents.defaults.model.primary = " + primary_now)
print("[V54] OK: plugins.entries." + PROVIDER_ID + ".enabled = true")
print("[V54] Providers en config: " + str(providers_now))
...
info "[V54] Esperando gateway (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if echo "$_LOG" | grep -q "\[health-monitor\] started"; then
        OPENCLAW_READY=true; ok "[V54] Gateway arrancado (health-monitor) tras $((_oc_i * 2))s"; break
    fi
    if echo "$_LOG" | grep -q "\[gateway\] security warning"; then
        OPENCLAW_READY=true; ok "[V54] Gateway UP (security-warning) tras $((_oc_i * 2))s"; break
    fi
    sleep 2
done
[[ "$OPENCLAW_READY" == "false" ]] && warn "[V54] Timeout gateway (${TIMEOUT_OPENCLAW}s)"

# ─── Esperar estado "running" ─────────────────────────────────────────────────
info "[V54] Verificando estado del contenedor…"
_OC_RUNNING=false
for _cr in $(seq 1 20); do
    _ST=$(docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo "missing")
    if [[ "$_ST" == "running" ]]; then
        _OC_RUNNING=true; ok "[V54] Contenedor 'running' tras $((_cr * 3))s"; break
    fi
    sleep 3
done
...
info "[V54] Esperando que Router V14 reporte backends activos…"
_HEALTH_JSON="{}"
...
" 2>/dev/null || echo "0")
    if [[ "$_BK" -gt 0 ]]; then
        _HEALTH_JSON="$_HJ"
        ok "[V54] Router reporta ${_BK} backends activos (intento ${_rw})"
        break
    fi
    info "[V54] Router sin backends aún (intento ${_rw}/10) — esperando 2s…"
    sleep 2
done

# ─── Construir lista de modelos ───────────────────────────────────────────────
info "[V54] Consultando backends activos en Router V14…"
...
info "[V54] Backends activos: ${_ACTIVE_BKS}"
...
if [[ -z "$_MODELS_JSON" || "$_MODEL_COUNT" -lt 2 ]]; then
    warn "[V54] Router sin backends confirmados — usando lista estática completa"
    _MODELS_JSON='[
...
info "[V54] Modelos a registrar: ${_MODEL_COUNT}"

# ─── Copiar modelos y script al contenedor y ejecutar ─────────────────────────
info "[V54] Inyectando provider OMEN Router V14 en openclaw.json…"
echo "$_MODELS_JSON" > /tmp/oc_models_v54.json

_INJECT_OK=false
for _inj_retry in 1 2 3; do
    # docker cp: copia desde HOST al contenedor — sin heredoc, sin stdin problems
    docker cp /tmp/oc_models_v54.json openclaw-server:/tmp/oc_models.json  2>/dev/null || true
    docker cp "$_INJECT_HOST_PATH"    openclaw-server:/tmp/inject_v54.py   2>/dev/null || true

    _OC_INJ_LOG="/tmp/oc_inject_v54_try${_inj_retry}.log"
    docker exec openclaw-server python3 /tmp/inject_v54.py > "$_OC_INJ_LOG" 2>&1
    _INJ_EXIT=$?
    ...
    if [[ $_INJ_EXIT -eq 0 ]] && grep -q "omen-router" "$_OC_INJ_LOG" 2>/dev/null; then
        _INJECT_OK=true
        ok "[V54] inject_v54.py completado en intento ${_inj_retry}"
        break
    else
        warn "[V54] inject fallido (intento ${_inj_retry}/3, exit=${_INJ_EXIT}) — esperando 5s…"
        sleep 5
    fi
done

if [[ "$_INJECT_OK" == "false" ]]; then
    warn "[V54] ATENCIÓN: inject no pudo completar. OpenClaw usará config de configure/doctor."
    warn "[V54] Comandos manuales para corregir:"
    warn "[V54]   docker cp /tmp/oc_models_v54.json openclaw-server:/tmp/oc_models.json"
    warn "[V54]   docker cp $_INJECT_HOST_PATH openclaw-server:/tmp/inject_v54.py"
    warn "[V54]   docker exec openclaw-server python3 /tmp/inject_v54.py"
    warn "[V54]   docker kill --signal=HUP openclaw-server"
fi

# ─── SIGHUP para reload del config ───────────────────────────────────────────
info "[V54] Enviando SIGHUP al gateway para reload del config…"
...
if [[ -n "$_GW_PID" && "$_GW_PID" =~ ^[0-9]+$ ]]; then
    ...
    ok "[V54] SIGHUP enviado al PID ${_GW_PID}"
else
...
info "[V54] Verificando config final…"
_OC_VERIFY_LOG="/tmp/oc_verify_v54.log"
...
try:
    ...
    print('[V54] Providers registrados: ' + str(provider_keys))
    print('[V54] Modelo primario: ' + primary_val)
    for pid in provider_keys:
        pdata  = providers[pid]
        mcount = len(pdata.get('models', []))
        base   = pdata.get('baseUrl', '?')
        print('[V54]   ' + pid + ' -> ' + base + '  (' + str(mcount) + ' modelos)')
    if 'omen-router' in providers:
        mc = len(providers['omen-router'].get('models', []))
        print('[V54] STATUS: OK omen-router registrado correctamente (' + str(mc) + ' modelos)')
    else:
        print('[V54] STATUS: FALLO omen-router NO encontrado en providers')
        sys.exit(2)
except Exception as e:
    print('[V54] ERROR verificando config: ' + str(e))
    sys.exit(1)
" > "$_OC_VERIFY_LOG" 2>&1
_VERIFY_EXIT=$?
...
if [[ $_VERIFY_EXIT -ne 0 ]]; then
    warn "[V54] Config sin omen-router. Corrección manual:"
    warn "[V54]   docker cp /tmp/oc_models_v54.json openclaw-server:/tmp/oc_models.json"
    warn "[V54]   docker cp $_INJECT_HOST_PATH openclaw-server:/tmp/inject_v54.py"
    warn "[V54]   docker exec openclaw-server python3 /tmp/inject_v54.py"
    warn "[V54]   docker kill --signal=HUP openclaw-server"
...
if command -v xdg-open &>/dev/null; then
    ...
    ok "[V54] Navegador abierto con token embebido"
else
    info "[V54] Abre manualmente: ${OPENCLAW_URL_TOKEN}"
fi

# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V54"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V54 — iniciado${NC}"
