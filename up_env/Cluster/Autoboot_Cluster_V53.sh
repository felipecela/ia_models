...
# ║             Se define SCRIPT_VERSION="V53" al inicio del script.           ║
...
set -euo pipefail
SCRIPT_VERSION="V53"  # [V44-B1] Versión del script — usada en banners y logs
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V53 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V53"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V53"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V53]
#
# CAMBIOS V52 (respecto a V51):
#   FIX-A  OPENAI_API_KEY=""         → configure no activa provider OpenAI
#   FIX-B  Sin OLLAMA_BASE_URL       → no registra provider Ollama (llama3.3 404)
#   FIX-C  f-strings sin backslashes → compatibilidad Python <3.12 en contenedor
#   FIX-D  Espera 8s + retry /health → router responde backends antes del inject
#   FIX-E  Fallback lista estática   → siempre registra los 6 modelos mínimos
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

# ─── Limpieza ────────────────────────────────────────────────────────────────
info "[V53] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
# ─── Preseed mínimo ─────────────────────────────────────────────────────────
# FIX-A: sin OPENAI_API_KEY activa en el docker run, configure no registra
#        el provider OpenAI → doctor no puede escalarlo a gpt-5.5.
# El preseed solo inyecta token de auth y flags controlUi.
info "[V53] Generando preseed (gateway.auth + controlUi)…"
...
info "[V53] Copiando preseed al volumen…"
docker run --rm \
...
# ─── Escribir inject_v53.py en el HOST (FUERA de cualquier bucle) ─────────────
# FIX-C: NINGÚN backslash dentro de expresiones f-string → Python <3.12 safe.
#        Las claves del dict se sacan a variables antes del print().
# El heredoc usa comillas simples 'INJECT_PY_V53_EOF' para que bash NO expanda
# nada dentro; la sustitución de PORT_ROUTER se hace con sed después.
_INJECT_HOST_PATH="/tmp/inject_v53_host.py"
cat > "$_INJECT_HOST_PATH" << 'INJECT_PY_V53_EOF'
import json, sys
...
try:
    ...
    print("[V53] ERROR leyendo modelos: " + str(e))
    sys.exit(1)
...
try:
    ...
    print("[V53] WARN config no legible (" + str(e) + "), partiendo de vacio")
    cfg = {}
...
# Eliminar providers externos (openai, ollama) que configure/doctor añadió
for _rm in ("ollama", "openai"):
    removed = cfg["models"]["providers"].pop(_rm, None)
    if removed:
        print("[V53] Provider externo eliminado: " + _rm)

# Registrar omen-router
...
cfg["agents"]["defaults"]["model"]["primary"] = PROVIDER_ID + "/ruteador-auto"
...
# FIX-C: sin backslashes en f-strings — variables intermedias para los valores
model_count   = len(models)
providers_now = list(cfg["models"]["providers"].keys())
primary_now   = cfg["agents"]["defaults"]["model"]["primary"]

print("[V53] OK: provider registrado: " + PROVIDER_ID)
print("[V53] OK: " + str(model_count) + " modelos bajo models.providers." + PROVIDER_ID + ".models")
print("[V53] OK: agents.defaults.model.primary = " + primary_now)
print("[V53] OK: plugins.entries." + PROVIDER_ID + ".enabled = true")
print("[V53] Providers en config: " + str(providers_now))
# Línea centinela que grep busca para confirmar éxito
print("omen-router")
INJECT_PY_V53_EOF

# Sustituir PORT_ROUTER_PLACEHOLDER con el valor real (en el HOST, antes del docker cp)
sed -i "s|PORT_ROUTER_PLACEHOLDER|${PORT_ROUTER}|g" "$_INJECT_HOST_PATH"
ok "[V53] inject_v53.py escrito en HOST: $_INJECT_HOST_PATH"

# ─── Lanzar openclaw-server ───────────────────────────────────────────────────
# FIX-A: OPENAI_API_KEY="" → configure no activa provider OpenAI
# FIX-B: sin OLLAMA_BASE_URL → no registra provider Ollama con llama3.3
info "[V53] Lanzando openclaw-server…"
docker run -d \
...
info "[V53] Esperando gateway (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if echo "$_LOG" | grep -q "\[health-monitor\] started"; then
        OPENCLAW_READY=true; ok "[V53] Gateway arrancado (health-monitor) tras $((_oc_i * 2))s"; break
    fi
    if echo "$_LOG" | grep -q "\[gateway\] security warning"; then
        OPENCLAW_READY=true; ok "[V53] Gateway UP (security-warning) tras $((_oc_i * 2))s"; break
    fi
    sleep 2
done
[[ "$OPENCLAW_READY" == "false" ]] && warn "[V53] Timeout gateway (${TIMEOUT_OPENCLAW}s)"

# ─── Esperar estado "running" ─────────────────────────────────────────────────
info "[V53] Verificando estado del contenedor…"
_OC_RUNNING=false
for _cr in $(seq 1 20); do
    _ST=$(docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo "missing")
    if [[ "$_ST" == "running" ]]; then
        _OC_RUNNING=true; ok "[V53] Contenedor 'running' tras $((_cr * 3))s"; break
    fi
    sleep 3
done
[[ "$_OC_RUNNING" == "false" ]] && warn "[V53] Contenedor NO 'running' — estado: $(docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo '?')"
sleep 3

# ─── FIX-D: espera activa del router antes del inject ────────────────────────
# El router se lanzó ~2min antes, pero el health check puede tardar en devolver
# backends activos. Esperamos hasta 20s con reintentos cada 2s.
info "[V53] Esperando que Router V14 reporte backends activos…"
_HEALTH_JSON="{}"
for _rw in $(seq 1 10); do
    _HJ=$(curl --noproxy "*" -s -m 5 "http://localhost:${PORT_ROUTER}/health" 2>/dev/null || echo "{}")
    _BK=$(echo "$_HJ" | python3 -c "
import json,sys
try:
...
" 2>/dev/null || echo "0")
    if [[ "$_BK" -gt 0 ]]; then
        _HEALTH_JSON="$_HJ"
        ok "[V53] Router reporta ${_BK} backends activos (intento ${_rw})"
        break
    fi
    info "[V53] Router sin backends aún (intento ${_rw}/10) — esperando 2s…"
    sleep 2
done

# ─── Construir lista de modelos ───────────────────────────────────────────────
info "[V53] Consultando backends activos en Router V14…"
...
info "[V53] Backends activos: ${_ACTIVE_BKS}"

_MODELS_JSON=$(echo "$_HEALTH_JSON" | python3 -c "
import json, sys
...
BACKEND_MODEL_MAP = [
    ...
    ('PRECISO',     'phi-mayor-precision', 'Phi Mayor Precision — phi4-reasoning:plus',  32768),
    ('PRECISO_OPT', 'phi-optimizada',      'Phi Optimizada — phi4-reasoning:14b-q4_K_M', 32768),
    ('MASIVO',      'masivo',              'Masivo — Qwen2.5 32B',                        32768),
    ('AGIL',        'agil',                'Agil — SGLang',                               32768),
    ('CHAT',        'chat',                'Chat — Llama 3.1 8B EXL2',                   32768),
    ('INSTANTANEO', 'instantaneo',         'Instantaneo — Qwen2.5 Coder 7B',             32768),
    ('CODIGO',      'codigo',              'Codigo — Qwen Coder 7B',                      32768),
]
...
if health.get('agent', {}).get('db_exists', False):
    models.append({'id': 'agent-autonomo', 'name': 'Agente Autonomo — planifica+ejecuta+valida', 'contextWindow': 32768})
...
# FIX-E: fallback estático garantizado — si el router aún no reporta backends
# (p.ej. deepseek-r1 todavía se está descargando), registramos la lista completa.
if [[ -z "$_MODELS_JSON" || "$_MODEL_COUNT" -lt 2 ]]; then
    warn "[V53] Router sin backends confirmados — usando lista estática completa"
    _MODELS_JSON='[
      {"id":"ruteador-auto",        "name":"Auto — clasificador 4 capas",                   "contextWindow":32768},
      {"id":"profundo",             "name":"Profundo — DeepSeek R1 14B",                    "contextWindow":32768},
      {"id":"phi-mayor-precision",  "name":"Phi Mayor Precision — phi4-reasoning:plus",     "contextWindow":32768},
      {"id":"phi-optimizada",       "name":"Phi Optimizada — phi4-reasoning:14b-q4_K_M",    "contextWindow":32768},
      {"id":"masivo",               "name":"Masivo — Qwen2.5 32B",                          "contextWindow":32768},
      {"id":"agent-autonomo",       "name":"Agente Autonomo — planifica+ejecuta+valida",    "contextWindow":32768},
      {"id":"phi4",                 "name":"Phi-4 CPU — clasificador directo",              "contextWindow":32768}
    ]'
    _MODEL_COUNT=7
fi
info "[V53] Modelos a registrar: ${_MODEL_COUNT}"

# ─── Copiar modelos y script al contenedor y ejecutar ─────────────────────────
info "[V53] Inyectando provider OMEN Router V14 en openclaw.json…"
echo "$_MODELS_JSON" > /tmp/oc_models_v53.json

_INJECT_OK=false
for _inj_retry in 1 2 3; do
    # docker cp: copia desde HOST al contenedor — sin heredoc, sin stdin problems
    docker cp /tmp/oc_models_v53.json openclaw-server:/tmp/oc_models.json  2>/dev/null || true
    docker cp "$_INJECT_HOST_PATH"    openclaw-server:/tmp/inject_v53.py   2>/dev/null || true

    _OC_INJ_LOG="/tmp/oc_inject_v53_try${_inj_retry}.log"
    docker exec openclaw-server python3 /tmp/inject_v53.py > "$_OC_INJ_LOG" 2>&1
    _INJ_EXIT=$?
    ...
    if [[ $_INJ_EXIT -eq 0 ]] && grep -q "omen-router" "$_OC_INJ_LOG" 2>/dev/null; then
        _INJECT_OK=true
        ok "[V53] inject_v53.py completado en intento ${_inj_retry}"
        break
    else
        warn "[V53] inject fallido (intento ${_inj_retry}/3, exit=${_INJ_EXIT}) — esperando 5s…"
        sleep 5
    fi
done

if [[ "$_INJECT_OK" == "false" ]]; then
    warn "[V53] ATENCIÓN: inject no pudo completar. OpenClaw usará config de configure/doctor."
    warn "[V53] Comandos manuales para corregir:"
    warn "[V53]   docker cp /tmp/oc_models_v53.json openclaw-server:/tmp/oc_models.json"
    warn "[V53]   docker cp $_INJECT_HOST_PATH openclaw-server:/tmp/inject_v53.py"
    warn "[V53]   docker exec openclaw-server python3 /tmp/inject_v53.py"
    warn "[V53]   docker kill --signal=HUP openclaw-server"
fi

# ─── SIGHUP para reload del config ───────────────────────────────────────────
info "[V53] Enviando SIGHUP al gateway para reload del config…"
...
if [[ -n "$_GW_PID" && "$_GW_PID" =~ ^[0-9]+$ ]]; then
    ...
    ok "[V53] SIGHUP enviado al PID ${_GW_PID}"
else
    docker kill --signal=HUP openclaw-server 2>/dev/null || true
    ok "[V53] SIGHUP enviado al contenedor"
fi
sleep 4
...
info "[V53] Verificando config final…"
_OC_VERIFY_LOG="/tmp/oc_verify_v53.log"
...
try:
    ...
    print('[V53] Providers registrados: ' + str(provider_keys))
    print('[V53] Modelo primario: ' + primary_val)
    for pid in provider_keys:
        pdata  = providers[pid]
        mcount = len(pdata.get('models', []))
        base   = pdata.get('baseUrl', '?')
        print('[V53]   ' + pid + ' -> ' + base + '  (' + str(mcount) + ' modelos)')
    if 'omen-router' in providers:
        mc = len(providers['omen-router'].get('models', []))
        print('[V53] STATUS: OK omen-router registrado correctamente (' + str(mc) + ' modelos)')
    else:
        print('[V53] STATUS: FALLO omen-router NO encontrado en providers')
        sys.exit(2)
except Exception as e:
    print('[V53] ERROR verificando config: ' + str(e))
    sys.exit(1)
" > "$_OC_VERIFY_LOG" 2>&1
_VERIFY_EXIT=$?
...
if [[ $_VERIFY_EXIT -ne 0 ]]; then
    warn "[V53] Config sin omen-router. Corrección manual:"
    warn "[V53]   docker cp /tmp/oc_models_v53.json openclaw-server:/tmp/oc_models.json"
    warn "[V53]   docker cp $_INJECT_HOST_PATH openclaw-server:/tmp/inject_v53.py"
    warn "[V53]   docker exec openclaw-server python3 /tmp/inject_v53.py"
    warn "[V53]   docker kill --signal=HUP openclaw-server"
...
if command -v xdg-open &>/dev/null; then
    ...
    ok "[V53] Navegador abierto con token embebido"
else
    info "[V53] Abre manualmente: ${OPENCLAW_URL_TOKEN}"
fi

# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V53"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V53 — iniciado${NC}"
