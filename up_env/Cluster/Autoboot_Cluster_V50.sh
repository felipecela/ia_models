...
# ║             Se define SCRIPT_VERSION="V50" al inicio del script.           ║
...
set -euo pipefail
SCRIPT_VERSION="V50"  # [V44-B1] Versión del script — usada en banners y logs
...
if [[ "$_FLAG_STOP" == "true" ]]; then
    ...
    ok "Cluster V50 detenido."
    exit 0
fi

# ─── [V25-F2] --status ───────────────────────────────────────────────────────
if [[ "$_FLAG_STATUS" == "true" ]]; then
    section "Estado del OMEN AI Cluster V50"
    echo ""
...
# INICIO
# ═══════════════════════════════════════════════════════════════════════════════
section "OMEN AI Cluster — Autoboot V50"
...
# ═══════════════════════════════════════════════════════════════════════════════
# 8/8 — OpenClaw Server (:$PORT_OPENCLAW)  [V50]
#
# ROOT CAUSE confirmado V49:
#   - configure añade providers: openai (OPENAI_API_KEY set) + ollama (OLLAMA_BASE_URL set)
#   - doctor upgrades model.primary de "openai/gpt-5.2" → "openai/gpt-5.5"
#   - timeout de 50s se agotó ANTES de que health-monitor dijera "started"
#     (linuxbrew init + doctor pueden tardar 60-90s en arranque en frío)
#   - inject_v49.py nunca ejecutó → config quedó con openai/gpt-5.5 como primario
#   - verificación final imprimía OK aunque omen-router NO estaba registrado
#
# FIX V50:
#   1. TIMEOUT_OPENCLAW = 120s (en variables arriba ya ajustado)
#   2. Esperar status "running" del contenedor ANTES de intentar docker exec
#   3. inject con 3 reintentos (5s entre intentos) con verificación explícita
#   4. Verificación final comprueba que 'omen-router' está en providers;
#      si no → WARN rojo visible (no OK silencioso)
#   5. Eliminación explícita de provider 'openai' además de 'ollama'
# ═══════════════════════════════════════════════════════════════════════════════
section "8/8 — OpenClaw Server (:$PORT_OPENCLAW)"

_OC_PROVIDER_ID="omen-router"

# ─── Limpieza ────────────────────────────────────────────────────────────────
info "[V50] Deteniendo y eliminando contenedor previo 'openclaw-server'…"
...
# ─── Preseed mínimo: solo gateway.auth + controlUi ───────────────────────────
info "[V50] Generando preseed (gateway.auth + controlUi)…"
...
info "[V50] Copiando preseed al volumen…"
docker run --rm \
...
# [V50] OPENAI_API_KEY satisface la validación obligatoria del entrypoint.
# [V50] OLLAMA_BASE_URL (sin /v1) permite que configure detecte Ollama local.
# [V50] inject_v50.py elimina openai + ollama y registra omen-router.
info "[V50] Lanzando openclaw-server…"
docker run -d \
    ...
    -e OPENAI_API_KEY="sk-ollama-local" \
    -e OLLAMA_BASE_URL="http://host.docker.internal:${PORT_OLLAMA_CPU}" \
    -v openclaw_data_final:/data \
    "$IMG_OPENCLAW"

# ─── Esperar health-monitor (máx TIMEOUT_OPENCLAW=120s) ──────────────────────
# [V50] El timeout aumentó de 50s a 120s para absorber linuxbrew + doctor.
# Señal buscada: "[health-monitor] started" en los logs del contenedor.
info "[V50] Esperando que doctor + gateway terminen (máx ${TIMEOUT_OPENCLAW}s)…"
...
for _oc_i in $(seq 1 $((TIMEOUT_OPENCLAW / 2))); do
    ...
    if echo "$_LOG" | grep -q "\[health-monitor\] started"; then
        OPENCLAW_READY=true
        ok "[V50] Gateway arrancado tras $((_oc_i * 2))s"
        break
    fi
    if echo "$_LOG" | grep -q "\[gateway\] security warning"; then
        OPENCLAW_READY=true
        ok "[V50] Gateway UP (security-warning detectado) tras $((_oc_i * 2))s"
        break
    fi
    sleep 2
done
if [[ "$OPENCLAW_READY" == "false" ]]; then
    warn "[V50] Timeout esperando health-monitor (${TIMEOUT_OPENCLAW}s)"
...
# [V50] Antes del inject, verificar que no esté en "restarting".
info "[V50] Verificando estado del contenedor antes del inject…"
_OC_RUNNING=false
for _cr in $(seq 1 20); do
    _ST=$(docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo "missing")
    if [[ "$_ST" == "running" ]]; then
        _OC_RUNNING=true
        ok "[V50] Contenedor en estado 'running' tras $((_cr * 3))s"
        break
    fi
    sleep 3
done
if [[ "$_OC_RUNNING" == "false" ]]; then
    warn "[V50] Contenedor NO está en estado 'running' — inject puede fallar"
    _ST_FINAL=$(docker inspect -f '{{.State.Status}}' openclaw-server 2>/dev/null || echo "missing")
    warn "[V50] Estado actual: $_ST_FINAL"
fi
sleep 3

# ─── Consultar backends activos en Router V14 ────────────────────────────────
info "[V50] Consultando backends activos en Router V14…"
_HEALTH_JSON=$(curl --noproxy "*" -s -m 5 "http://localhost:${PORT_ROUTER}/health" 2>/dev/null || echo "{}")
_ACTIVE_BKS=$(echo "$_HEALTH_JSON" | python3 -c "
import json,sys
try:
    d=json.load(sys.stdin)
    b=d.get('backends',{})
    active=[k for k,v in b.items() if v]
    print(', '.join(active) if active else 'ninguno')
except:
    print('error')
" 2>/dev/null || echo "desconocido")
info "[V50] Backends activos: ${_ACTIVE_BKS}"

# ─── Construir lista de modelos según backends activos ───────────────────────
cat > /tmp/build_models_v50.py << 'PYEOF'
import json, sys
health_raw = sys.stdin.read().strip()
try:
...
BACKEND_MODEL_MAP = [
    ("PROFUNDO",    "profundo",            "Profundo — DeepSeek R1 14B",                  32768),
    ("PRECISO",     "phi-mayor-precision", "Phi Mayor Precisión — phi4-reasoning:plus",   32768),
    ("PRECISO_OPT", "phi-optimizada",      "Phi Optimizada — phi4-reasoning:14b-q4_K_M",  32768),
    ("MASIVO",      "masivo",              "Masivo — Qwen2.5 32B",                         32768),
    ("AGIL",        "agil",                "Ágil — SGLang",                                32768),
    ("CHAT",        "chat",                "Chat — Llama 3.1 8B EXL2",                    32768),
    ("INSTANTANEO", "instantaneo",         "Instantáneo — Qwen2.5 Coder 7B",              32768),
    ("CODIGO",      "codigo",              "Código — Qwen Coder 7B",                       32768),
]
models = [{"id": "ruteador-auto", "name": "Auto — clasificador 4 capas", "contextWindow": 32768}]
for backend, mid, mname, ctx in BACKEND_MODEL_MAP:
    if backends.get(backend, False):
        models.append({"id": mid, "name": mname, "contextWindow": ctx})
if health.get("agent", {}).get("db_exists", False):
    models.append({"id": "agent-autonomo", "name": "Agente Autónomo — planifica+ejecuta+valida", "contextWindow": 32768})
if health.get("clasificador", {}).get("phi4_cpu_avail", False):
    models.append({"id": "phi4", "name": "Phi-4 CPU — clasificador directo", "contextWindow": 32768})
print(json.dumps(models, ensure_ascii=False))
PYEOF

_MODELS_JSON=$(echo "$_HEALTH_JSON" | python3 /tmp/build_models_v50.py 2>/dev/null || echo "[]")
_MODEL_COUNT=$(echo "$_MODELS_JSON" | python3 -c "import json,sys; print(len(json.load(sys.stdin)))" 2>/dev/null || echo "0")

if [[ "$_MODELS_JSON" == "[]" || -z "$_MODELS_JSON" || "$_MODEL_COUNT" == "0" ]]; then
    warn "[V50] Usando lista de modelos estática (fallback)"
    _MODELS_JSON='[{"id":"ruteador-auto","name":"Auto — clasificador 4 capas","contextWindow":32768},{"id":"profundo","name":"Profundo — DeepSeek R1 14B","contextWindow":32768},{"id":"phi-mayor-precision","name":"Phi Mayor Precisión — phi4-reasoning:plus","contextWindow":32768},{"id":"phi-optimizada","name":"Phi Optimizada — phi4-reasoning:14b-q4_K_M","contextWindow":32768},{"id":"masivo","name":"Masivo — Qwen2.5 32B","contextWindow":32768},{"id":"agent-autonomo","name":"Agente Autónomo — planifica+ejecuta+valida","contextWindow":32768}]'
    _MODEL_COUNT=6
fi
info "[V50] Modelos a registrar: ${_MODEL_COUNT}"

# ─── Preparar inject_v50.py ───────────────────────────────────────────────────
echo "$_MODELS_JSON" > /tmp/oc_models_v50.json

# ─── Copiar y ejecutar inject_v50.py (con 3 reintentos) ──────────────────────
info "[V50] Inyectando provider OMEN Router V14 en openclaw.json…"

_INJECT_SCRIPT_CONTENT='import json, sys

CFG_PATH    = "/data/.openclaw/openclaw.json"
MODELS_PATH = "/tmp/oc_models.json"
PROVIDER_ID = "omen-router"
ROUTER_URL  = "http://host.docker.internal:PORT_ROUTER_PLACEHOLDER/v1"
API_KEY     = "sk-router-local"

try:
    with open(MODELS_PATH) as f:
        models = json.load(f)
except Exception as e:
    print(f"[V50] ERROR leyendo modelos: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"[V50] WARN: config no legible ({e}), partiendo de dict vacío")
    cfg = {}

# Asegurar models.providers como objeto dict
...
# Eliminar providers inyectados por configure y doctor
for _rm in ("ollama", "openai"):
    removed = cfg["models"]["providers"].pop(_rm, None)
    if removed:
        print(f"[V50] Provider externo eliminado: {_rm}")
...
cfg["agents"]["defaults"]["model"]["primary"] = f"{PROVIDER_ID}/ruteador-auto"

# Plugin habilitado — eliminar openai y ollama, añadir omen-router
...
print(f"[V50] OK: provider registrado: {PROVIDER_ID}")
print(f"[V50] OK: {len(models)} modelos bajo models.providers.{PROVIDER_ID}.models")
print(f"[V50] OK: agents.defaults.model.primary = {PROVIDER_ID}/ruteador-auto")
print(f"[V50] OK: plugins.entries.{PROVIDER_ID}.enabled = true")
print(f"[V50] Providers en config: {list(cfg[\"models\"][\"providers\"].keys())}")
'

_INJECT_OK=false
for _inj_retry in 1 2 3; do
    # Copiar modelos y script
    docker cp /tmp/oc_models_v50.json  openclaw-server:/tmp/oc_models.json 2>/dev/null || true
    docker exec openclaw-server sh -c "cat > /tmp/inject_v50.py" << INJECT_EOF
${_INJECT_SCRIPT_CONTENT}
INJECT_EOF
    docker exec openclaw-server sh -c \
        "sed -i 's|PORT_ROUTER_PLACEHOLDER|${PORT_ROUTER}|g' /tmp/inject_v50.py" 2>/dev/null || true

    _OC_INJ_LOG="/tmp/oc_inject_v50_try${_inj_retry}.log"
    docker exec openclaw-server python3 /tmp/inject_v50.py > "$_OC_INJ_LOG" 2>&1
    _INJ_EXIT=$?

    if [[ -f "$_OC_INJ_LOG" ]]; then
        while IFS= read -r _il; do
            if echo "$_il" | grep -qi "error\|fail\|exit"; then warn "  $_il"; else info "  $_il"; fi
        done < "$_OC_INJ_LOG"
    fi

    if [[ $_INJ_EXIT -eq 0 ]] && grep -q "omen-router" "$_OC_INJ_LOG" 2>/dev/null; then
        _INJECT_OK=true
        ok "[V50] inject_v50.py completado en intento ${_inj_retry}"
        break
    else
        warn "[V50] inject fallido (intento ${_inj_retry}/3) — esperando 5s…"
        sleep 5
    fi
done

if [[ "$_INJECT_OK" == "false" ]]; then
    warn "[V50] ATENCIÓN: inject_v50.py no pudo completar. OpenClaw usará provider 'openai' (no omen-router)."
    warn "[V50] Ejecuta manualmente: docker exec openclaw-server python3 /tmp/inject_v50.py"
fi

# ─── SIGHUP para reload del config ───────────────────────────────────────────
info "[V50] Enviando SIGHUP al gateway para reload del config…"
...
if [[ -n "$_GW_PID" && "$_GW_PID" =~ ^[0-9]+$ ]]; then
    ...
    ok "[V50] SIGHUP enviado al PID ${_GW_PID}"
else
    docker kill --signal=HUP openclaw-server 2>/dev/null || true
    ok "[V50] SIGHUP enviado al contenedor"
fi
sleep 4
...
# ─── Verificación final con comprobación explícita de omen-router ────────────
info "[V50] Verificando config final…"
_OC_VERIFY_LOG="/tmp/oc_verify_v50.log"
docker exec openclaw-server python3 - << 'VERIFY_EOF' > "$_OC_VERIFY_LOG" 2>&1
import json, sys
try:
    ...
    print(f"[V50] Providers registrados: {list(providers.keys())}")
    print(f"[V50] Modelo primario: {primary}")
    for pid, pdata in providers.items():
        mcount = len(pdata.get('models', []))
        print(f"[V50]   {pid} → {pdata.get('baseUrl', '?')}  ({mcount} modelos)")
    if 'omen-router' in providers:
        print("[V50] STATUS: ✅ omen-router registrado correctamente")
    else:
        print("[V50] STATUS: ❌ omen-router NO encontrado en providers")
        sys.exit(2)
except Exception as e:
    print(f"[V50] ERROR verificando config: {e}")
    sys.exit(1)
VERIFY_EOF
_VERIFY_EXIT=$?
...
if [[ -f "$_OC_VERIFY_LOG" ]]; then
    while IFS= read -r _vl; do
        if echo "$_vl" | grep -q "STATUS: ✅"; then
            ok "  $_vl"
        elif echo "$_vl" | grep -qi "❌\|ERROR\|STATUS.*no"; then
            warn "  $_vl"
        else
            ok "  $_vl"
        fi
...
if [[ $_VERIFY_EXIT -ne 0 ]]; then
    warn "[V50] Config final NO contiene omen-router. Reintentar inject manualmente:"
    warn "[V50]   docker exec openclaw-server python3 /tmp/inject_v50.py"
    warn "[V50]   docker kill --signal=HUP openclaw-server"
fi
...
if command -v xdg-open &>/dev/null; then
    ...
    ok "[V50] Navegador abierto con token embebido"
else
    info "[V50] Abre manualmente: ${OPENCLAW_URL_TOKEN}"
fi

# RESUMEN FINAL
# ═══════════════════════════════════════════════════════════════════════════════
section "Resumen del Cluster V50"
...
echo ""
echo -e "${GRN}${BLD}OMEN AI Cluster V50 — iniciado${NC}"
